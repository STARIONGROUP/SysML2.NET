...
blocks `Skill(...)` calls. This makes the researcher's Hypha grounding path
(`Skill(hypha:metamodel-lookup)`, `Skill(hypha:spec-citation)`, when
`{{GROUNDING_MODE}} == hypha`) completely unreachable from the sub-agent, and
the researcher also cannot `Write` its notes file. `mode: "acceptEdits"`
overrides the inheritance so the sub-agent can invoke both Skills and Writes
as its role requires. Discovered during a dry-run against
`SysML2.NET/Extend/StepExtensions.cs` (2026-07-01).

If a `Skill(hypha:*)` call is still denied inside the sub-agent, the
researcher must NOT silently fall back to reading raw sources — it messages
the orchestrator, which runs the Hypha lookup from its own context and
returns the result (or writes the notes file itself).

Applies to: researcher (5), implementer (6), targeted-fixture tester (6),
regression-sweep tester (8), reviewer (9). If any spawn omits
`mode: "acceptEdits"`, the run may silently degrade to the deferred-write
workaround (see the team template's "Plan-mode-aware prompting" section),
which is much slower and requires the orchestrator to apply on-behalf writes.

## Pre-flight: Hypha plugin detection (runs before Gate 0)

The Hypha plugin is the preferred grounding source for SysML v2 / KerML
lookups (`hypha:metamodel-lookup` for structure, `hypha:spec-citation` for
intent). When installed, the researcher grounds in Hypha; when not, the
researcher grounds in the checked-in XMI metamodel. See
`.claude/team-templates/extension-impl.md` → "How Hypha grounding plugs in"
for the design.
...
1. **Detect availability**. Record `hypha_available: bool` on the
   ...
   are the authoritative check). Derive `{{GROUNDING_MODE}}`: `hypha` when
   `hypha_available == true`, else `xmi`.

2. **When `hypha_available == true`** — no user interaction needed. The
   researcher at step 5 gets the hypha-mode grounding section of the
   template's researcher prompt.

3. **When `hypha_available == false`** — surface a short recommendation to
   the user (once per invocation, in the same orchestrator turn as the
   Gate 0 plan / step 4 sanity check, whichever comes first):
   > The Hypha plugin isn't installed. It grounds SysML v2 / KerML research
   > in the metamodel and the normative spec text — installing it is
   > recommended. Proceeding with research grounded in the checked-in XMI
   > metamodel.

   Do NOT block the run. Do NOT loop the recommendation. The researcher at
   step 5 gets the xmi-mode grounding section instead.

4. **Mid-run downgrade**: if a Hypha tool call errors during step 5 (plugin
   removed / knowledge base not built / auth failure), the researcher
   completes the remaining methods grounded in the XMI, flags each affected
   method in `{{NOTES_FILE}}`, and the orchestrator surfaces the downgrade
   explicitly in step 10's final summary so the user knows which methods
   didn't get Hypha grounding.
...
2. **Write the plan file** (at the path Claude Code provides in the plan-mode system reminder) with the standard structure (Context, Recommended approach, Critical files, Verification). The "Recommended approach" section is a concise description of the proposed execution:
   ...
   - Workflow shape: step 5 (researcher spawn — Hypha-grounded when `hypha_available`, else XMI-grounded) → step 5.5 (Gate R-A approval on the notes file) → step 6 (implementer + tester) → step 7 (verification) → step 8 (regression sweep) → step 9 (reviewer) → step 10 (final summary) → step 11 (issue checklist sync).
   - `hypha_available` value (and the resulting researcher grounding mode) from the pre-flight above, so the user sees the workflow shape they'll actually get.
   - Explicit mention of the structural checkpoint after this one: Gate R-A fires after the researcher's notes are inline-previewed (one more checkpoint after this one).
...
- **Notes file**: `.team-notes/<foo>-extensions-spec.md` (kebab-case
  `<foo>`). The `.team-notes/` directory is gitignored at `.gitignore` line
  `/.team-notes/*`, so the per-run notes file is per-contributor scratch.
...
- **trivial signals**
  - No OCL block — prose-derived redefinition with sibling precedent (e.g.
...
- Trivial 2-method no-OCL file → Haiku researcher, Sonnet for the
  rest.
...
- The auto-derived paths (test fixture, interface, reference template, notes
  file `{{NOTES_FILE}}`).
- `hypha_available` (`true` / `false`) and the resulting researcher grounding
  mode (`hypha` / `xmi`). When `hypha_available == false` and this session
  has not been recommended the plugin yet, include the short install
  recommendation from the pre-flight in the sanity-check text — do NOT add
  it as a question option (it's informational; the user acts on it
  out-of-band).
...
list from step 4, and include the grounding section of the researcher prompt
selected by `{{GROUNDING_MODE}}` from the pre-flight (`hypha` when the plugin
is installed, else `xmi`).

Spawn the **researcher** as
`Agent({subagent_type: "general-purpose", model: <researcher_model>})`
with the v2 researcher prompt, where `<researcher_model>` is the model
...
- Treat the OCL `<defaultValue>`/`<ownedRule>` body in the XMI as the canonical
  source of truth — retrieved via `hypha:metamodel-lookup` in `hypha` mode,
  grepped from the XMI directly in `xmi` mode.
- Fall back to the OCL block in the production file's `<remarks>` (mirrored from
  XMI by codegen).
- For methods with NO OCL body (e.g. `Type::isConjugated`), record a short
  prose derivation rule — grounded in `hypha:spec-citation` (with the clause)
  in `hypha` mode, or in the XMI `ownedComment` prose with an explicit
  `prose-only (no OCL; Hypha not installed)` flag in `xmi` mode — and
  EXPLICITLY FLAG the no-OCL origin in the notes.
...
**Post-researcher coverage check**: after the agent returns, read
`{{NOTES_FILE}}` yourself to verify coverage. Re-dispatch the researcher if
the file is empty / missing sections / didn't flag no-OCL cases where
applicable. Do NOT proceed until the notes file is complete. Then proceed to
step 5.5 (Gate R-A).
...
1. **Render** an inline preview in the chat response. Pull from
   ...
   - Per method:
     ...
     - Derivation source tag: `OCL in XMI`, `OCL in <remarks>`,
       `hypha:metamodel-lookup(<metaclass>::<property>)`,
       `hypha:spec-citation(<clause>)`, or
...
2. **Ask** via `AskUserQuestion` (single question, 2 options — "drop files" collapses out in single-file mode):
   ...
   - **Abort — research again with feedback**. Orchestrator forwards the user's free-form `Other` text to the researcher via `SendMessage`, waits for a fresh `spec ready`, then re-runs step 5.5.
...
first (substitute the concrete path before spawning).
...
- Test counts (X/Y green for the targeted fixture; M/N green for the full solution).
- Reviewer verdict + any unresolved findings.
- **Grounding**: which mode the researcher ran in (`hypha` / `xmi` /
...
- No-OCL (prose-derived) methods (e.g. `IsConjugated`) — flag separately so
  the user knows the implementation is grounded in spec prose (via
  `hypha:spec-citation`) or XMI comment prose rather than OCL.
...
- The researcher runs **FIRST and is mandatory** — even when the production file's
  ...
  role that handles no-OCL methods cleanly. Its grounding mode is set by the
  pre-flight (`hypha` when the plugin is installed, else `xmi`) — see the
  pre-flight section at the top of this file and the template's "How Hypha
  grounding plugs in" section.
