...
namespace SysML2.NET.Dal
{
    ...
    /// <summary>
    /// A static class that provides extension methods for the <see cref="Core.POCO.Systems.Requirements.SatisfyRequirementUsage"/> class
    /// </summary>
    public static class SatisfyRequirementUsageExtensions
    {
        /// <summary>
        /// Updates the value properties of the <see cref="Core.POCO.Systems.Requirements.SatisfyRequirementUsage"/> by setting the value equal to that of the dto
        /// Removes deleted objects from the reference properties and returns the unique identifiers
        /// of the objects that have been removed from contained properties
        /// </summary>
        /// <param name="poco">
        /// The <see cref="Core.POCO.Systems.Requirements.SatisfyRequirementUsage"/> that is to be updated
        /// </param>
        /// <param name="dto">
        /// The DTO that is used to update the <see cref="Core.DTO.Systems.Requirements.SatisfyRequirementUsage"/> with
        /// </param>
        /// <returns>
        ...
        /// </exception>
        public static IEnumerable<Guid> UpdateValueAndRemoveDeletedReferenceProperties(this Core.POCO.Systems.Requirements.SatisfyRequirementUsage poco, Core.DTO.Systems.Requirements.SatisfyRequirementUsage dto)
        {
            if (poco == null)
            {
        ...
        /// <summary>
        /// Updates the Reference properties of the <see cref="Core.POCO.Systems.Requirements.SatisfyRequirementUsage"/> using the data (identifiers) encapsulated in the DTO
        ...
        public static void UpdateReferenceProperties(this Core.POCO.Systems.Requirements.SatisfyRequirementUsage poco, Core.DTO.Systems.Requirements.SatisfyRequirementUsage dto, ConcurrentDictionary<Guid, Lazy<Core.POCO.Root.Elements.IElement>> cache)
        {
            if (poco == null)
            {
                throw new ArgumentNullException(nameof(poco), $"the {nameof(poco)} may not be null");
        ...
        /// <summary>
        /// Creates a <see cref="Core.DTO.Systems.Requirements.SatisfyRequirementUsage"/> based on the provided POCO
        /// </summary>
        /// <param name="poco">
        /// The subject <see cref="Core.POCO.Systems.Requirements.SatisfyRequirementUsage"/> from which a DTO is to be created
        /// </param>
        /// <param name="includeDerivedProperties">
        /// Asserts that derived properties should also be mapped during the creation of the <see cref="Core.DTO.Systems.Requirements.SatisfyRequirementUsage"/>
        /// </param>
        /// <returns>
        /// An instance of <see cref="Core.POCO.Systems.Requirements.SatisfyRequirementUsage"/>
        /// </returns>
        public static Core.DTO.Systems.Requirements.SatisfyRequirementUsage ToDto(this Core.POCO.Systems.Requirements.SatisfyRequirementUsage poco, bool includeDerivedProperties = false)
        {
            var dto = new Core.DTO.Systems.Requirements.SatisfyRequirementUsage();
            ...
            if (includeDerivedProperties)
            {
                ...
                dto.assumedConstraint = poco.assumedConstraint.Select(x => x.Id).ToList();
                ...
                dto.framedConcern = poco.framedConcern.Select(x => x.Id).ToList();
                ...
                dto.requiredConstraint = poco.requiredConstraint.Select(x => x.Id).ToList();
                dto.requirementDefinition = poco.requirementDefinition?.Id;
                dto.result = poco.result.Id;
                dto.satisfiedRequirement = poco.satisfiedRequirement.Id;
                dto.satisfyingFeature = poco.satisfyingFeature.Id;
                dto.shortName = poco.shortName;
                dto.stakeholderParameter = poco.stakeholderParameter.Select(x => x.Id).ToList();
                dto.subjectParameter = poco.subjectParameter.Id;
                dto.text = poco.text;
