...
namespace SysML2.NET.Dal
{
    ...
    using Core.POCO.Systems.Flows;

    /// <summary>
    /// A static class that provides extension methods for the <see cref="Core.POCO.Systems.Flows.SuccessionFlowUsage"/> class
    /// </summary>
    public static class SuccessionFlowUsageExtensions
    {
        /// <summary>
        /// Updates the value properties of the <see cref="Core.POCO.Systems.Flows.SuccessionFlowUsage"/> by setting the value equal to that of the dto
        /// Removes deleted objects from the reference properties and returns the unique identifiers
        /// of the objects that have been removed from contained properties
        /// </summary>
        /// <param name="poco">
        /// The <see cref="Core.POCO.Systems.Flows.SuccessionFlowUsage"/> that is to be updated
        /// </param>
        /// <param name="dto">
        /// The DTO that is used to update the <see cref="Core.DTO.Systems.Flows.SuccessionFlowUsage"/> with
        /// </param>
        /// <returns>
        ...
        /// </exception>
        public static IEnumerable<Guid> UpdateValueAndRemoveDeletedReferenceProperties(this Core.POCO.Systems.Flows.SuccessionFlowUsage poco, Core.DTO.Systems.Flows.SuccessionFlowUsage dto)
        {
            if (poco == null)
            {
        ...
        /// <summary>
        /// Updates the Reference properties of the <see cref="Core.POCO.Systems.Flows.SuccessionFlowUsage"/> using the data (identifiers) encapsulated in the DTO
        ...
        public static void UpdateReferenceProperties(this Core.POCO.Systems.Flows.SuccessionFlowUsage poco, Core.DTO.Systems.Flows.SuccessionFlowUsage dto, ConcurrentDictionary<Guid, Lazy<Core.POCO.Root.Elements.IElement>> cache)
        {
            if (poco == null)
            {
                throw new ArgumentNullException(nameof(poco), $"the {nameof(poco)} may not be null");
        ...
        /// <summary>
        /// Creates a <see cref="Core.DTO.Systems.Flows.SuccessionFlowUsage"/> based on the provided POCO
        /// </summary>
        /// <param name="poco">
        /// The subject <see cref="Core.POCO.Systems.Flows.SuccessionFlowUsage"/> from which a DTO is to be created
        /// </param>
        /// <param name="includeDerivedProperties">
        /// Asserts that derived properties should also be mapped during the creation of the <see cref="Core.DTO.Systems.Flows.SuccessionFlowUsage"/>
        /// </param>
        /// <returns>
        /// An instance of <see cref="Core.POCO.Systems.Flows.SuccessionFlowUsage"/>
        /// </returns>
        public static Core.DTO.Systems.Flows.SuccessionFlowUsage ToDto(this Core.POCO.Systems.Flows.SuccessionFlowUsage poco, bool includeDerivedProperties = false)
        {
            var dto = new Core.DTO.Systems.Flows.SuccessionFlowUsage();
            ...
            if (includeDerivedProperties)
            {
                ...
                dto.flowDefinition = poco.flowDefinition.Select(x => x.Id).ToList();
                dto.flowEnd = poco.flowEnd.Select(x => x.Id).ToList();
                ...
                dto.payloadFeature = poco.payloadFeature?.Id;
                dto.payloadType = poco.payloadType.Select(x => x.Id).ToList();
                ...
                dto.sourceOutputFeature = poco.sourceOutputFeature?.Id;
                dto.targetFeature = poco.targetFeature.Select(x => x.Id).ToList();
                dto.targetInputFeature = poco.targetInputFeature?.Id;
