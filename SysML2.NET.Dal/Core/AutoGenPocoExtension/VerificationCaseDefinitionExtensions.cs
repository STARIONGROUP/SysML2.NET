...
namespace SysML2.NET.Dal
{
    ...
    using Core.POCO.Systems.VerificationCases;

    /// <summary>
    /// A static class that provides extension methods for the <see cref="Core.POCO.Systems.VerificationCases.VerificationCaseDefinition"/> class
    /// </summary>
    public static class VerificationCaseDefinitionExtensions
    {
        /// <summary>
        /// Updates the value properties of the <see cref="Core.POCO.Systems.VerificationCases.VerificationCaseDefinition"/> by setting the value equal to that of the dto
        /// Removes deleted objects from the reference properties and returns the unique identifiers
        /// of the objects that have been removed from contained properties
        /// </summary>
        /// <param name="poco">
        /// The <see cref="Core.POCO.Systems.VerificationCases.VerificationCaseDefinition"/> that is to be updated
        /// </param>
        /// <param name="dto">
        /// The DTO that is used to update the <see cref="Core.DTO.Systems.VerificationCases.VerificationCaseDefinition"/> with
        /// </param>
        /// <returns>
        ...
        /// </exception>
        public static IEnumerable<Guid> UpdateValueAndRemoveDeletedReferenceProperties(this Core.POCO.Systems.VerificationCases.VerificationCaseDefinition poco, Core.DTO.Systems.VerificationCases.VerificationCaseDefinition dto)
        {
            if (poco == null)
            {
        ...
        /// <summary>
        /// Updates the Reference properties of the <see cref="Core.POCO.Systems.VerificationCases.VerificationCaseDefinition"/> using the data (identifiers) encapsulated in the DTO
        ...
        public static void UpdateReferenceProperties(this Core.POCO.Systems.VerificationCases.VerificationCaseDefinition poco, Core.DTO.Systems.VerificationCases.VerificationCaseDefinition dto, ConcurrentDictionary<Guid, Lazy<Core.POCO.Root.Elements.IElement>> cache)
        {
            if (poco == null)
            {
                throw new ArgumentNullException(nameof(poco), $"the {nameof(poco)} may not be null");
        ...
        /// <summary>
        /// Creates a <see cref="Core.DTO.Systems.VerificationCases.VerificationCaseDefinition"/> based on the provided POCO
        /// </summary>
        /// <param name="poco">
        /// The subject <see cref="Core.POCO.Systems.VerificationCases.VerificationCaseDefinition"/> from which a DTO is to be created
        /// </param>
        /// <param name="includeDerivedProperties">
        /// Asserts that derived properties should also be mapped during the creation of the <see cref="Core.DTO.Systems.VerificationCases.VerificationCaseDefinition"/>
        /// </param>
        /// <returns>
        /// An instance of <see cref="Core.POCO.Systems.VerificationCases.VerificationCaseDefinition"/>
        /// </returns>
        public static Core.DTO.Systems.VerificationCases.VerificationCaseDefinition ToDto(this Core.POCO.Systems.VerificationCases.VerificationCaseDefinition poco, bool includeDerivedProperties = false)
        {
            var dto = new Core.DTO.Systems.VerificationCases.VerificationCaseDefinition();
            ...
            {
                dto.action = poco.action.Select(x => x.Id).ToList();
                dto.actorParameter = poco.actorParameter.Select(x => x.Id).ToList();
                dto.calculation = poco.calculation.Select(x => x.Id).ToList();
                ...
                dto.verifiedRequirement = poco.verifiedRequirement.Select(x => x.Id).ToList();
            }
