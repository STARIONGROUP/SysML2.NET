...
namespace SysML2.NET.Serializer.Json.Core.DTO
{
    ...
    internal static class FeatureChainExpressionDeSerializer
    {
        ...
        internal static IFeatureChainExpression DeSerialize(JsonElement jsonElement, SerializationModeKind serializationModeKind, ILoggerFactory loggerFactory = null)
        {
            ...
            {
                logger.LogDebug("the behavior Json property was not found in the FeatureChainExpression: { Id }", dtoInstance.Id);
            }
            ...
            {
                logger.LogDebug("the directedFeature Json property was not found in the FeatureChainExpression: { Id }", dtoInstance.Id);
            }
            ...
            {
                if (targetFeatureProperty.ValueKind == JsonValueKind.Null)
                {
                ...
                {
                    if (targetFeatureProperty.TryGetProperty("@id"u8, out var targetFeatureExternalIdProperty))
                    {
                        var propertyValue = targetFeatureExternalIdProperty.GetString();

                        if (propertyValue != null)
                        {
                            dtoInstance.targetFeature = Guid.Parse(propertyValue);
                        }
                    }
                }
            }
