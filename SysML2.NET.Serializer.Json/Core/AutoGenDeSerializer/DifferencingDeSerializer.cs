...
namespace SysML2.NET.Serializer.Json.Core.DTO
{
    ...
    internal static class DifferencingDeSerializer
    {
        ...
        internal static IDifferencing DeSerialize(JsonElement jsonElement, SerializationModeKind serializationModeKind, ILoggerFactory loggerFactory = null)
        {
            ...
            {
                logger.LogDebug("the source Json property was not found in the Differencing: { Id }", dtoInstance.Id);
            }
            ...
            {
                logger.LogDebug("the target Json property was not found in the Differencing: { Id }", dtoInstance.Id);
            }
            ...
            if (jsonElement.TryGetProperty("typeDifferenced"u8, out var typeDifferencedProperty))
            {
                if (typeDifferencedProperty.ValueKind == JsonValueKind.Null)
                {
                ...
                {
                    if (typeDifferencedProperty.TryGetProperty("@id"u8, out var typeDifferencedExternalIdProperty))
                    {
                        var propertyValue = typeDifferencedExternalIdProperty.GetString();

                        if (propertyValue != null)
                        {
                            dtoInstance.typeDifferenced = Guid.Parse(propertyValue);
                        }
                    }
                }
            }
