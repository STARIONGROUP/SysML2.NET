...
namespace SysML2.NET.Serializer.Json.Core.DTO
{
    ...
    internal static class RenderingUsageDeSerializer
    {
        ...
        internal static IRenderingUsage DeSerialize(JsonElement jsonElement, SerializationModeKind serializationModeKind, ILoggerFactory loggerFactory = null)
        {
            ...
            {
                logger.LogDebug("the definition Json property was not found in the RenderingUsage: { Id }", dtoInstance.Id);
            }
            ...
            {
                logger.LogDebug("the isVariable Json property was not found in the RenderingUsage: { Id }", dtoInstance.Id);
            }
            ...
            {
                logger.LogDebug("the partDefinition Json property was not found in the RenderingUsage: { Id }", dtoInstance.Id);
            }
            ...
            {
                if (renderingDefinitionProperty.ValueKind == JsonValueKind.Null)
                {
                    dtoInstance.renderingDefinition = null;
                }
                else
                {
                    if (renderingDefinitionProperty.TryGetProperty("@id"u8, out var renderingDefinitionExternalIdProperty))
                    {
                        var propertyValue = renderingDefinitionExternalIdProperty.GetString();

                        if (propertyValue != null)
                        {
                            dtoInstance.renderingDefinition = Guid.Parse(propertyValue);
                        }
                    }
                }
            }
            ...
            {
                logger.LogDebug("the type Json property was not found in the RenderingUsage: { Id }", dtoInstance.Id);
            }
