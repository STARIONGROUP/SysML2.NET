...
namespace SysML2.NET.Serializer.Json.Core.DTO
{
    ...
    internal static class StateDefinitionDeSerializer
    {
        ...
        internal static IStateDefinition DeSerialize(JsonElement jsonElement, SerializationModeKind serializationModeKind, ILoggerFactory loggerFactory = null)
        {
            ...
            if (@type.GetString() != "StateDefinition")
            {
                throw new InvalidOperationException($"The StateDefinitionDeSerializer can only be used to deserialize objects of type IStateDefinition, a {@type.GetString()} was provided");
            }
            ...
            {
                logger.LogDebug("the directedFeature Json property was not found in the StateDefinition: { Id }", dtoInstance.Id);
            }
            ...
            if (jsonElement.TryGetProperty("state"u8, out var stateProperty))
            {
                foreach (var arrayItem in stateProperty.EnumerateArray())
                {
                    if (arrayItem.TryGetProperty("@id"u8, out var stateExternalIdProperty))
                    {
                        var propertyValue = stateExternalIdProperty.GetString();

                        if (propertyValue != null)
                        {
                            dtoInstance.state.Add(Guid.Parse(propertyValue));
                        }
                    }
                }
            }
