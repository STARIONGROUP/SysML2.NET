...
namespace SysML2.NET.Serializer.Json.Core.DTO
{
    ...
    {
        /// <summary>
        /// Deserializes an instance of <see cref="IViewpointUsage"/> from the provided <see cref="JsonElement"/>
        /// </summary>
        ...
        internal static IViewpointUsage DeSerialize(JsonElement jsonElement, SerializationModeKind serializationModeKind, ILoggerFactory loggerFactory = null)
        {
            ...
            {
                throw new InvalidOperationException("The @type property is not available, the ViewpointUsageDeSerializer cannot be used to deserialize this JsonElement");
            }

            if (@type.GetString() != "ViewpointUsage")
            {
            ...
            if (jsonElement.TryGetProperty("viewpointDefinition"u8, out var viewpointDefinitionProperty))
            {
                if (viewpointDefinitionProperty.ValueKind == JsonValueKind.Null)
                {
                    dtoInstance.viewpointDefinition = null;
                }
                else
                {
                    if (viewpointDefinitionProperty.TryGetProperty("@id"u8, out var viewpointDefinitionExternalIdProperty))
                    {
                        var propertyValue = viewpointDefinitionExternalIdProperty.GetString();

                        if (propertyValue != null)
                        {
                            dtoInstance.viewpointDefinition = Guid.Parse(propertyValue);
                        }
                    }
                }
            }
            ...
            if (jsonElement.TryGetProperty("viewpointStakeholder"u8, out var viewpointStakeholderProperty))
            {
                foreach (var arrayItem in viewpointStakeholderProperty.EnumerateArray())
                {
                    if (arrayItem.TryGetProperty("@id"u8, out var viewpointStakeholderExternalIdProperty))
                    {
                        var propertyValue = viewpointStakeholderExternalIdProperty.GetString();

                        if (propertyValue != null)
                        {
                            dtoInstance.viewpointStakeholder.Add(Guid.Parse(propertyValue));
                        }
                    }
                }
            }
