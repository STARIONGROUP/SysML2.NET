...
namespace SysML2.NET.Serializer.Json.Core.DTO
{
    ...
    {
        /// <summary>
        /// Deserializes an instance of <see cref="ITransitionFeatureMembership"/> from the provided <see cref="Utf8JsonReader"/>
        /// </summary>
        /// <param name="reader">
        /// The <see cref="Utf8JsonReader"/> positioned on the <see cref="JsonTokenType.StartObject"/> of the
        /// <see cref="ITransitionFeatureMembership"/> json object. On return the reader is positioned on the matching
        ...
        /// </remarks>
        internal static ITransitionFeatureMembership DeSerialize(ref Utf8JsonReader reader, SerializationModeKind serializationModeKind, bool deserializeDerivedProperties, ILoggerFactory loggerFactory = null)
        {
            ...
            var dtoInstance = new SysML2.NET.Core.DTO.Systems.States.TransitionFeatureMembership();
        ...
        /// <summary>
        /// Deserializes properties of a <see cref="SysML2.NET.Core.DTO.Systems.States.TransitionFeatureMembership" />
        /// from a <see cref="Utf8JsonReader" />, including derived properties
        /// </summary>
        /// <param name="dtoInstance">
        /// The <see cref="SysML2.NET.Core.DTO.Systems.States.TransitionFeatureMembership"/> instance holding deserialized values
        /// </param>
        /// <param name="reader">
        /// The <see cref="Utf8JsonReader"/> positioned on the <see cref="JsonTokenType.StartObject"/> of the
        /// <see cref="ITransitionFeatureMembership"/> json object
        /// </param>
        /// <param name="logger">
        /// The <see cref="ILogger"/> to produce logging statement
        /// </param>
        private static void DeserializeDtoIncludingDerivedProperties(SysML2.NET.Core.DTO.Systems.States.TransitionFeatureMembership dtoInstance, ref Utf8JsonReader reader, ILogger logger)
        {
            ...
            var transitionFeatureSeen = false;
            ...
            while (reader.Read() && reader.TokenType != JsonTokenType.EndObject)
            {
                ...
                {
                    throw new JsonException("Expected a property name in the TransitionFeatureMembership json object.");
                }
                ...
                if (reader.ValueTextEquals("kind"u8))
                {
                    ...
                    dtoInstance.Kind = TransitionFeatureKindDeSerializer.Deserialize(reader.GetString());
                ...
                if (reader.ValueTextEquals("owningType"u8))
                {
                    ...
                    if (reader.TokenType == JsonTokenType.Null)
                    {
                        ...
                        {
                            logger.LogDebug("the TransitionFeatureMembership.owningType property was not found in the Json. The value is set to Guid.Empty");
                        }
                    }
                ...
                if (reader.ValueTextEquals("transitionFeature"u8))
                {
                    transitionFeatureSeen = true;
                    reader.Read();

                    if (reader.TokenType == JsonTokenType.Null)
                    {
                        dtoInstance.transitionFeature = Guid.Empty;

                        if (logger.IsEnabled(LogLevel.Debug))
                        {
                            logger.LogDebug("the TransitionFeatureMembership.transitionFeature property was not found in the Json. The value is set to Guid.Empty");
                        }
                    }
                    else if (Utf8JsonReaderHelper.TryReadReferenceIdentifier(ref reader, out var transitionFeatureValue))
                    {
                        dtoInstance.transitionFeature = transitionFeatureValue;
                    }
            ...
            if (logger.IsEnabled(LogLevel.Debug))
            {
                ...
                {
                    logger.LogDebug("the documentation Json property was not found in the TransitionFeatureMembership: {Id}", dtoInstance.Id);
                }
                if (!elementIdSeen)
                {
                ...
                {
                    logger.LogDebug("the isLibraryElement Json property was not found in the TransitionFeatureMembership: {Id}", dtoInstance.Id);
                }
                if (!kindSeen)
                {
                ...
                if (!nameSeen)
                {
                    logger.LogDebug("the name Json property was not found in the TransitionFeatureMembership: {Id}", dtoInstance.Id);
                }
                if (!ownedAnnotationSeen)
                {
                    logger.LogDebug("the ownedAnnotation Json property was not found in the TransitionFeatureMembership: {Id}", dtoInstance.Id);
                }
                if (!ownedElementSeen)
                {
                    logger.LogDebug("the ownedElement Json property was not found in the TransitionFeatureMembership: {Id}", dtoInstance.Id);
                }
                if (!ownedMemberElementIdSeen)
                {
                    logger.LogDebug("the ownedMemberElementId Json property was not found in the TransitionFeatureMembership: {Id}", dtoInstance.Id);
                }
                if (!ownedMemberNameSeen)
                {
                    logger.LogDebug("the ownedMemberName Json property was not found in the TransitionFeatureMembership: {Id}", dtoInstance.Id);
                }
                if (!ownedMemberShortNameSeen)
                {
                    logger.LogDebug("the ownedMemberShortName Json property was not found in the TransitionFeatureMembership: {Id}", dtoInstance.Id);
                }
                ...
                if (!ownerSeen)
                {
                    logger.LogDebug("the owner Json property was not found in the TransitionFeatureMembership: {Id}", dtoInstance.Id);
                }
                if (!owningMembershipSeen)
                {
                    logger.LogDebug("the owningMembership Json property was not found in the TransitionFeatureMembership: {Id}", dtoInstance.Id);
                }
                if (!owningNamespaceSeen)
                {
                    logger.LogDebug("the owningNamespace Json property was not found in the TransitionFeatureMembership: {Id}", dtoInstance.Id);
                }
                ...
                if (!owningTypeSeen)
                {
                    logger.LogDebug("the owningType Json property was not found in the TransitionFeatureMembership: {Id}", dtoInstance.Id);
                }
                if (!qualifiedNameSeen)
                {
                    logger.LogDebug("the qualifiedName Json property was not found in the TransitionFeatureMembership: {Id}", dtoInstance.Id);
                }
                if (!relatedElementSeen)
                {
                    logger.LogDebug("the relatedElement Json property was not found in the TransitionFeatureMembership: {Id}", dtoInstance.Id);
                }
                if (!shortNameSeen)
                {
                    logger.LogDebug("the shortName Json property was not found in the TransitionFeatureMembership: {Id}", dtoInstance.Id);
                }
                if (!textualRepresentationSeen)
                {
                    logger.LogDebug("the textualRepresentation Json property was not found in the TransitionFeatureMembership: {Id}", dtoInstance.Id);
                }
                if (!transitionFeatureSeen)
                {
                    logger.LogDebug("the transitionFeature Json property was not found in the TransitionFeatureMembership: {Id}", dtoInstance.Id);
                }
                if (!visibilitySeen)
                {
        ...
        /// The <see cref="ILogger"/> to produce logging statement
        /// </param>
        private static void DeserializeDtoExcludingDerivedProperties(SysML2.NET.Core.DTO.Systems.States.TransitionFeatureMembership dtoInstance, ref Utf8JsonReader reader, ILogger logger)
        {
            var aliasIdsSeen = false;
            var declaredNameSeen = false;
            var declaredShortNameSeen = false;
