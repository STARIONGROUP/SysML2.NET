...
namespace SysML2.NET.Serializer.Json.Core.DTO
{
    ...
    internal static class TypeFeaturingDeSerializer
    {
        ...
        internal static ITypeFeaturing DeSerialize(JsonElement jsonElement, SerializationModeKind serializationModeKind, ILoggerFactory loggerFactory = null)
        {
            ...
            if (jsonElement.TryGetProperty("owningFeatureOfType"u8, out var owningFeatureOfTypeProperty))
            {
                if (owningFeatureOfTypeProperty.ValueKind == JsonValueKind.Null)
                {
                    dtoInstance.owningFeatureOfType = null;
                }
                else
                {
                    if (owningFeatureOfTypeProperty.TryGetProperty("@id"u8, out var owningFeatureOfTypeExternalIdProperty))
                    {
                        var propertyValue = owningFeatureOfTypeExternalIdProperty.GetString();

                        if (propertyValue != null)
                        {
                            dtoInstance.owningFeatureOfType = Guid.Parse(propertyValue);
                        }
                    }
                }
            }
            ...
            {
                logger.LogDebug("the source Json property was not found in the TypeFeaturing: { Id }", dtoInstance.Id);
            }
            ...
            {
                logger.LogDebug("the target Json property was not found in the TypeFeaturing: { Id }", dtoInstance.Id);
            }
