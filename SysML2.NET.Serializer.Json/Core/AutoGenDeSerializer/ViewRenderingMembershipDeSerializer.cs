// -------------------------------------------------------------------------------------------------
// <copyright file="ViewRenderingMembershipDeSerializer.cs" company="Starion Group S.A.">
//
...
namespace SysML2.NET.Serializer.Json.Core.DTO
{
    ...
    {
        /// <summary>
        /// Deserializes an instance of <see cref="IViewRenderingMembership"/> from the provided <see cref="Utf8JsonReader"/>
        /// </summary>
        /// <param name="reader">
        /// The <see cref="Utf8JsonReader"/> positioned on the <see cref="JsonTokenType.StartObject"/> of the
        /// <see cref="IViewRenderingMembership"/> json object. On return the reader is positioned on the matching
        ...
        /// </remarks>
        internal static IViewRenderingMembership DeSerialize(ref Utf8JsonReader reader, SerializationModeKind serializationModeKind, bool deserializeDerivedProperties, ILoggerFactory loggerFactory = null)
        {
            ...
            var dtoInstance = new SysML2.NET.Core.DTO.Systems.Views.ViewRenderingMembership();
        ...
        /// <summary>
        /// Deserializes properties of a <see cref="SysML2.NET.Core.DTO.Systems.Views.ViewRenderingMembership" />
        /// from a <see cref="Utf8JsonReader" />, including derived properties
        /// </summary>
        /// <param name="dtoInstance">
        /// The <see cref="SysML2.NET.Core.DTO.Systems.Views.ViewRenderingMembership"/> instance holding deserialized values
        /// </param>
        /// <param name="reader">
        /// The <see cref="Utf8JsonReader"/> positioned on the <see cref="JsonTokenType.StartObject"/> of the
        /// <see cref="IViewRenderingMembership"/> json object
        /// </param>
        /// <param name="logger">
        /// The <see cref="ILogger"/> to produce logging statement
        /// </param>
        private static void DeserializeDtoIncludingDerivedProperties(SysML2.NET.Core.DTO.Systems.Views.ViewRenderingMembership dtoInstance, ref Utf8JsonReader reader, ILogger logger)
        {
            ...
            var referencedRenderingSeen = false;
            ...
            while (reader.Read() && reader.TokenType != JsonTokenType.EndObject)
            {
                ...
                {
                    throw new JsonException("Expected a property name in the ViewRenderingMembership json object.");
                }
                ...
                if (reader.ValueTextEquals("ownedRendering"u8))
                {
                    ...
                    if (reader.TokenType == JsonTokenType.Null)
                    {
                        dtoInstance.ownedRendering = Guid.Empty;

                        if (logger.IsEnabled(LogLevel.Debug))
                        {
                            logger.LogDebug("the ViewRenderingMembership.ownedRendering property was not found in the Json. The value is set to Guid.Empty");
                        }
                    }
                    else if (Utf8JsonReaderHelper.TryReadReferenceIdentifier(ref reader, out var ownedRenderingValue))
                    {
                        dtoInstance.ownedRendering = ownedRenderingValue;
                    }
                ...
                if (reader.ValueTextEquals("owningType"u8))
                {
                    ...
                    if (reader.TokenType == JsonTokenType.Null)
                    {
                        ...
                        if (logger.IsEnabled(LogLevel.Debug))
                        {
                            logger.LogDebug("the ViewRenderingMembership.owningType property was not found in the Json. The value is set to Guid.Empty");
                        }
                    }
                ...
                if (reader.ValueTextEquals("referencedRendering"u8))
                {
                    referencedRenderingSeen = true;
                    reader.Read();

                    if (reader.TokenType == JsonTokenType.Null)
                    {
                        dtoInstance.referencedRendering = Guid.Empty;

                        if (logger.IsEnabled(LogLevel.Debug))
                        {
                            logger.LogDebug("the ViewRenderingMembership.referencedRendering property was not found in the Json. The value is set to Guid.Empty");
                        }
                    }
                    else if (Utf8JsonReaderHelper.TryReadReferenceIdentifier(ref reader, out var referencedRenderingValue))
                    {
                        dtoInstance.referencedRendering = referencedRenderingValue;
                    }
            ...
            if (logger.IsEnabled(LogLevel.Debug))
            {
                ...
                {
                    logger.LogDebug("the documentation Json property was not found in the ViewRenderingMembership: {Id}", dtoInstance.Id);
                }
                if (!elementIdSeen)
                {
                ...
                {
                    logger.LogDebug("the isLibraryElement Json property was not found in the ViewRenderingMembership: {Id}", dtoInstance.Id);
                }
                if (!nameSeen)
                {
                    logger.LogDebug("the name Json property was not found in the ViewRenderingMembership: {Id}", dtoInstance.Id);
                }
                if (!ownedAnnotationSeen)
                {
                    logger.LogDebug("the ownedAnnotation Json property was not found in the ViewRenderingMembership: {Id}", dtoInstance.Id);
                }
                if (!ownedElementSeen)
                {
                    logger.LogDebug("the ownedElement Json property was not found in the ViewRenderingMembership: {Id}", dtoInstance.Id);
                }
                if (!ownedMemberElementIdSeen)
                {
                    logger.LogDebug("the ownedMemberElementId Json property was not found in the ViewRenderingMembership: {Id}", dtoInstance.Id);
                }
                if (!ownedMemberNameSeen)
                {
                    logger.LogDebug("the ownedMemberName Json property was not found in the ViewRenderingMembership: {Id}", dtoInstance.Id);
                }
                if (!ownedMemberShortNameSeen)
                {
                    logger.LogDebug("the ownedMemberShortName Json property was not found in the ViewRenderingMembership: {Id}", dtoInstance.Id);
                }
                ...
                {
                    logger.LogDebug("the ownedRendering Json property was not found in the ViewRenderingMembership: {Id}", dtoInstance.Id);
                }
                if (!ownerSeen)
                {
                    logger.LogDebug("the owner Json property was not found in the ViewRenderingMembership: {Id}", dtoInstance.Id);
                }
                if (!owningMembershipSeen)
                {
                    logger.LogDebug("the owningMembership Json property was not found in the ViewRenderingMembership: {Id}", dtoInstance.Id);
                }
                if (!owningNamespaceSeen)
                {
                    logger.LogDebug("the owningNamespace Json property was not found in the ViewRenderingMembership: {Id}", dtoInstance.Id);
                }
                ...
                if (!owningTypeSeen)
                {
                    logger.LogDebug("the owningType Json property was not found in the ViewRenderingMembership: {Id}", dtoInstance.Id);
                }
                if (!qualifiedNameSeen)
                {
                    logger.LogDebug("the qualifiedName Json property was not found in the ViewRenderingMembership: {Id}", dtoInstance.Id);
                }
                if (!referencedRenderingSeen)
                {
                    logger.LogDebug("the referencedRendering Json property was not found in the ViewRenderingMembership: {Id}", dtoInstance.Id);
                }
                if (!relatedElementSeen)
                {
                    logger.LogDebug("the relatedElement Json property was not found in the ViewRenderingMembership: {Id}", dtoInstance.Id);
                }
                if (!shortNameSeen)
                {
                    logger.LogDebug("the shortName Json property was not found in the ViewRenderingMembership: {Id}", dtoInstance.Id);
                }
                if (!textualRepresentationSeen)
                {
                    logger.LogDebug("the textualRepresentation Json property was not found in the ViewRenderingMembership: {Id}", dtoInstance.Id);
                }
                if (!visibilitySeen)
                {
        ...
        /// The <see cref="ILogger"/> to produce logging statement
        /// </param>
        private static void DeserializeDtoExcludingDerivedProperties(SysML2.NET.Core.DTO.Systems.Views.ViewRenderingMembership dtoInstance, ref Utf8JsonReader reader, ILogger logger)
        {
            var aliasIdsSeen = false;
            var declaredNameSeen = false;
            var declaredShortNameSeen = false;
