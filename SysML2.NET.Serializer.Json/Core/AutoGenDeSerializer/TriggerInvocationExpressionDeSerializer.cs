...
namespace SysML2.NET.Serializer.Json.Core.DTO
{
    ...
    {
        /// <summary>
        /// Deserializes an instance of <see cref="ITriggerInvocationExpression"/> from the provided <see cref="JsonElement"/>
        /// </summary>
        ...
        internal static ITriggerInvocationExpression DeSerialize(JsonElement jsonElement, SerializationModeKind serializationModeKind, ILoggerFactory loggerFactory = null)
        {
            ...
            {
                throw new InvalidOperationException("The @type property is not available, the TriggerInvocationExpressionDeSerializer cannot be used to deserialize this JsonElement");
            }

            if (@type.GetString() != "TriggerInvocationExpression")
            {
                throw new InvalidOperationException($"The TriggerInvocationExpressionDeSerializer can only be used to deserialize objects of type ITriggerInvocationExpression, a {@type.GetString()} was provided");
            }
            ...
            {
                foreach (var arrayItem in argumentProperty.EnumerateArray())
                {
                    if (arrayItem.TryGetProperty("@id"u8, out var argumentExternalIdProperty))
                    {
                        var propertyValue = argumentExternalIdProperty.GetString();

                        if (propertyValue != null)
                        {
                            dtoInstance.argument.Add(Guid.Parse(propertyValue));
                        }
                    }
                }
            }
            ...
            {
                logger.LogDebug("the behavior Json property was not found in the TriggerInvocationExpression: { Id }", dtoInstance.Id);
            }
            ...
            {
                logger.LogDebug("the directedFeature Json property was not found in the TriggerInvocationExpression: { Id }", dtoInstance.Id);
            }
            ...
            {
                if (instantiatedTypeProperty.ValueKind == JsonValueKind.Null)
                {
                ...
                {
                    if (instantiatedTypeProperty.TryGetProperty("@id"u8, out var instantiatedTypeExternalIdProperty))
                    {
                        var propertyValue = instantiatedTypeExternalIdProperty.GetString();
