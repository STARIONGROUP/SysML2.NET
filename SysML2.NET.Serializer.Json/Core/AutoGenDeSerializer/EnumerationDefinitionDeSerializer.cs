...
namespace SysML2.NET.Serializer.Json.Core.DTO
{
    ...
    internal static class EnumerationDefinitionDeSerializer
    {
        ...
        internal static IEnumerationDefinition DeSerialize(JsonElement jsonElement, SerializationModeKind serializationModeKind, ILoggerFactory loggerFactory = null)
        {
            ...
            {
                foreach (var arrayItem in enumeratedValueProperty.EnumerateArray())
                {
                    if (arrayItem.TryGetProperty("@id"u8, out var enumeratedValueExternalIdProperty))
                    {
                        var propertyValue = enumeratedValueExternalIdProperty.GetString();

                        if (propertyValue != null)
                        {
                            dtoInstance.enumeratedValue.Add(Guid.Parse(propertyValue));
                        }
                    }
                }
            }
