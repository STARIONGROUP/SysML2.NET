...
namespace SysML2.NET.Serializer.Json.Core.DTO
{
    ...
    /// </summary>
    internal static class BooleanExpressionDeSerializer
    {
        /// <summary>
        /// Deserializes an instance of <see cref="IBooleanExpression"/> from the provided <see cref="Utf8JsonReader"/>
        /// </summary>
        /// <param name="reader">
        /// The <see cref="Utf8JsonReader"/> positioned on the <see cref="JsonTokenType.StartObject"/> of the
        /// <see cref="IBooleanExpression"/> json object. On return the reader is positioned on the matching
        ...
        /// </remarks>
        internal static IBooleanExpression DeSerialize(ref Utf8JsonReader reader, SerializationModeKind serializationModeKind, bool deserializeDerivedProperties, ILoggerFactory loggerFactory = null)
        {
        ...
        /// <summary>
        /// Deserializes properties of a <see cref="SysML2.NET.Core.DTO.Kernel.Functions.BooleanExpression" />
        /// from a <see cref="Utf8JsonReader" />, including derived properties
        /// </summary>
        /// <param name="dtoInstance">
        /// The <see cref="SysML2.NET.Core.DTO.Kernel.Functions.BooleanExpression"/> instance holding deserialized values
        /// </param>
        /// <param name="reader">
        /// The <see cref="Utf8JsonReader"/> positioned on the <see cref="JsonTokenType.StartObject"/> of the
        /// <see cref="IBooleanExpression"/> json object
        /// </param>
        /// <param name="logger">
        /// The <see cref="ILogger"/> to produce logging statement
        /// </param>
        private static void DeserializeDtoIncludingDerivedProperties(SysML2.NET.Core.DTO.Kernel.Functions.BooleanExpression dtoInstance, ref Utf8JsonReader reader, ILogger logger)
        {
            ...
            while (reader.Read() && reader.TokenType != JsonTokenType.EndObject)
            {
                ...
                {
                    throw new JsonException("Expected a property name in the BooleanExpression json object.");
                }
                ...
                if (reader.ValueTextEquals("featureTarget"u8))
                {
                    ...
                    if (reader.TokenType == JsonTokenType.Null)
                    {
                        ...
                        {
                            logger.LogDebug("the BooleanExpression.featureTarget property was not found in the Json. The value is set to Guid.Empty");
                        }
                    }
                ...
                if (reader.ValueTextEquals("predicate"u8))
                {
                    predicateSeen = true;
                    reader.Read();
                    ...
                    else if (Utf8JsonReaderHelper.TryReadReferenceIdentifier(ref reader, out var predicateValue))
                    {
                        dtoInstance.predicate = predicateValue;
                    }
                ...
                if (reader.ValueTextEquals("result"u8))
                {
                    ...
                    if (reader.TokenType == JsonTokenType.Null)
                    {
                        ...
                        if (logger.IsEnabled(LogLevel.Debug))
                        {
                            logger.LogDebug("the BooleanExpression.result property was not found in the Json. The value is set to Guid.Empty");
                        }
                    }
            ...
            if (logger.IsEnabled(LogLevel.Debug))
            {
                ...
                {
                    logger.LogDebug("the chainingFeature Json property was not found in the BooleanExpression: {Id}", dtoInstance.Id);
                }
                if (!crossFeatureSeen)
                {
                    logger.LogDebug("the crossFeature Json property was not found in the BooleanExpression: {Id}", dtoInstance.Id);
                }
                if (!declaredNameSeen)
                {
                ...
                {
                    logger.LogDebug("the differencingType Json property was not found in the BooleanExpression: {Id}", dtoInstance.Id);
                }
                if (!directionSeen)
                {
                ...
                {
                    logger.LogDebug("the documentation Json property was not found in the BooleanExpression: {Id}", dtoInstance.Id);
                }
                if (!elementIdSeen)
                {
                ...
                if (!endFeatureSeen)
                {
                    logger.LogDebug("the endFeature Json property was not found in the BooleanExpression: {Id}", dtoInstance.Id);
                }
                if (!endOwningTypeSeen)
                {
                    logger.LogDebug("the endOwningType Json property was not found in the BooleanExpression: {Id}", dtoInstance.Id);
                }
                if (!featureSeen)
                {
                    logger.LogDebug("the feature Json property was not found in the BooleanExpression: {Id}", dtoInstance.Id);
                }
                if (!featureMembershipSeen)
                {
                    logger.LogDebug("the featureMembership Json property was not found in the BooleanExpression: {Id}", dtoInstance.Id);
                }
                if (!featureTargetSeen)
                {
                    logger.LogDebug("the featureTarget Json property was not found in the BooleanExpression: {Id}", dtoInstance.Id);
                }
                if (!featuringTypeSeen)
                {
                    logger.LogDebug("the featuringType Json property was not found in the BooleanExpression: {Id}", dtoInstance.Id);
                }
                if (!importedMembershipSeen)
                {
                    logger.LogDebug("the importedMembership Json property was not found in the BooleanExpression: {Id}", dtoInstance.Id);
                }
                if (!inheritedFeatureSeen)
                {
                    logger.LogDebug("the inheritedFeature Json property was not found in the BooleanExpression: {Id}", dtoInstance.Id);
                }
                if (!inheritedMembershipSeen)
                {
                    logger.LogDebug("the inheritedMembership Json property was not found in the BooleanExpression: {Id}", dtoInstance.Id);
                }
                if (!inputSeen)
                {
                    logger.LogDebug("the input Json property was not found in the BooleanExpression: {Id}", dtoInstance.Id);
                }
                if (!intersectingTypeSeen)
                {
                    logger.LogDebug("the intersectingType Json property was not found in the BooleanExpression: {Id}", dtoInstance.Id);
                }
                if (!isAbstractSeen)
                {
                ...
                if (!isConjugatedSeen)
                {
                    logger.LogDebug("the isConjugated Json property was not found in the BooleanExpression: {Id}", dtoInstance.Id);
                }
                if (!isConstantSeen)
                {
                ...
                {
                    logger.LogDebug("the isLibraryElement Json property was not found in the BooleanExpression: {Id}", dtoInstance.Id);
                }
                if (!isModelLevelEvaluableSeen)
                {
                    logger.LogDebug("the isModelLevelEvaluable Json property was not found in the BooleanExpression: {Id}", dtoInstance.Id);
                }
                if (!isOrderedSeen)
                {
                ...
                if (!memberSeen)
                {
                    logger.LogDebug("the member Json property was not found in the BooleanExpression: {Id}", dtoInstance.Id);
                }
                if (!membershipSeen)
                {
                    logger.LogDebug("the membership Json property was not found in the BooleanExpression: {Id}", dtoInstance.Id);
                }
                if (!multiplicitySeen)
                {
                    logger.LogDebug("the multiplicity Json property was not found in the BooleanExpression: {Id}", dtoInstance.Id);
                }
                if (!nameSeen)
                {
                    logger.LogDebug("the name Json property was not found in the BooleanExpression: {Id}", dtoInstance.Id);
                }
                if (!outputSeen)
                {
                    logger.LogDebug("the output Json property was not found in the BooleanExpression: {Id}", dtoInstance.Id);
                }
                if (!ownedAnnotationSeen)
                {
                    logger.LogDebug("the ownedAnnotation Json property was not found in the BooleanExpression: {Id}", dtoInstance.Id);
                }
                if (!ownedConjugatorSeen)
                {
                    logger.LogDebug("the ownedConjugator Json property was not found in the BooleanExpression: {Id}", dtoInstance.Id);
                }
                if (!ownedCrossSubsettingSeen)
                {
                    logger.LogDebug("the ownedCrossSubsetting Json property was not found in the BooleanExpression: {Id}", dtoInstance.Id);
                }
                if (!ownedDifferencingSeen)
                {
                    logger.LogDebug("the ownedDifferencing Json property was not found in the BooleanExpression: {Id}", dtoInstance.Id);
                }
                if (!ownedDisjoiningSeen)
                {
                    logger.LogDebug("the ownedDisjoining Json property was not found in the BooleanExpression: {Id}", dtoInstance.Id);
                }
                if (!ownedElementSeen)
                {
                    logger.LogDebug("the ownedElement Json property was not found in the BooleanExpression: {Id}", dtoInstance.Id);
                }
                if (!ownedEndFeatureSeen)
                {
                    logger.LogDebug("the ownedEndFeature Json property was not found in the BooleanExpression: {Id}", dtoInstance.Id);
                }
                if (!ownedFeatureSeen)
                {
                    logger.LogDebug("the ownedFeature Json property was not found in the BooleanExpression: {Id}", dtoInstance.Id);
                }
                if (!ownedFeatureChainingSeen)
                {
                    logger.LogDebug("the ownedFeatureChaining Json property was not found in the BooleanExpression: {Id}", dtoInstance.Id);
                }
                if (!ownedFeatureInvertingSeen)
                {
                    logger.LogDebug("the ownedFeatureInverting Json property was not found in the BooleanExpression: {Id}", dtoInstance.Id);
                }
                if (!ownedFeatureMembershipSeen)
                {
                    logger.LogDebug("the ownedFeatureMembership Json property was not found in the BooleanExpression: {Id}", dtoInstance.Id);
                }
                if (!ownedImportSeen)
                {
                    logger.LogDebug("the ownedImport Json property was not found in the BooleanExpression: {Id}", dtoInstance.Id);
                }
                if (!ownedIntersectingSeen)
                {
                    logger.LogDebug("the ownedIntersecting Json property was not found in the BooleanExpression: {Id}", dtoInstance.Id);
                }
                if (!ownedMemberSeen)
                {
                    logger.LogDebug("the ownedMember Json property was not found in the BooleanExpression: {Id}", dtoInstance.Id);
                }
                if (!ownedMembershipSeen)
                {
                    logger.LogDebug("the ownedMembership Json property was not found in the BooleanExpression: {Id}", dtoInstance.Id);
                }
                if (!ownedRedefinitionSeen)
                {
                    logger.LogDebug("the ownedRedefinition Json property was not found in the BooleanExpression: {Id}", dtoInstance.Id);
                }
                if (!ownedReferenceSubsettingSeen)
                {
                    logger.LogDebug("the ownedReferenceSubsetting Json property was not found in the BooleanExpression: {Id}", dtoInstance.Id);
                }
                ...
                {
                    logger.LogDebug("the ownedSpecialization Json property was not found in the BooleanExpression: {Id}", dtoInstance.Id);
                }
                if (!ownedSubsettingSeen)
                {
                    logger.LogDebug("the ownedSubsetting Json property was not found in the BooleanExpression: {Id}", dtoInstance.Id);
                }
                if (!ownedTypeFeaturingSeen)
                {
                    logger.LogDebug("the ownedTypeFeaturing Json property was not found in the BooleanExpression: {Id}", dtoInstance.Id);
                }
                if (!ownedTypingSeen)
                {
                    logger.LogDebug("the ownedTyping Json property was not found in the BooleanExpression: {Id}", dtoInstance.Id);
                }
                if (!ownedUnioningSeen)
                {
                    logger.LogDebug("the ownedUnioning Json property was not found in the BooleanExpression: {Id}", dtoInstance.Id);
                }
                if (!ownerSeen)
                {
                    logger.LogDebug("the owner Json property was not found in the BooleanExpression: {Id}", dtoInstance.Id);
                }
                if (!owningFeatureMembershipSeen)
                {
                    logger.LogDebug("the owningFeatureMembership Json property was not found in the BooleanExpression: {Id}", dtoInstance.Id);
                }
                if (!owningMembershipSeen)
                {
                    logger.LogDebug("the owningMembership Json property was not found in the BooleanExpression: {Id}", dtoInstance.Id);
                }
                if (!owningNamespaceSeen)
                {
                    logger.LogDebug("the owningNamespace Json property was not found in the BooleanExpression: {Id}", dtoInstance.Id);
                }
                ...
                if (!owningTypeSeen)
                {
                    logger.LogDebug("the owningType Json property was not found in the BooleanExpression: {Id}", dtoInstance.Id);
                }
                if (!parameterSeen)
                {
                    logger.LogDebug("the parameter Json property was not found in the BooleanExpression: {Id}", dtoInstance.Id);
                }
                if (!predicateSeen)
                {
                    logger.LogDebug("the predicate Json property was not found in the BooleanExpression: {Id}", dtoInstance.Id);
                }
                if (!qualifiedNameSeen)
                {
                    logger.LogDebug("the qualifiedName Json property was not found in the BooleanExpression: {Id}", dtoInstance.Id);
                }
                if (!resultSeen)
                {
                    logger.LogDebug("the result Json property was not found in the BooleanExpression: {Id}", dtoInstance.Id);
                }
                if (!shortNameSeen)
                {
                    logger.LogDebug("the shortName Json property was not found in the BooleanExpression: {Id}", dtoInstance.Id);
                }
                if (!textualRepresentationSeen)
                {
                    logger.LogDebug("the textualRepresentation Json property was not found in the BooleanExpression: {Id}", dtoInstance.Id);
                }
                if (!typeSeen)
                {
                    logger.LogDebug("the type Json property was not found in the BooleanExpression: {Id}", dtoInstance.Id);
                }
                if (!unioningTypeSeen)
                {
        ...
        /// The <see cref="ILogger"/> to produce logging statement
        /// </param>
        private static void DeserializeDtoExcludingDerivedProperties(SysML2.NET.Core.DTO.Kernel.Functions.BooleanExpression dtoInstance, ref Utf8JsonReader reader, ILogger logger)
        {
            var aliasIdsSeen = false;
            var declaredNameSeen = false;
            var declaredShortNameSeen = false;
