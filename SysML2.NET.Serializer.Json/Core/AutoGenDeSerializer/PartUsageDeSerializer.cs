// -------------------------------------------------------------------------------------------------
// <copyright file="PartUsageDeSerializer.cs" company="Starion Group S.A.">
//
...
namespace SysML2.NET.Serializer.Json.Core.DTO
{
    ...
    /// <summary>
    /// The purpose of the <see cref="PartUsageDeSerializer"/> is to provide deserialization capabilities
    /// for the <see cref="IPartUsage"/> interface
    /// </summary>
    internal static class PartUsageDeSerializer
    {
        /// <summary>
        /// Deserializes an instance of <see cref="IPartUsage"/> from the provided <see cref="Utf8JsonReader"/>
        /// </summary>
        /// <param name="reader">
        /// The <see cref="Utf8JsonReader"/> positioned on the <see cref="JsonTokenType.StartObject"/> of the
        /// <see cref="IPartUsage"/> json object. On return the reader is positioned on the matching
        ...
        /// </remarks>
        internal static IPartUsage DeSerialize(ref Utf8JsonReader reader, SerializationModeKind serializationModeKind, bool deserializeDerivedProperties, ILoggerFactory loggerFactory = null)
        {
            ...
            var dtoInstance = new SysML2.NET.Core.DTO.Systems.Parts.PartUsage();
        ...
        /// <summary>
        /// Deserializes properties of a <see cref="SysML2.NET.Core.DTO.Systems.Parts.PartUsage" />
        /// from a <see cref="Utf8JsonReader" />, including derived properties
        /// </summary>
        /// <param name="dtoInstance">
        /// The <see cref="SysML2.NET.Core.DTO.Systems.Parts.PartUsage"/> instance holding deserialized values
        /// </param>
        /// <param name="reader">
        /// The <see cref="Utf8JsonReader"/> positioned on the <see cref="JsonTokenType.StartObject"/> of the
        /// <see cref="IPartUsage"/> json object
        /// </param>
        /// <param name="logger">
        /// The <see cref="ILogger"/> to produce logging statement
        /// </param>
        private static void DeserializeDtoIncludingDerivedProperties(SysML2.NET.Core.DTO.Systems.Parts.PartUsage dtoInstance, ref Utf8JsonReader reader, ILogger logger)
        {
            ...
            while (reader.Read() && reader.TokenType != JsonTokenType.EndObject)
            {
                ...
                {
                    throw new JsonException("Expected a property name in the PartUsage json object.");
                }
                ...
                if (reader.ValueTextEquals("@id"u8))
                {
                    ...
                    {
                        throw new JsonException("The @id property is not present, the PartUsage cannot be deserialized");
                    }
                ...
                if (reader.ValueTextEquals("featureTarget"u8))
                {
                    ...
                    if (reader.TokenType == JsonTokenType.Null)
                    {
                        ...
                        {
                            logger.LogDebug("the PartUsage.featureTarget property was not found in the Json. The value is set to Guid.Empty");
                        }
                    }
            ...
            if (logger.IsEnabled(LogLevel.Debug))
            {
                ...
                {
                    logger.LogDebug("the chainingFeature Json property was not found in the PartUsage: {Id}", dtoInstance.Id);
                }
                if (!crossFeatureSeen)
                {
                    logger.LogDebug("the crossFeature Json property was not found in the PartUsage: {Id}", dtoInstance.Id);
                }
                if (!declaredNameSeen)
                {
                ...
                {
                    logger.LogDebug("the differencingType Json property was not found in the PartUsage: {Id}", dtoInstance.Id);
                }
                if (!directedFeatureSeen)
                {
                    logger.LogDebug("the directedFeature Json property was not found in the PartUsage: {Id}", dtoInstance.Id);
                }
                if (!directedUsageSeen)
                {
                    logger.LogDebug("the directedUsage Json property was not found in the PartUsage: {Id}", dtoInstance.Id);
                }
                if (!directionSeen)
                {
                ...
                {
                    logger.LogDebug("the documentation Json property was not found in the PartUsage: {Id}", dtoInstance.Id);
                }
                if (!elementIdSeen)
                {
                ...
                if (!endFeatureSeen)
                {
                    logger.LogDebug("the endFeature Json property was not found in the PartUsage: {Id}", dtoInstance.Id);
                }
                if (!endOwningTypeSeen)
                {
                    logger.LogDebug("the endOwningType Json property was not found in the PartUsage: {Id}", dtoInstance.Id);
                }
                if (!featureSeen)
                {
                    logger.LogDebug("the feature Json property was not found in the PartUsage: {Id}", dtoInstance.Id);
                }
                if (!featureMembershipSeen)
                {
                    logger.LogDebug("the featureMembership Json property was not found in the PartUsage: {Id}", dtoInstance.Id);
                }
                if (!featureTargetSeen)
                {
                    logger.LogDebug("the featureTarget Json property was not found in the PartUsage: {Id}", dtoInstance.Id);
                }
                if (!featuringTypeSeen)
                {
                    logger.LogDebug("the featuringType Json property was not found in the PartUsage: {Id}", dtoInstance.Id);
                }
                if (!importedMembershipSeen)
                {
                    logger.LogDebug("the importedMembership Json property was not found in the PartUsage: {Id}", dtoInstance.Id);
                }
                if (!individualDefinitionSeen)
                {
                    logger.LogDebug("the individualDefinition Json property was not found in the PartUsage: {Id}", dtoInstance.Id);
                }
                if (!inheritedFeatureSeen)
                {
                    logger.LogDebug("the inheritedFeature Json property was not found in the PartUsage: {Id}", dtoInstance.Id);
                }
                if (!inheritedMembershipSeen)
                {
                    logger.LogDebug("the inheritedMembership Json property was not found in the PartUsage: {Id}", dtoInstance.Id);
                }
                if (!inputSeen)
                {
                    logger.LogDebug("the input Json property was not found in the PartUsage: {Id}", dtoInstance.Id);
                }
                if (!intersectingTypeSeen)
                {
                    logger.LogDebug("the intersectingType Json property was not found in the PartUsage: {Id}", dtoInstance.Id);
                }
                if (!isAbstractSeen)
                {
                ...
                if (!isConjugatedSeen)
                {
                    logger.LogDebug("the isConjugated Json property was not found in the PartUsage: {Id}", dtoInstance.Id);
                }
                if (!isConstantSeen)
                {
                ...
                {
                    logger.LogDebug("the isLibraryElement Json property was not found in the PartUsage: {Id}", dtoInstance.Id);
                }
                if (!isOrderedSeen)
                {
                ...
                if (!isReferenceSeen)
                {
                    logger.LogDebug("the isReference Json property was not found in the PartUsage: {Id}", dtoInstance.Id);
                }
                if (!isSufficientSeen)
                {
                ...
                {
                    logger.LogDebug("the itemDefinition Json property was not found in the PartUsage: {Id}", dtoInstance.Id);
                }
                if (!mayTimeVarySeen)
                {
                    logger.LogDebug("the mayTimeVary Json property was not found in the PartUsage: {Id}", dtoInstance.Id);
                }
                if (!memberSeen)
                {
                    logger.LogDebug("the member Json property was not found in the PartUsage: {Id}", dtoInstance.Id);
                }
                if (!membershipSeen)
                {
                    logger.LogDebug("the membership Json property was not found in the PartUsage: {Id}", dtoInstance.Id);
                }
                if (!multiplicitySeen)
                {
                    logger.LogDebug("the multiplicity Json property was not found in the PartUsage: {Id}", dtoInstance.Id);
                }
                if (!nameSeen)
                {
                    logger.LogDebug("the name Json property was not found in the PartUsage: {Id}", dtoInstance.Id);
                }
                if (!nestedActionSeen)
                {
                    logger.LogDebug("the nestedAction Json property was not found in the PartUsage: {Id}", dtoInstance.Id);
                }
                if (!nestedAllocationSeen)
                {
                    logger.LogDebug("the nestedAllocation Json property was not found in the PartUsage: {Id}", dtoInstance.Id);
                }
                if (!nestedAnalysisCaseSeen)
                {
                    logger.LogDebug("the nestedAnalysisCase Json property was not found in the PartUsage: {Id}", dtoInstance.Id);
                }
                if (!nestedAttributeSeen)
                {
                    logger.LogDebug("the nestedAttribute Json property was not found in the PartUsage: {Id}", dtoInstance.Id);
                }
                if (!nestedCalculationSeen)
                {
                    logger.LogDebug("the nestedCalculation Json property was not found in the PartUsage: {Id}", dtoInstance.Id);
                }
                if (!nestedCaseSeen)
                {
                    logger.LogDebug("the nestedCase Json property was not found in the PartUsage: {Id}", dtoInstance.Id);
                }
                if (!nestedConcernSeen)
                {
                    logger.LogDebug("the nestedConcern Json property was not found in the PartUsage: {Id}", dtoInstance.Id);
                }
                if (!nestedConnectionSeen)
                {
                    logger.LogDebug("the nestedConnection Json property was not found in the PartUsage: {Id}", dtoInstance.Id);
                }
                if (!nestedConstraintSeen)
                {
                    logger.LogDebug("the nestedConstraint Json property was not found in the PartUsage: {Id}", dtoInstance.Id);
                }
                if (!nestedEnumerationSeen)
                {
                    logger.LogDebug("the nestedEnumeration Json property was not found in the PartUsage: {Id}", dtoInstance.Id);
                }
                if (!nestedFlowSeen)
                {
                    logger.LogDebug("the nestedFlow Json property was not found in the PartUsage: {Id}", dtoInstance.Id);
                }
                if (!nestedInterfaceSeen)
                {
                    logger.LogDebug("the nestedInterface Json property was not found in the PartUsage: {Id}", dtoInstance.Id);
                }
                if (!nestedItemSeen)
                {
                    logger.LogDebug("the nestedItem Json property was not found in the PartUsage: {Id}", dtoInstance.Id);
                }
                if (!nestedMetadataSeen)
                {
                    logger.LogDebug("the nestedMetadata Json property was not found in the PartUsage: {Id}", dtoInstance.Id);
                }
                if (!nestedOccurrenceSeen)
                {
                    logger.LogDebug("the nestedOccurrence Json property was not found in the PartUsage: {Id}", dtoInstance.Id);
                }
                if (!nestedPartSeen)
                {
                    logger.LogDebug("the nestedPart Json property was not found in the PartUsage: {Id}", dtoInstance.Id);
                }
                if (!nestedPortSeen)
                {
                    logger.LogDebug("the nestedPort Json property was not found in the PartUsage: {Id}", dtoInstance.Id);
                }
                if (!nestedReferenceSeen)
                {
                    logger.LogDebug("the nestedReference Json property was not found in the PartUsage: {Id}", dtoInstance.Id);
                }
                if (!nestedRenderingSeen)
                {
                    logger.LogDebug("the nestedRendering Json property was not found in the PartUsage: {Id}", dtoInstance.Id);
                }
                if (!nestedRequirementSeen)
                {
                    logger.LogDebug("the nestedRequirement Json property was not found in the PartUsage: {Id}", dtoInstance.Id);
                }
                if (!nestedStateSeen)
                {
                    logger.LogDebug("the nestedState Json property was not found in the PartUsage: {Id}", dtoInstance.Id);
                }
                if (!nestedTransitionSeen)
                {
                    logger.LogDebug("the nestedTransition Json property was not found in the PartUsage: {Id}", dtoInstance.Id);
                }
                if (!nestedUsageSeen)
                {
                    logger.LogDebug("the nestedUsage Json property was not found in the PartUsage: {Id}", dtoInstance.Id);
                }
                if (!nestedUseCaseSeen)
                {
                    logger.LogDebug("the nestedUseCase Json property was not found in the PartUsage: {Id}", dtoInstance.Id);
                }
                if (!nestedVerificationCaseSeen)
                {
                    logger.LogDebug("the nestedVerificationCase Json property was not found in the PartUsage: {Id}", dtoInstance.Id);
                }
                if (!nestedViewSeen)
                {
                    logger.LogDebug("the nestedView Json property was not found in the PartUsage: {Id}", dtoInstance.Id);
                }
                if (!nestedViewpointSeen)
                {
                    logger.LogDebug("the nestedViewpoint Json property was not found in the PartUsage: {Id}", dtoInstance.Id);
                }
                if (!occurrenceDefinitionSeen)
                {
                    logger.LogDebug("the occurrenceDefinition Json property was not found in the PartUsage: {Id}", dtoInstance.Id);
                }
                if (!outputSeen)
                {
                    logger.LogDebug("the output Json property was not found in the PartUsage: {Id}", dtoInstance.Id);
                }
                if (!ownedAnnotationSeen)
                {
                    logger.LogDebug("the ownedAnnotation Json property was not found in the PartUsage: {Id}", dtoInstance.Id);
                }
                if (!ownedConjugatorSeen)
                {
                    logger.LogDebug("the ownedConjugator Json property was not found in the PartUsage: {Id}", dtoInstance.Id);
                }
                if (!ownedCrossSubsettingSeen)
                {
                    logger.LogDebug("the ownedCrossSubsetting Json property was not found in the PartUsage: {Id}", dtoInstance.Id);
                }
                if (!ownedDifferencingSeen)
                {
                    logger.LogDebug("the ownedDifferencing Json property was not found in the PartUsage: {Id}", dtoInstance.Id);
                }
                if (!ownedDisjoiningSeen)
                {
                    logger.LogDebug("the ownedDisjoining Json property was not found in the PartUsage: {Id}", dtoInstance.Id);
                }
                if (!ownedElementSeen)
                {
                    logger.LogDebug("the ownedElement Json property was not found in the PartUsage: {Id}", dtoInstance.Id);
                }
                if (!ownedEndFeatureSeen)
                {
                    logger.LogDebug("the ownedEndFeature Json property was not found in the PartUsage: {Id}", dtoInstance.Id);
                }
                if (!ownedFeatureSeen)
                {
                    logger.LogDebug("the ownedFeature Json property was not found in the PartUsage: {Id}", dtoInstance.Id);
                }
                if (!ownedFeatureChainingSeen)
                {
                    logger.LogDebug("the ownedFeatureChaining Json property was not found in the PartUsage: {Id}", dtoInstance.Id);
                }
                if (!ownedFeatureInvertingSeen)
                {
                    logger.LogDebug("the ownedFeatureInverting Json property was not found in the PartUsage: {Id}", dtoInstance.Id);
                }
                if (!ownedFeatureMembershipSeen)
                {
                    logger.LogDebug("the ownedFeatureMembership Json property was not found in the PartUsage: {Id}", dtoInstance.Id);
                }
                if (!ownedImportSeen)
                {
                    logger.LogDebug("the ownedImport Json property was not found in the PartUsage: {Id}", dtoInstance.Id);
                }
                if (!ownedIntersectingSeen)
                {
                    logger.LogDebug("the ownedIntersecting Json property was not found in the PartUsage: {Id}", dtoInstance.Id);
                }
                if (!ownedMemberSeen)
                {
                    logger.LogDebug("the ownedMember Json property was not found in the PartUsage: {Id}", dtoInstance.Id);
                }
                if (!ownedMembershipSeen)
                {
                    logger.LogDebug("the ownedMembership Json property was not found in the PartUsage: {Id}", dtoInstance.Id);
                }
                if (!ownedRedefinitionSeen)
                {
                    logger.LogDebug("the ownedRedefinition Json property was not found in the PartUsage: {Id}", dtoInstance.Id);
                }
                if (!ownedReferenceSubsettingSeen)
                {
                    logger.LogDebug("the ownedReferenceSubsetting Json property was not found in the PartUsage: {Id}", dtoInstance.Id);
                }
                ...
                {
                    logger.LogDebug("the ownedSpecialization Json property was not found in the PartUsage: {Id}", dtoInstance.Id);
                }
                if (!ownedSubsettingSeen)
                {
                    logger.LogDebug("the ownedSubsetting Json property was not found in the PartUsage: {Id}", dtoInstance.Id);
                }
                if (!ownedTypeFeaturingSeen)
                {
                    logger.LogDebug("the ownedTypeFeaturing Json property was not found in the PartUsage: {Id}", dtoInstance.Id);
                }
                if (!ownedTypingSeen)
                {
                    logger.LogDebug("the ownedTyping Json property was not found in the PartUsage: {Id}", dtoInstance.Id);
                }
                if (!ownedUnioningSeen)
                {
                    logger.LogDebug("the ownedUnioning Json property was not found in the PartUsage: {Id}", dtoInstance.Id);
                }
                if (!ownerSeen)
                {
                    logger.LogDebug("the owner Json property was not found in the PartUsage: {Id}", dtoInstance.Id);
                }
                if (!owningDefinitionSeen)
                {
                    logger.LogDebug("the owningDefinition Json property was not found in the PartUsage: {Id}", dtoInstance.Id);
                }
                if (!owningFeatureMembershipSeen)
                {
                    logger.LogDebug("the owningFeatureMembership Json property was not found in the PartUsage: {Id}", dtoInstance.Id);
                }
                if (!owningMembershipSeen)
                {
                    logger.LogDebug("the owningMembership Json property was not found in the PartUsage: {Id}", dtoInstance.Id);
                }
                if (!owningNamespaceSeen)
                {
                    logger.LogDebug("the owningNamespace Json property was not found in the PartUsage: {Id}", dtoInstance.Id);
                }
                ...
                if (!owningTypeSeen)
                {
                    logger.LogDebug("the owningType Json property was not found in the PartUsage: {Id}", dtoInstance.Id);
                }
                if (!owningUsageSeen)
                {
                    logger.LogDebug("the owningUsage Json property was not found in the PartUsage: {Id}", dtoInstance.Id);
                }
                if (!partDefinitionSeen)
                {
                    logger.LogDebug("the partDefinition Json property was not found in the PartUsage: {Id}", dtoInstance.Id);
                }
                if (!portionKindSeen)
                {
                ...
                {
                    logger.LogDebug("the qualifiedName Json property was not found in the PartUsage: {Id}", dtoInstance.Id);
                }
                if (!shortNameSeen)
                {
                    logger.LogDebug("the shortName Json property was not found in the PartUsage: {Id}", dtoInstance.Id);
                }
                if (!textualRepresentationSeen)
                {
                    logger.LogDebug("the textualRepresentation Json property was not found in the PartUsage: {Id}", dtoInstance.Id);
                }
                if (!unioningTypeSeen)
                {
                    logger.LogDebug("the unioningType Json property was not found in the PartUsage: {Id}", dtoInstance.Id);
                }
                if (!usageSeen)
                {
                    logger.LogDebug("the usage Json property was not found in the PartUsage: {Id}", dtoInstance.Id);
                }
                if (!variantSeen)
                {
                    logger.LogDebug("the variant Json property was not found in the PartUsage: {Id}", dtoInstance.Id);
                }
        ...
        /// The <see cref="ILogger"/> to produce logging statement
        /// </param>
        private static void DeserializeDtoExcludingDerivedProperties(SysML2.NET.Core.DTO.Systems.Parts.PartUsage dtoInstance, ref Utf8JsonReader reader, ILogger logger)
        {
            var aliasIdsSeen = false;
            var declaredNameSeen = false;
            var declaredShortNameSeen = false;
