// -------------------------------------------------------------------------------------------------
// <copyright file="ConcernDefinitionDeSerializer.cs" company="Starion Group S.A.">
//
...
namespace SysML2.NET.Serializer.Json.Core.DTO
{
    ...
    /// <summary>
    /// The purpose of the <see cref="ConcernDefinitionDeSerializer"/> is to provide deserialization capabilities
    /// for the <see cref="IConcernDefinition"/> interface
    /// </summary>
    internal static class ConcernDefinitionDeSerializer
    {
        /// <summary>
        /// Deserializes an instance of <see cref="IConcernDefinition"/> from the provided <see cref="Utf8JsonReader"/>
        /// </summary>
        /// <param name="reader">
        /// The <see cref="Utf8JsonReader"/> positioned on the <see cref="JsonTokenType.StartObject"/> of the
        /// <see cref="IConcernDefinition"/> json object. On return the reader is positioned on the matching
        ...
        /// </remarks>
        internal static IConcernDefinition DeSerialize(ref Utf8JsonReader reader, SerializationModeKind serializationModeKind, bool deserializeDerivedProperties, ILoggerFactory loggerFactory = null)
        {
        ...
        /// <summary>
        /// Deserializes properties of a <see cref="SysML2.NET.Core.DTO.Systems.Requirements.ConcernDefinition" />
        /// from a <see cref="Utf8JsonReader" />, including derived properties
        /// </summary>
        /// <param name="dtoInstance">
        /// The <see cref="SysML2.NET.Core.DTO.Systems.Requirements.ConcernDefinition"/> instance holding deserialized values
        /// </param>
        /// <param name="reader">
        /// The <see cref="Utf8JsonReader"/> positioned on the <see cref="JsonTokenType.StartObject"/> of the
        /// <see cref="IConcernDefinition"/> json object
        /// </param>
        /// <param name="logger">
        /// The <see cref="ILogger"/> to produce logging statement
        /// </param>
        private static void DeserializeDtoIncludingDerivedProperties(SysML2.NET.Core.DTO.Systems.Requirements.ConcernDefinition dtoInstance, ref Utf8JsonReader reader, ILogger logger)
        {
            ...
            while (reader.Read() && reader.TokenType != JsonTokenType.EndObject)
            {
                ...
                {
                    throw new JsonException("Expected a property name in the ConcernDefinition json object.");
                }
                ...
                if (reader.ValueTextEquals("@id"u8))
                {
                    ...
                    {
                        throw new JsonException("The @id property is not present, the ConcernDefinition cannot be deserialized");
                    }
                ...
                if (reader.ValueTextEquals("result"u8))
                {
                    ...
                    if (reader.TokenType == JsonTokenType.Null)
                    {
                        ...
                        {
                            logger.LogDebug("the ConcernDefinition.result property was not found in the Json. The value is set to Guid.Empty");
                        }
                    }
                ...
                if (reader.ValueTextEquals("subjectParameter"u8))
                {
                    ...
                    if (reader.TokenType == JsonTokenType.Null)
                    {
                        ...
                        if (logger.IsEnabled(LogLevel.Debug))
                        {
                            logger.LogDebug("the ConcernDefinition.subjectParameter property was not found in the Json. The value is set to Guid.Empty");
                        }
                    }
            ...
            if (logger.IsEnabled(LogLevel.Debug))
            {
                ...
                {
                    logger.LogDebug("the actorParameter Json property was not found in the ConcernDefinition: {Id}", dtoInstance.Id);
                }
                if (!aliasIdsSeen)
                {
                ...
                {
                    logger.LogDebug("the assumedConstraint Json property was not found in the ConcernDefinition: {Id}", dtoInstance.Id);
                }
                if (!declaredNameSeen)
                {
                ...
                {
                    logger.LogDebug("the directedUsage Json property was not found in the ConcernDefinition: {Id}", dtoInstance.Id);
                }
                if (!documentationSeen)
                {
                    logger.LogDebug("the documentation Json property was not found in the ConcernDefinition: {Id}", dtoInstance.Id);
                }
                if (!elementIdSeen)
                {
                ...
                if (!endFeatureSeen)
                {
                    logger.LogDebug("the endFeature Json property was not found in the ConcernDefinition: {Id}", dtoInstance.Id);
                }
                if (!expressionSeen)
                {
                    logger.LogDebug("the expression Json property was not found in the ConcernDefinition: {Id}", dtoInstance.Id);
                }
                if (!featureSeen)
                {
                    logger.LogDebug("the feature Json property was not found in the ConcernDefinition: {Id}", dtoInstance.Id);
                }
                if (!featureMembershipSeen)
                {
                    logger.LogDebug("the featureMembership Json property was not found in the ConcernDefinition: {Id}", dtoInstance.Id);
                }
                if (!framedConcernSeen)
                {
                    logger.LogDebug("the framedConcern Json property was not found in the ConcernDefinition: {Id}", dtoInstance.Id);
                }
                if (!importedMembershipSeen)
                {
                    logger.LogDebug("the importedMembership Json property was not found in the ConcernDefinition: {Id}", dtoInstance.Id);
                }
                if (!inheritedFeatureSeen)
                {
                    logger.LogDebug("the inheritedFeature Json property was not found in the ConcernDefinition: {Id}", dtoInstance.Id);
                }
                if (!inheritedMembershipSeen)
                {
                    logger.LogDebug("the inheritedMembership Json property was not found in the ConcernDefinition: {Id}", dtoInstance.Id);
                }
                if (!inputSeen)
                {
                    logger.LogDebug("the input Json property was not found in the ConcernDefinition: {Id}", dtoInstance.Id);
                }
                if (!intersectingTypeSeen)
                {
                    logger.LogDebug("the intersectingType Json property was not found in the ConcernDefinition: {Id}", dtoInstance.Id);
                }
                if (!isAbstractSeen)
                {
                ...
                if (!isConjugatedSeen)
                {
                    logger.LogDebug("the isConjugated Json property was not found in the ConcernDefinition: {Id}", dtoInstance.Id);
                }
                ...
                {
                    logger.LogDebug("the isLibraryElement Json property was not found in the ConcernDefinition: {Id}", dtoInstance.Id);
                }
                if (!isModelLevelEvaluableSeen)
                {
                    logger.LogDebug("the isModelLevelEvaluable Json property was not found in the ConcernDefinition: {Id}", dtoInstance.Id);
                }
                if (!isSufficientSeen)
                {
                ...
                if (!memberSeen)
                {
                    logger.LogDebug("the member Json property was not found in the ConcernDefinition: {Id}", dtoInstance.Id);
                }
                if (!membershipSeen)
                {
                    logger.LogDebug("the membership Json property was not found in the ConcernDefinition: {Id}", dtoInstance.Id);
                }
                if (!multiplicitySeen)
                {
                    logger.LogDebug("the multiplicity Json property was not found in the ConcernDefinition: {Id}", dtoInstance.Id);
                }
                if (!nameSeen)
                {
                    logger.LogDebug("the name Json property was not found in the ConcernDefinition: {Id}", dtoInstance.Id);
                }
                if (!outputSeen)
                {
                    logger.LogDebug("the output Json property was not found in the ConcernDefinition: {Id}", dtoInstance.Id);
                }
                if (!ownedActionSeen)
                {
                    logger.LogDebug("the ownedAction Json property was not found in the ConcernDefinition: {Id}", dtoInstance.Id);
                }
                if (!ownedAllocationSeen)
                {
                    logger.LogDebug("the ownedAllocation Json property was not found in the ConcernDefinition: {Id}", dtoInstance.Id);
                }
                if (!ownedAnalysisCaseSeen)
                {
                    logger.LogDebug("the ownedAnalysisCase Json property was not found in the ConcernDefinition: {Id}", dtoInstance.Id);
                }
                if (!ownedAnnotationSeen)
                {
                    logger.LogDebug("the ownedAnnotation Json property was not found in the ConcernDefinition: {Id}", dtoInstance.Id);
                }
                if (!ownedAttributeSeen)
                {
                    logger.LogDebug("the ownedAttribute Json property was not found in the ConcernDefinition: {Id}", dtoInstance.Id);
                }
                if (!ownedCalculationSeen)
                {
                    logger.LogDebug("the ownedCalculation Json property was not found in the ConcernDefinition: {Id}", dtoInstance.Id);
                }
                if (!ownedCaseSeen)
                {
                    logger.LogDebug("the ownedCase Json property was not found in the ConcernDefinition: {Id}", dtoInstance.Id);
                }
                if (!ownedConcernSeen)
                {
                    logger.LogDebug("the ownedConcern Json property was not found in the ConcernDefinition: {Id}", dtoInstance.Id);
                }
                if (!ownedConjugatorSeen)
                {
                    logger.LogDebug("the ownedConjugator Json property was not found in the ConcernDefinition: {Id}", dtoInstance.Id);
                }
                if (!ownedConnectionSeen)
                {
                    logger.LogDebug("the ownedConnection Json property was not found in the ConcernDefinition: {Id}", dtoInstance.Id);
                }
                if (!ownedConstraintSeen)
                {
                    logger.LogDebug("the ownedConstraint Json property was not found in the ConcernDefinition: {Id}", dtoInstance.Id);
                }
                if (!ownedDifferencingSeen)
                {
                    logger.LogDebug("the ownedDifferencing Json property was not found in the ConcernDefinition: {Id}", dtoInstance.Id);
                }
                if (!ownedDisjoiningSeen)
                {
                    logger.LogDebug("the ownedDisjoining Json property was not found in the ConcernDefinition: {Id}", dtoInstance.Id);
                }
                if (!ownedElementSeen)
                {
                    logger.LogDebug("the ownedElement Json property was not found in the ConcernDefinition: {Id}", dtoInstance.Id);
                }
                if (!ownedEndFeatureSeen)
                {
                    logger.LogDebug("the ownedEndFeature Json property was not found in the ConcernDefinition: {Id}", dtoInstance.Id);
                }
                if (!ownedEnumerationSeen)
                {
                    logger.LogDebug("the ownedEnumeration Json property was not found in the ConcernDefinition: {Id}", dtoInstance.Id);
                }
                if (!ownedFeatureSeen)
                {
                    logger.LogDebug("the ownedFeature Json property was not found in the ConcernDefinition: {Id}", dtoInstance.Id);
                }
                if (!ownedFeatureMembershipSeen)
                {
                    logger.LogDebug("the ownedFeatureMembership Json property was not found in the ConcernDefinition: {Id}", dtoInstance.Id);
                }
                if (!ownedFlowSeen)
                {
                    logger.LogDebug("the ownedFlow Json property was not found in the ConcernDefinition: {Id}", dtoInstance.Id);
                }
                if (!ownedImportSeen)
                {
                    logger.LogDebug("the ownedImport Json property was not found in the ConcernDefinition: {Id}", dtoInstance.Id);
                }
                if (!ownedInterfaceSeen)
                {
                    logger.LogDebug("the ownedInterface Json property was not found in the ConcernDefinition: {Id}", dtoInstance.Id);
                }
                if (!ownedIntersectingSeen)
                {
                    logger.LogDebug("the ownedIntersecting Json property was not found in the ConcernDefinition: {Id}", dtoInstance.Id);
                }
                if (!ownedItemSeen)
                {
                    logger.LogDebug("the ownedItem Json property was not found in the ConcernDefinition: {Id}", dtoInstance.Id);
                }
                if (!ownedMemberSeen)
                {
                    logger.LogDebug("the ownedMember Json property was not found in the ConcernDefinition: {Id}", dtoInstance.Id);
                }
                if (!ownedMembershipSeen)
                {
                    logger.LogDebug("the ownedMembership Json property was not found in the ConcernDefinition: {Id}", dtoInstance.Id);
                }
                if (!ownedMetadataSeen)
                {
                    logger.LogDebug("the ownedMetadata Json property was not found in the ConcernDefinition: {Id}", dtoInstance.Id);
                }
                if (!ownedOccurrenceSeen)
                {
                    logger.LogDebug("the ownedOccurrence Json property was not found in the ConcernDefinition: {Id}", dtoInstance.Id);
                }
                if (!ownedPartSeen)
                {
                    logger.LogDebug("the ownedPart Json property was not found in the ConcernDefinition: {Id}", dtoInstance.Id);
                }
                if (!ownedPortSeen)
                {
                    logger.LogDebug("the ownedPort Json property was not found in the ConcernDefinition: {Id}", dtoInstance.Id);
                }
                if (!ownedReferenceSeen)
                {
                    logger.LogDebug("the ownedReference Json property was not found in the ConcernDefinition: {Id}", dtoInstance.Id);
                }
                ...
                {
                    logger.LogDebug("the ownedRendering Json property was not found in the ConcernDefinition: {Id}", dtoInstance.Id);
                }
                if (!ownedRequirementSeen)
                {
                    logger.LogDebug("the ownedRequirement Json property was not found in the ConcernDefinition: {Id}", dtoInstance.Id);
                }
                if (!ownedSpecializationSeen)
                {
                    logger.LogDebug("the ownedSpecialization Json property was not found in the ConcernDefinition: {Id}", dtoInstance.Id);
                }
                if (!ownedStateSeen)
                {
                    logger.LogDebug("the ownedState Json property was not found in the ConcernDefinition: {Id}", dtoInstance.Id);
                }
                if (!ownedSubclassificationSeen)
                {
                    logger.LogDebug("the ownedSubclassification Json property was not found in the ConcernDefinition: {Id}", dtoInstance.Id);
                }
                if (!ownedTransitionSeen)
                {
                    logger.LogDebug("the ownedTransition Json property was not found in the ConcernDefinition: {Id}", dtoInstance.Id);
                }
                if (!ownedUnioningSeen)
                {
                    logger.LogDebug("the ownedUnioning Json property was not found in the ConcernDefinition: {Id}", dtoInstance.Id);
                }
                if (!ownedUsageSeen)
                {
                    logger.LogDebug("the ownedUsage Json property was not found in the ConcernDefinition: {Id}", dtoInstance.Id);
                }
                if (!ownedUseCaseSeen)
                {
                    logger.LogDebug("the ownedUseCase Json property was not found in the ConcernDefinition: {Id}", dtoInstance.Id);
                }
                if (!ownedVerificationCaseSeen)
                {
                    logger.LogDebug("the ownedVerificationCase Json property was not found in the ConcernDefinition: {Id}", dtoInstance.Id);
                }
                if (!ownedViewSeen)
                {
                    logger.LogDebug("the ownedView Json property was not found in the ConcernDefinition: {Id}", dtoInstance.Id);
                }
                if (!ownedViewpointSeen)
                {
                    logger.LogDebug("the ownedViewpoint Json property was not found in the ConcernDefinition: {Id}", dtoInstance.Id);
                }
                if (!ownerSeen)
                {
                    logger.LogDebug("the owner Json property was not found in the ConcernDefinition: {Id}", dtoInstance.Id);
                }
                if (!owningMembershipSeen)
                {
                    logger.LogDebug("the owningMembership Json property was not found in the ConcernDefinition: {Id}", dtoInstance.Id);
                }
                if (!owningNamespaceSeen)
                {
                    logger.LogDebug("the owningNamespace Json property was not found in the ConcernDefinition: {Id}", dtoInstance.Id);
                }
                ...
                if (!parameterSeen)
                {
                    logger.LogDebug("the parameter Json property was not found in the ConcernDefinition: {Id}", dtoInstance.Id);
                }
                if (!qualifiedNameSeen)
                {
                    logger.LogDebug("the qualifiedName Json property was not found in the ConcernDefinition: {Id}", dtoInstance.Id);
                }
                if (!reqIdSeen)
                {
                ...
                {
                    logger.LogDebug("the requiredConstraint Json property was not found in the ConcernDefinition: {Id}", dtoInstance.Id);
                }
                if (!resultSeen)
                {
                    logger.LogDebug("the result Json property was not found in the ConcernDefinition: {Id}", dtoInstance.Id);
                }
                if (!shortNameSeen)
                {
                    logger.LogDebug("the shortName Json property was not found in the ConcernDefinition: {Id}", dtoInstance.Id);
                }
                if (!stakeholderParameterSeen)
                {
                    logger.LogDebug("the stakeholderParameter Json property was not found in the ConcernDefinition: {Id}", dtoInstance.Id);
                }
                if (!stepSeen)
                {
                    logger.LogDebug("the step Json property was not found in the ConcernDefinition: {Id}", dtoInstance.Id);
                }
                if (!subjectParameterSeen)
                {
                    logger.LogDebug("the subjectParameter Json property was not found in the ConcernDefinition: {Id}", dtoInstance.Id);
                }
                if (!textSeen)
                {
                    logger.LogDebug("the text Json property was not found in the ConcernDefinition: {Id}", dtoInstance.Id);
                }
                if (!textualRepresentationSeen)
                {
                    logger.LogDebug("the textualRepresentation Json property was not found in the ConcernDefinition: {Id}", dtoInstance.Id);
                }
                if (!unioningTypeSeen)
                {
                    logger.LogDebug("the unioningType Json property was not found in the ConcernDefinition: {Id}", dtoInstance.Id);
                }
                if (!usageSeen)
                {
                    logger.LogDebug("the usage Json property was not found in the ConcernDefinition: {Id}", dtoInstance.Id);
                }
                if (!variantSeen)
                {
                    logger.LogDebug("the variant Json property was not found in the ConcernDefinition: {Id}", dtoInstance.Id);
                }
        ...
        /// The <see cref="ILogger"/> to produce logging statement
        /// </param>
        private static void DeserializeDtoExcludingDerivedProperties(SysML2.NET.Core.DTO.Systems.Requirements.ConcernDefinition dtoInstance, ref Utf8JsonReader reader, ILogger logger)
        {
            var aliasIdsSeen = false;
            var declaredNameSeen = false;
            var elementIdSeen = false;
