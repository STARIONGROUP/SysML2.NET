...
namespace SysML2.NET.Serializer.Json.Core.DTO
{
    ...
    {
        /// <summary>
        /// Deserializes an instance of <see cref="IRenderingDefinition"/> from the provided <see cref="JsonElement"/>
        /// </summary>
        ...
        internal static IRenderingDefinition DeSerialize(JsonElement jsonElement, SerializationModeKind serializationModeKind, bool deserializeDerivedProperties, ILoggerFactory loggerFactory = null)
        {
            ...
            {
                throw new InvalidOperationException("The @type property is not available, the RenderingDefinitionDeSerializer cannot be used to deserialize this JsonElement");
            }

            if (@type.GetString() != "RenderingDefinition")
            {
        ...
        private static void DeserializeDtoIncludingDerivedProperties(SysML2.NET.Core.DTO.Systems.Views.RenderingDefinition dtoInstance, JsonElement jsonElement, ILogger logger)
        {
            ...
            if (jsonElement.TryGetProperty("rendering"u8, out var renderingProperty))
            {
                foreach (var arrayItem in renderingProperty.EnumerateArray())
                {
                    if (arrayItem.TryGetProperty("@id"u8, out var renderingExternalIdProperty))
                    {
                        var propertyValue = renderingExternalIdProperty.GetString();

                        if (propertyValue != null)
                        {
                            dtoInstance.rendering.Add(Guid.Parse(propertyValue));
                        }
                    }
                }
            }
