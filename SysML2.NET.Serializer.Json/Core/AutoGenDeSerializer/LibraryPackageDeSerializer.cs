...
namespace SysML2.NET.Serializer.Json.Core.DTO
{
    ...
    {
        /// <summary>
        /// Deserializes an instance of <see cref="ILibraryPackage"/> from the provided <see cref="Utf8JsonReader"/>
        /// </summary>
        /// <param name="reader">
        /// The <see cref="Utf8JsonReader"/> positioned on the <see cref="JsonTokenType.StartObject"/> of the
        /// <see cref="ILibraryPackage"/> json object. On return the reader is positioned on the matching
        ...
        /// </remarks>
        internal static ILibraryPackage DeSerialize(ref Utf8JsonReader reader, SerializationModeKind serializationModeKind, bool deserializeDerivedProperties, ILoggerFactory loggerFactory = null)
        {
            ...
            var dtoInstance = new SysML2.NET.Core.DTO.Kernel.Packages.LibraryPackage();
        ...
        /// <summary>
        /// Deserializes properties of a <see cref="SysML2.NET.Core.DTO.Kernel.Packages.LibraryPackage" />
        /// from a <see cref="Utf8JsonReader" />, including derived properties
        /// </summary>
        /// <param name="dtoInstance">
        /// The <see cref="SysML2.NET.Core.DTO.Kernel.Packages.LibraryPackage"/> instance holding deserialized values
        /// </param>
        /// <param name="reader">
        /// The <see cref="Utf8JsonReader"/> positioned on the <see cref="JsonTokenType.StartObject"/> of the
        /// <see cref="ILibraryPackage"/> json object
        /// </param>
        /// <param name="logger">
        /// The <see cref="ILogger"/> to produce logging statement
        /// </param>
        private static void DeserializeDtoIncludingDerivedProperties(SysML2.NET.Core.DTO.Kernel.Packages.LibraryPackage dtoInstance, ref Utf8JsonReader reader, ILogger logger)
        {
            ...
            while (reader.Read() && reader.TokenType != JsonTokenType.EndObject)
            {
                ...
                {
                    throw new JsonException("Expected a property name in the LibraryPackage json object.");
                }
                ...
                if (reader.ValueTextEquals("filterCondition"u8))
                {
                    filterConditionSeen = true;
                    reader.Read();
                    ...
                    while (reader.Read() && reader.TokenType != JsonTokenType.EndArray)
                    {
                        if (Utf8JsonReaderHelper.TryReadReferenceIdentifier(ref reader, out var filterConditionValue))
                        {
                            dtoInstance.filterCondition.Add(filterConditionValue);
                        }
                    }
                ...
                if (reader.ValueTextEquals("isStandard"u8))
                {
                    isStandardSeen = true;
                    reader.Read();
                    ...
                    if (reader.TokenType != JsonTokenType.Null)
                    {
                        dtoInstance.IsStandard = reader.GetBoolean();
                    }
            ...
            if (logger.IsEnabled(LogLevel.Debug))
            {
                ...
                {
                    logger.LogDebug("the documentation Json property was not found in the LibraryPackage: {Id}", dtoInstance.Id);
                }
                if (!elementIdSeen)
                {
                    logger.LogDebug("the elementId Json property was not found in the LibraryPackage: {Id}", dtoInstance.Id);
                }
                if (!filterConditionSeen)
                {
                    logger.LogDebug("the filterCondition Json property was not found in the LibraryPackage: {Id}", dtoInstance.Id);
                }
                if (!importedMembershipSeen)
                {
                    logger.LogDebug("the importedMembership Json property was not found in the LibraryPackage: {Id}", dtoInstance.Id);
                }
                ...
                {
                    logger.LogDebug("the isLibraryElement Json property was not found in the LibraryPackage: {Id}", dtoInstance.Id);
                }
                if (!isStandardSeen)
                {
                ...
                if (!memberSeen)
                {
                    logger.LogDebug("the member Json property was not found in the LibraryPackage: {Id}", dtoInstance.Id);
                }
                if (!membershipSeen)
                {
                    logger.LogDebug("the membership Json property was not found in the LibraryPackage: {Id}", dtoInstance.Id);
                }
                if (!nameSeen)
                {
                    logger.LogDebug("the name Json property was not found in the LibraryPackage: {Id}", dtoInstance.Id);
                }
                if (!ownedAnnotationSeen)
                {
                    logger.LogDebug("the ownedAnnotation Json property was not found in the LibraryPackage: {Id}", dtoInstance.Id);
                }
                if (!ownedElementSeen)
                {
                    logger.LogDebug("the ownedElement Json property was not found in the LibraryPackage: {Id}", dtoInstance.Id);
                }
                if (!ownedImportSeen)
                {
                    logger.LogDebug("the ownedImport Json property was not found in the LibraryPackage: {Id}", dtoInstance.Id);
                }
                if (!ownedMemberSeen)
                {
                    logger.LogDebug("the ownedMember Json property was not found in the LibraryPackage: {Id}", dtoInstance.Id);
                }
                if (!ownedMembershipSeen)
                {
                    logger.LogDebug("the ownedMembership Json property was not found in the LibraryPackage: {Id}", dtoInstance.Id);
                }
                ...
                if (!ownerSeen)
                {
                    logger.LogDebug("the owner Json property was not found in the LibraryPackage: {Id}", dtoInstance.Id);
                }
                if (!owningMembershipSeen)
                {
                    logger.LogDebug("the owningMembership Json property was not found in the LibraryPackage: {Id}", dtoInstance.Id);
                }
                if (!owningNamespaceSeen)
                {
                    logger.LogDebug("the owningNamespace Json property was not found in the LibraryPackage: {Id}", dtoInstance.Id);
                }
                ...
                {
                    logger.LogDebug("the qualifiedName Json property was not found in the LibraryPackage: {Id}", dtoInstance.Id);
                }
                if (!shortNameSeen)
                {
                    logger.LogDebug("the shortName Json property was not found in the LibraryPackage: {Id}", dtoInstance.Id);
                }
        ...
        /// The <see cref="ILogger"/> to produce logging statement
        /// </param>
        private static void DeserializeDtoExcludingDerivedProperties(SysML2.NET.Core.DTO.Kernel.Packages.LibraryPackage dtoInstance, ref Utf8JsonReader reader, ILogger logger)
        {
            var aliasIdsSeen = false;
            var declaredNameSeen = false;
            var declaredShortNameSeen = false;
