...
namespace SysML2.NET.Serializer.Json.Core.DTO
{
    ...
    {
        /// <summary>
        /// Deserializes an instance of <see cref="IFeatureChaining"/> from the provided <see cref="Utf8JsonReader"/>
        /// </summary>
        /// <param name="reader">
        /// The <see cref="Utf8JsonReader"/> positioned on the <see cref="JsonTokenType.StartObject"/> of the
        /// <see cref="IFeatureChaining"/> json object. On return the reader is positioned on the matching
        ...
        /// </remarks>
        internal static IFeatureChaining DeSerialize(ref Utf8JsonReader reader, SerializationModeKind serializationModeKind, bool deserializeDerivedProperties, ILoggerFactory loggerFactory = null)
        {
            ...
            var dtoInstance = new SysML2.NET.Core.DTO.Core.Features.FeatureChaining();
        ...
        /// <summary>
        /// Deserializes properties of a <see cref="SysML2.NET.Core.DTO.Core.Features.FeatureChaining" />
        /// from a <see cref="Utf8JsonReader" />, including derived properties
        /// </summary>
        /// <param name="dtoInstance">
        /// The <see cref="SysML2.NET.Core.DTO.Core.Features.FeatureChaining"/> instance holding deserialized values
        /// </param>
        /// <param name="reader">
        /// The <see cref="Utf8JsonReader"/> positioned on the <see cref="JsonTokenType.StartObject"/> of the
        /// <see cref="IFeatureChaining"/> json object
        /// </param>
        /// <param name="logger">
        /// The <see cref="ILogger"/> to produce logging statement
        /// </param>
        private static void DeserializeDtoIncludingDerivedProperties(SysML2.NET.Core.DTO.Core.Features.FeatureChaining dtoInstance, ref Utf8JsonReader reader, ILogger logger)
        {
            ...
            while (reader.Read() && reader.TokenType != JsonTokenType.EndObject)
            {
                ...
                {
                    throw new JsonException("Expected a property name in the FeatureChaining json object.");
                }
                ...
                if (reader.ValueTextEquals("chainingFeature"u8))
                {
                    ...
                    else if (Utf8JsonReaderHelper.TryReadReferenceIdentifier(ref reader, out var chainingFeatureValue))
                    {
                        dtoInstance.ChainingFeature = chainingFeatureValue;
                    }
                ...
                if (reader.ValueTextEquals("featureChained"u8))
                {
                    featureChainedSeen = true;
                    reader.Read();

                    if (reader.TokenType == JsonTokenType.Null)
                    {
                        dtoInstance.featureChained = Guid.Empty;

                        if (logger.IsEnabled(LogLevel.Debug))
                        {
                            logger.LogDebug("the FeatureChaining.featureChained property was not found in the Json. The value is set to Guid.Empty");
                        }
                    }
                    else if (Utf8JsonReaderHelper.TryReadReferenceIdentifier(ref reader, out var featureChainedValue))
                    {
                        dtoInstance.featureChained = featureChainedValue;
                    }
            ...
            if (logger.IsEnabled(LogLevel.Debug))
            {
                ...
                {
                    logger.LogDebug("the documentation Json property was not found in the FeatureChaining: {Id}", dtoInstance.Id);
                }
                if (!elementIdSeen)
                {
                    logger.LogDebug("the elementId Json property was not found in the FeatureChaining: {Id}", dtoInstance.Id);
                }
                if (!featureChainedSeen)
                {
                    logger.LogDebug("the featureChained Json property was not found in the FeatureChaining: {Id}", dtoInstance.Id);
                }
                if (!isImpliedSeen)
                {
                ...
                {
                    logger.LogDebug("the isLibraryElement Json property was not found in the FeatureChaining: {Id}", dtoInstance.Id);
                }
                if (!nameSeen)
                {
                    logger.LogDebug("the name Json property was not found in the FeatureChaining: {Id}", dtoInstance.Id);
                }
                if (!ownedAnnotationSeen)
                {
                    logger.LogDebug("the ownedAnnotation Json property was not found in the FeatureChaining: {Id}", dtoInstance.Id);
                }
                if (!ownedElementSeen)
                {
                    logger.LogDebug("the ownedElement Json property was not found in the FeatureChaining: {Id}", dtoInstance.Id);
                }
                ...
                if (!ownerSeen)
                {
                    logger.LogDebug("the owner Json property was not found in the FeatureChaining: {Id}", dtoInstance.Id);
                }
                if (!owningMembershipSeen)
                {
                    logger.LogDebug("the owningMembership Json property was not found in the FeatureChaining: {Id}", dtoInstance.Id);
                }
                if (!owningNamespaceSeen)
                {
                    logger.LogDebug("the owningNamespace Json property was not found in the FeatureChaining: {Id}", dtoInstance.Id);
                }
                ...
                {
                    logger.LogDebug("the qualifiedName Json property was not found in the FeatureChaining: {Id}", dtoInstance.Id);
                }
                if (!relatedElementSeen)
                {
                    logger.LogDebug("the relatedElement Json property was not found in the FeatureChaining: {Id}", dtoInstance.Id);
                }
                if (!shortNameSeen)
                {
                    logger.LogDebug("the shortName Json property was not found in the FeatureChaining: {Id}", dtoInstance.Id);
                }
        ...
        /// The <see cref="ILogger"/> to produce logging statement
        /// </param>
        private static void DeserializeDtoExcludingDerivedProperties(SysML2.NET.Core.DTO.Core.Features.FeatureChaining dtoInstance, ref Utf8JsonReader reader, ILogger logger)
        {
            var aliasIdsSeen = false;
            var chainingFeatureSeen = false;
            var declaredNameSeen = false;
