...
namespace SysML2.NET.Serializer.Json.Core.DTO
{
    ...
    internal static class FeatureChainingDeSerializer
    {
        ...
        internal static IFeatureChaining DeSerialize(JsonElement jsonElement, SerializationModeKind serializationModeKind, ILoggerFactory loggerFactory = null)
        {
            ...
            if (jsonElement.TryGetProperty("featureChained"u8, out var featureChainedProperty))
            {
                if (featureChainedProperty.ValueKind == JsonValueKind.Null)
                {
                ...
                {
                    if (featureChainedProperty.TryGetProperty("@id"u8, out var featureChainedExternalIdProperty))
                    {
                        var propertyValue = featureChainedExternalIdProperty.GetString();

                        if (propertyValue != null)
                        {
                            dtoInstance.featureChained = Guid.Parse(propertyValue);
                        }
                    }
                }
            }
            ...
            {
                logger.LogDebug("the source Json property was not found in the FeatureChaining: { Id }", dtoInstance.Id);
            }
            ...
            {
                logger.LogDebug("the target Json property was not found in the FeatureChaining: { Id }", dtoInstance.Id);
            }
