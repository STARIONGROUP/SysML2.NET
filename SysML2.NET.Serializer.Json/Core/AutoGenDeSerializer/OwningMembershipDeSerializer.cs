...
namespace SysML2.NET.Serializer.Json.Core.DTO
{
    ...
    /// </summary>
    internal static class OwningMembershipDeSerializer
    {
        /// <summary>
        /// Deserializes an instance of <see cref="IOwningMembership"/> from the provided <see cref="Utf8JsonReader"/>
        /// </summary>
        /// <param name="reader">
        /// The <see cref="Utf8JsonReader"/> positioned on the <see cref="JsonTokenType.StartObject"/> of the
        /// <see cref="IOwningMembership"/> json object. On return the reader is positioned on the matching
        ...
        /// </remarks>
        internal static IOwningMembership DeSerialize(ref Utf8JsonReader reader, SerializationModeKind serializationModeKind, bool deserializeDerivedProperties, ILoggerFactory loggerFactory = null)
        {
            ...
            var dtoInstance = new SysML2.NET.Core.DTO.Root.Namespaces.OwningMembership();
        ...
        /// <summary>
        /// Deserializes properties of a <see cref="SysML2.NET.Core.DTO.Root.Namespaces.OwningMembership" />
        /// from a <see cref="Utf8JsonReader" />, including derived properties
        /// </summary>
        /// <param name="dtoInstance">
        /// The <see cref="SysML2.NET.Core.DTO.Root.Namespaces.OwningMembership"/> instance holding deserialized values
        /// </param>
        /// <param name="reader">
        /// The <see cref="Utf8JsonReader"/> positioned on the <see cref="JsonTokenType.StartObject"/> of the
        /// <see cref="IOwningMembership"/> json object
        /// </param>
        /// <param name="logger">
        /// The <see cref="ILogger"/> to produce logging statement
        /// </param>
        private static void DeserializeDtoIncludingDerivedProperties(SysML2.NET.Core.DTO.Root.Namespaces.OwningMembership dtoInstance, ref Utf8JsonReader reader, ILogger logger)
        {
            ...
            var ownedMemberElementSeen = false;
            ...
            while (reader.Read() && reader.TokenType != JsonTokenType.EndObject)
            {
                ...
                {
                    throw new JsonException("Expected a property name in the OwningMembership json object.");
                }
                ...
                if (reader.ValueTextEquals("membershipOwningNamespace"u8))
                {
                    ...
                    if (reader.TokenType == JsonTokenType.Null)
                    {
                        ...
                        {
                            logger.LogDebug("the OwningMembership.membershipOwningNamespace property was not found in the Json. The value is set to Guid.Empty");
                        }
                    }
                ...
                if (reader.ValueTextEquals("ownedMemberElement"u8))
                {
                    ownedMemberElementSeen = true;
                    reader.Read();
                    ...
                    if (reader.TokenType == JsonTokenType.Null)
                    {
                        ...
                        if (logger.IsEnabled(LogLevel.Debug))
                        {
                            logger.LogDebug("the OwningMembership.ownedMemberElement property was not found in the Json. The value is set to Guid.Empty");
                        }
                    }
                    else if (Utf8JsonReaderHelper.TryReadReferenceIdentifier(ref reader, out var ownedMemberElementValue))
                    {
                        dtoInstance.ownedMemberElement = ownedMemberElementValue;
                    }
            ...
            if (logger.IsEnabled(LogLevel.Debug))
            {
                ...
                {
                    logger.LogDebug("the documentation Json property was not found in the OwningMembership: {Id}", dtoInstance.Id);
                }
                if (!elementIdSeen)
                {
                ...
                {
                    logger.LogDebug("the isLibraryElement Json property was not found in the OwningMembership: {Id}", dtoInstance.Id);
                }
                if (!membershipOwningNamespaceSeen)
                {
                    logger.LogDebug("the membershipOwningNamespace Json property was not found in the OwningMembership: {Id}", dtoInstance.Id);
                }
                if (!nameSeen)
                {
                    logger.LogDebug("the name Json property was not found in the OwningMembership: {Id}", dtoInstance.Id);
                }
                if (!ownedAnnotationSeen)
                {
                    logger.LogDebug("the ownedAnnotation Json property was not found in the OwningMembership: {Id}", dtoInstance.Id);
                }
                if (!ownedElementSeen)
                {
                    logger.LogDebug("the ownedElement Json property was not found in the OwningMembership: {Id}", dtoInstance.Id);
                }
                if (!ownedMemberElementSeen)
                {
                    logger.LogDebug("the ownedMemberElement Json property was not found in the OwningMembership: {Id}", dtoInstance.Id);
                }
                if (!ownedMemberElementIdSeen)
                {
                    logger.LogDebug("the ownedMemberElementId Json property was not found in the OwningMembership: {Id}", dtoInstance.Id);
                }
                if (!ownedMemberNameSeen)
                {
                    logger.LogDebug("the ownedMemberName Json property was not found in the OwningMembership: {Id}", dtoInstance.Id);
                }
                if (!ownedMemberShortNameSeen)
                {
                    logger.LogDebug("the ownedMemberShortName Json property was not found in the OwningMembership: {Id}", dtoInstance.Id);
                }
                ...
                if (!ownerSeen)
                {
                    logger.LogDebug("the owner Json property was not found in the OwningMembership: {Id}", dtoInstance.Id);
                }
                if (!owningMembershipSeen)
                {
                    logger.LogDebug("the owningMembership Json property was not found in the OwningMembership: {Id}", dtoInstance.Id);
                }
                if (!owningNamespaceSeen)
                {
                    logger.LogDebug("the owningNamespace Json property was not found in the OwningMembership: {Id}", dtoInstance.Id);
                }
                ...
                {
                    logger.LogDebug("the qualifiedName Json property was not found in the OwningMembership: {Id}", dtoInstance.Id);
                }
                if (!relatedElementSeen)
                {
                    logger.LogDebug("the relatedElement Json property was not found in the OwningMembership: {Id}", dtoInstance.Id);
                }
                if (!shortNameSeen)
                {
                    logger.LogDebug("the shortName Json property was not found in the OwningMembership: {Id}", dtoInstance.Id);
                }
                if (!textualRepresentationSeen)
                {
                    logger.LogDebug("the textualRepresentation Json property was not found in the OwningMembership: {Id}", dtoInstance.Id);
                }
                if (!visibilitySeen)
                {
        ...
        /// The <see cref="ILogger"/> to produce logging statement
        /// </param>
        private static void DeserializeDtoExcludingDerivedProperties(SysML2.NET.Core.DTO.Root.Namespaces.OwningMembership dtoInstance, ref Utf8JsonReader reader, ILogger logger)
        {
            var aliasIdsSeen = false;
            var declaredNameSeen = false;
            var declaredShortNameSeen = false;
