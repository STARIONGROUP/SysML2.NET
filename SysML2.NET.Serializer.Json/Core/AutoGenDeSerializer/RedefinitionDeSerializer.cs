...
namespace SysML2.NET.Serializer.Json.Core.DTO
{
    ...
    {
        /// <summary>
        /// Deserializes an instance of <see cref="IRedefinition"/> from the provided <see cref="Utf8JsonReader"/>
        /// </summary>
        /// <param name="reader">
        /// The <see cref="Utf8JsonReader"/> positioned on the <see cref="JsonTokenType.StartObject"/> of the
        /// <see cref="IRedefinition"/> json object. On return the reader is positioned on the matching
        ...
        /// </remarks>
        internal static IRedefinition DeSerialize(ref Utf8JsonReader reader, SerializationModeKind serializationModeKind, bool deserializeDerivedProperties, ILoggerFactory loggerFactory = null)
        {
            ...
            var dtoInstance = new SysML2.NET.Core.DTO.Core.Features.Redefinition();
        ...
        /// <summary>
        /// Deserializes properties of a <see cref="SysML2.NET.Core.DTO.Core.Features.Redefinition" />
        /// from a <see cref="Utf8JsonReader" />, including derived properties
        /// </summary>
        /// <param name="dtoInstance">
        /// The <see cref="SysML2.NET.Core.DTO.Core.Features.Redefinition"/> instance holding deserialized values
        /// </param>
        /// <param name="reader">
        /// The <see cref="Utf8JsonReader"/> positioned on the <see cref="JsonTokenType.StartObject"/> of the
        /// <see cref="IRedefinition"/> json object
        /// </param>
        /// <param name="logger">
        /// The <see cref="ILogger"/> to produce logging statement
        /// </param>
        private static void DeserializeDtoIncludingDerivedProperties(SysML2.NET.Core.DTO.Core.Features.Redefinition dtoInstance, ref Utf8JsonReader reader, ILogger logger)
        {
            ...
            var redefinedFeatureSeen = false;
            var redefiningFeatureSeen = false;
            ...
            while (reader.Read() && reader.TokenType != JsonTokenType.EndObject)
            {
                ...
                {
                    throw new JsonException("Expected a property name in the Redefinition json object.");
                }
                ...
                if (reader.ValueTextEquals("redefinedFeature"u8))
                {
                    redefinedFeatureSeen = true;
                    reader.Read();
                    ...
                    else if (Utf8JsonReaderHelper.TryReadReferenceIdentifier(ref reader, out var redefinedFeatureValue))
                    {
                        dtoInstance.RedefinedFeature = redefinedFeatureValue;
                    }

                    continue;
                }

                if (reader.ValueTextEquals("redefiningFeature"u8))
                {
                    redefiningFeatureSeen = true;
                    reader.Read();
                    ...
                    else if (Utf8JsonReaderHelper.TryReadReferenceIdentifier(ref reader, out var redefiningFeatureValue))
                    {
                        dtoInstance.RedefiningFeature = redefiningFeatureValue;
                    }
            ...
            if (logger.IsEnabled(LogLevel.Debug))
            {
                ...
                {
                    logger.LogDebug("the documentation Json property was not found in the Redefinition: {Id}", dtoInstance.Id);
                }
                if (!elementIdSeen)
                {
                ...
                {
                    logger.LogDebug("the isLibraryElement Json property was not found in the Redefinition: {Id}", dtoInstance.Id);
                }
                if (!nameSeen)
                {
                    logger.LogDebug("the name Json property was not found in the Redefinition: {Id}", dtoInstance.Id);
                }
                if (!ownedAnnotationSeen)
                {
                    logger.LogDebug("the ownedAnnotation Json property was not found in the Redefinition: {Id}", dtoInstance.Id);
                }
                if (!ownedElementSeen)
                {
                    logger.LogDebug("the ownedElement Json property was not found in the Redefinition: {Id}", dtoInstance.Id);
                }
                ...
                if (!ownerSeen)
                {
                    logger.LogDebug("the owner Json property was not found in the Redefinition: {Id}", dtoInstance.Id);
                }
                if (!owningFeatureSeen)
                {
                    logger.LogDebug("the owningFeature Json property was not found in the Redefinition: {Id}", dtoInstance.Id);
                }
                if (!owningMembershipSeen)
                {
                    logger.LogDebug("the owningMembership Json property was not found in the Redefinition: {Id}", dtoInstance.Id);
                }
                if (!owningNamespaceSeen)
                {
                    logger.LogDebug("the owningNamespace Json property was not found in the Redefinition: {Id}", dtoInstance.Id);
                }
                ...
                {
                    logger.LogDebug("the qualifiedName Json property was not found in the Redefinition: {Id}", dtoInstance.Id);
                }
                if (!redefinedFeatureSeen)
                {
                    logger.LogDebug("the redefinedFeature Json property was not found in the Redefinition: {Id}", dtoInstance.Id);
                }
                if (!redefiningFeatureSeen)
                {
                ...
                {
                    logger.LogDebug("the relatedElement Json property was not found in the Redefinition: {Id}", dtoInstance.Id);
                }
                if (!shortNameSeen)
                {
                    logger.LogDebug("the shortName Json property was not found in the Redefinition: {Id}", dtoInstance.Id);
                }
        ...
        /// The <see cref="ILogger"/> to produce logging statement
        /// </param>
        private static void DeserializeDtoExcludingDerivedProperties(SysML2.NET.Core.DTO.Core.Features.Redefinition dtoInstance, ref Utf8JsonReader reader, ILogger logger)
        {
            var aliasIdsSeen = false;
            var declaredNameSeen = false;
            var declaredShortNameSeen = false;
