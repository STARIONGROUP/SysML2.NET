...
namespace SysML2.NET.Serializer.Json.Core.DTO
{
    ...
    /// </summary>
    internal static class ElementFilterMembershipDeSerializer
    {
        /// <summary>
        /// Deserializes an instance of <see cref="IElementFilterMembership"/> from the provided <see cref="Utf8JsonReader"/>
        /// </summary>
        /// <param name="reader">
        /// The <see cref="Utf8JsonReader"/> positioned on the <see cref="JsonTokenType.StartObject"/> of the
        /// <see cref="IElementFilterMembership"/> json object. On return the reader is positioned on the matching
        ...
        /// </remarks>
        internal static IElementFilterMembership DeSerialize(ref Utf8JsonReader reader, SerializationModeKind serializationModeKind, bool deserializeDerivedProperties, ILoggerFactory loggerFactory = null)
        {
            ...
            var dtoInstance = new SysML2.NET.Core.DTO.Kernel.Packages.ElementFilterMembership();
        ...
        /// <summary>
        /// Deserializes properties of a <see cref="SysML2.NET.Core.DTO.Kernel.Packages.ElementFilterMembership" />
        /// from a <see cref="Utf8JsonReader" />, including derived properties
        /// </summary>
        /// <param name="dtoInstance">
        /// The <see cref="SysML2.NET.Core.DTO.Kernel.Packages.ElementFilterMembership"/> instance holding deserialized values
        /// </param>
        /// <param name="reader">
        /// The <see cref="Utf8JsonReader"/> positioned on the <see cref="JsonTokenType.StartObject"/> of the
        /// <see cref="IElementFilterMembership"/> json object
        /// </param>
        /// <param name="logger">
        /// The <see cref="ILogger"/> to produce logging statement
        /// </param>
        private static void DeserializeDtoIncludingDerivedProperties(SysML2.NET.Core.DTO.Kernel.Packages.ElementFilterMembership dtoInstance, ref Utf8JsonReader reader, ILogger logger)
        {
            ...
            while (reader.Read() && reader.TokenType != JsonTokenType.EndObject)
            {
                ...
                {
                    throw new JsonException("Expected a property name in the ElementFilterMembership json object.");
                }
                ...
                if (reader.ValueTextEquals("condition"u8))
                {
                    conditionSeen = true;
                    reader.Read();
                    ...
                    if (reader.TokenType == JsonTokenType.Null)
                    {
                        dtoInstance.condition = Guid.Empty;

                        if (logger.IsEnabled(LogLevel.Debug))
                        {
                            logger.LogDebug("the ElementFilterMembership.condition property was not found in the Json. The value is set to Guid.Empty");
                        }
                    }
                    else if (Utf8JsonReaderHelper.TryReadReferenceIdentifier(ref reader, out var conditionValue))
                    {
                        dtoInstance.condition = conditionValue;
                    }
                ...
                if (reader.ValueTextEquals("membershipOwningNamespace"u8))
                {
                    ...
                    if (reader.TokenType == JsonTokenType.Null)
                    {
                        ...
                        if (logger.IsEnabled(LogLevel.Debug))
                        {
                            logger.LogDebug("the ElementFilterMembership.membershipOwningNamespace property was not found in the Json. The value is set to Guid.Empty");
                        }
                    }
            ...
            if (logger.IsEnabled(LogLevel.Debug))
            {
                ...
                if (!conditionSeen)
                {
                    logger.LogDebug("the condition Json property was not found in the ElementFilterMembership: {Id}", dtoInstance.Id);
                }
                if (!declaredNameSeen)
                {
                ...
                {
                    logger.LogDebug("the documentation Json property was not found in the ElementFilterMembership: {Id}", dtoInstance.Id);
                }
                if (!elementIdSeen)
                {
                ...
                {
                    logger.LogDebug("the isLibraryElement Json property was not found in the ElementFilterMembership: {Id}", dtoInstance.Id);
                }
                if (!membershipOwningNamespaceSeen)
                {
                    logger.LogDebug("the membershipOwningNamespace Json property was not found in the ElementFilterMembership: {Id}", dtoInstance.Id);
                }
                if (!nameSeen)
                {
                    logger.LogDebug("the name Json property was not found in the ElementFilterMembership: {Id}", dtoInstance.Id);
                }
                if (!ownedAnnotationSeen)
                {
                    logger.LogDebug("the ownedAnnotation Json property was not found in the ElementFilterMembership: {Id}", dtoInstance.Id);
                }
                if (!ownedElementSeen)
                {
                    logger.LogDebug("the ownedElement Json property was not found in the ElementFilterMembership: {Id}", dtoInstance.Id);
                }
                if (!ownedMemberElementIdSeen)
                {
                    logger.LogDebug("the ownedMemberElementId Json property was not found in the ElementFilterMembership: {Id}", dtoInstance.Id);
                }
                if (!ownedMemberNameSeen)
                {
                    logger.LogDebug("the ownedMemberName Json property was not found in the ElementFilterMembership: {Id}", dtoInstance.Id);
                }
                if (!ownedMemberShortNameSeen)
                {
                    logger.LogDebug("the ownedMemberShortName Json property was not found in the ElementFilterMembership: {Id}", dtoInstance.Id);
                }
                ...
                if (!ownerSeen)
                {
                    logger.LogDebug("the owner Json property was not found in the ElementFilterMembership: {Id}", dtoInstance.Id);
                }
                if (!owningMembershipSeen)
                {
                    logger.LogDebug("the owningMembership Json property was not found in the ElementFilterMembership: {Id}", dtoInstance.Id);
                }
                if (!owningNamespaceSeen)
                {
                    logger.LogDebug("the owningNamespace Json property was not found in the ElementFilterMembership: {Id}", dtoInstance.Id);
                }
                ...
                {
                    logger.LogDebug("the qualifiedName Json property was not found in the ElementFilterMembership: {Id}", dtoInstance.Id);
                }
                if (!relatedElementSeen)
                {
                    logger.LogDebug("the relatedElement Json property was not found in the ElementFilterMembership: {Id}", dtoInstance.Id);
                }
                if (!shortNameSeen)
                {
                    logger.LogDebug("the shortName Json property was not found in the ElementFilterMembership: {Id}", dtoInstance.Id);
                }
                if (!textualRepresentationSeen)
                {
                    logger.LogDebug("the textualRepresentation Json property was not found in the ElementFilterMembership: {Id}", dtoInstance.Id);
                }
                if (!visibilitySeen)
                {
        ...
        /// The <see cref="ILogger"/> to produce logging statement
        /// </param>
        private static void DeserializeDtoExcludingDerivedProperties(SysML2.NET.Core.DTO.Kernel.Packages.ElementFilterMembership dtoInstance, ref Utf8JsonReader reader, ILogger logger)
        {
            var aliasIdsSeen = false;
            var declaredNameSeen = false;
            var declaredShortNameSeen = false;
