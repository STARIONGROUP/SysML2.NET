...
namespace SysML2.NET.Serializer.Xmi.Writers
{
    ...
    public class SuccessionAsUsageWriter : XmiDataWriter<ISuccessionAsUsage>
    {
        ...
        public override void Write(XmlWriter xmlWriter, ISuccessionAsUsage poco, string elementName, bool includeDerivedProperties, bool includesImplied, IXmiElementOriginMap elementOriginMap = null, Uri currentFileUri = null)
        {
        ...
        public override async Task WriteAsync(XmlWriter xmlWriter, ISuccessionAsUsage poco, string elementName, bool includeDerivedProperties, bool includesImplied, IXmiElementOriginMap elementOriginMap = null, Uri currentFileUri = null)
        {
            ...
            if (includeDerivedProperties)
            {
                if (poco.association != null)
                {
                    foreach (var item in poco.association)
                    {
                        await this.XmiDataWriterFacade.WriteReferenceElementAsync(xmlWriter, (IData)item, "association", elementOriginMap, currentFileUri);
                    }
                }
            }
            ...
            if (includeDerivedProperties)
            {
                if (poco.definition != null)
                {
                    foreach (var item in poco.definition)
                    {
                        await this.XmiDataWriterFacade.WriteReferenceElementAsync(xmlWriter, (IData)item, "definition", elementOriginMap, currentFileUri);
                    }
                }
            }
