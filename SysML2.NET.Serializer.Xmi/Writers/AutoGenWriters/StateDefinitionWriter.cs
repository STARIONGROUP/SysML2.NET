...
namespace SysML2.NET.Serializer.Xmi.Writers
{
    ...
    public class StateDefinitionWriter : XmiDataWriter<IStateDefinition>
    {
        ...
        public override void Write(XmlWriter xmlWriter, IStateDefinition poco, string elementName, bool includeDerivedProperties, bool includesImplied, IXmiElementOriginMap elementOriginMap = null, Uri currentFileUri = null)
        {
        ...
        public override async Task WriteAsync(XmlWriter xmlWriter, IStateDefinition poco, string elementName, bool includeDerivedProperties, bool includesImplied, IXmiElementOriginMap elementOriginMap = null, Uri currentFileUri = null)
        {
            ...
            if (includeDerivedProperties)
            {
                if (poco.action != null)
                {
                    foreach (var item in poco.action)
                    {
                        await this.XmiDataWriterFacade.WriteReferenceElementAsync(xmlWriter, (IData)item, "action", elementOriginMap, currentFileUri);
                    }
                }
            }
            ...
            if (includeDerivedProperties)
            {
                if (poco.state != null)
                {
                    foreach (var item in poco.state)
                    {
                        await this.XmiDataWriterFacade.WriteReferenceElementAsync(xmlWriter, (IData)item, "state", elementOriginMap, currentFileUri);
                    }
                }
            }
