...
namespace SysML2.NET.Serializer.Xmi.Writers
{
    ...
    public class AnalysisCaseDefinitionWriter : XmiDataWriter<IAnalysisCaseDefinition>
    {
        ...
        public override void Write(XmlWriter xmlWriter, IAnalysisCaseDefinition poco, string elementName, bool includeDerivedProperties, bool includesImplied, IXmiElementOriginMap elementOriginMap = null, Uri currentFileUri = null)
        {
        ...
        public override async Task WriteAsync(XmlWriter xmlWriter, IAnalysisCaseDefinition poco, string elementName, bool includeDerivedProperties, bool includesImplied, IXmiElementOriginMap elementOriginMap = null, Uri currentFileUri = null)
        {
            ...
            if (includeDerivedProperties)
            {
                if (poco.calculation != null)
                {
                    foreach (var item in poco.calculation)
                    {
                        await this.XmiDataWriterFacade.WriteReferenceElementAsync(xmlWriter, (IData)item, "calculation", elementOriginMap, currentFileUri);
                    }
                }
            }
            ...
            if (includeDerivedProperties)
            {
                if (poco.resultExpression != null)
                {
                    await this.XmiDataWriterFacade.WriteReferenceElementAsync(xmlWriter, (IData)poco.resultExpression, "resultExpression", elementOriginMap, currentFileUri);
                }
            }
