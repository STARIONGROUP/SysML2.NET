...
namespace SysML2.NET.Serializer.Xmi.Writers
{
    ...
    /// to the XMI document
    /// </summary>
    public static class ViewpointUsageWriter
    {
        /// <summary>
        ...
        public static void Write(XmlWriter xmlWriter, IViewpointUsage poco, string elementName, bool includeDerivedProperties, IXmiDataWriterFacade xmiWriterFacade, IXmiElementOriginMap elementOriginMap = null, Uri currentFileUri = null)
        {
            ...
            if (includeDerivedProperties)
            {
                ...
                {
                    xmiWriterFacade.WriteReferenceElement(xmlWriter, (IData)poco.viewpointDefinition, "viewpointDefinition", elementOriginMap, currentFileUri);
                }
                if (poco.viewpointStakeholder != null)
                {
                    foreach (var item in poco.viewpointStakeholder)
                    {
                        xmiWriterFacade.WriteReferenceElement(xmlWriter, (IData)item, "viewpointStakeholder", elementOriginMap, currentFileUri);
                    }
                }
            }
