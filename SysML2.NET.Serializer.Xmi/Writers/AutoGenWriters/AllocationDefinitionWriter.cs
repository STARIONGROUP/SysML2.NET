...
namespace SysML2.NET.Serializer.Xmi.Writers
{
    ...
    public class AllocationDefinitionWriter : XmiDataWriter<IAllocationDefinition>
    {
        ...
        public override void Write(XmlWriter xmlWriter, IAllocationDefinition poco, string elementName, bool includeDerivedProperties, bool includesImplied, IXmiElementOriginMap elementOriginMap = null, Uri currentFileUri = null)
        {
        ...
        public override async Task WriteAsync(XmlWriter xmlWriter, IAllocationDefinition poco, string elementName, bool includeDerivedProperties, bool includesImplied, IXmiElementOriginMap elementOriginMap = null, Uri currentFileUri = null)
        {
            ...
            if (includeDerivedProperties)
            {
                if (poco.allocation != null)
                {
                    foreach (var item in poco.allocation)
                    {
                        await this.XmiDataWriterFacade.WriteReferenceElementAsync(xmlWriter, (IData)item, "allocation", elementOriginMap, currentFileUri);
                    }
                }
            }
            ...
            if (includeDerivedProperties)
            {
                if (poco.connectionEnd != null)
                {
                    foreach (var item in poco.connectionEnd)
                    {
                        await this.XmiDataWriterFacade.WriteReferenceElementAsync(xmlWriter, (IData)item, "connectionEnd", elementOriginMap, currentFileUri);
                    }
                }
            }
