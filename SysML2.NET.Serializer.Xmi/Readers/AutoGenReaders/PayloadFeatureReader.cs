// -------------------------------------------------------------------------------------------------
// <copyright file="PayloadFeatureReader.cs" company="Starion Group S.A.">
//
...
namespace SysML2.NET.Serializer.Xmi.Readers
{
    ...
    /// <summary>
    /// The purpose of the <see cref="PayloadFeatureReader" /> is to read an instance of <see cref="IPayloadFeature" />
    /// from the XMI document
    /// </summary>
    public class PayloadFeatureReader : XmiDataReader<IPayloadFeature>
    {
        /// <summary>
        /// The instantiated logger from the injected <see cref="ILoggerFactory" /> 
        /// </summary>
        private readonly ILogger<PayloadFeatureReader> logger;

        /// <summary>
        /// Initializes a new instance of the <see cref="PayloadFeatureReader" />
        /// </summary>
        /// <param name="cache">
        ...
        public PayloadFeatureReader(IXmiDataCache cache, IXmiDataReaderFacade xmiDataReaderFacade, IExternalReferenceService externalReferenceService, ILoggerFactory loggerFactory, IXmiElementOriginMap elementOriginMap = null) : base(cache, xmiDataReaderFacade, externalReferenceService, loggerFactory, elementOriginMap)
        {
            this.logger = loggerFactory == null ? NullLogger<PayloadFeatureReader>.Instance : loggerFactory.CreateLogger<PayloadFeatureReader>();
        }

        /// <summary>
        /// Reads the <see cref="IPayloadFeature" /> object from its XML representation
        /// </summary>
        ...
        /// <returns>The read <see cref="IPayloadFeature" /></returns>
        public override IPayloadFeature Read(XmlReader xmiReader, Uri currentLocation)
        {
            if (xmiReader == null)
            {
            ...
            IPayloadFeature poco = new SysML2.NET.Core.POCO.Kernel.Interactions.PayloadFeature();
            ...
            if (xmiReader.MoveToContent() == XmlNodeType.Element)
            {
                ...
                {
                    this.logger.LogCritical("Failed to add element type [{Poco}] with id [{Id}] as it was already in the Cache. The XMI document seems to have duplicate xmi:id values", "PayloadFeature", poco.Id);
                }
                ...
                while (xmiReader.Read())
                {
                    if (xmiReader.NodeType == XmlNodeType.Element)
                    {
                        switch (xmiReader.LocalName)
                        {
                            ...
                            default:
                                this.logger.LogWarning("Unexpected element '{LocalName}' encountered while reading PayloadFeature at line:position {LineNumber}:{LinePosition}", xmiReader.LocalName, xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                                xmiReader.Skip();
                                break;
                        }
                    }
        ...
        /// <summary>
        /// Reads asynchronously the <see cref="IPayloadFeature" /> object from its XML representation
        /// </summary>
        /// <param name="xmiReader">An instance of <see cref="XmlReader" /></param>
        /// <param name="currentLocation">The <see cref="Uri" /> that keep tracks of the current location</param>
        /// <returns>An awaitable <see cref="Task{TResult}"/> with the read <see cref="IPayloadFeature" /></returns>
        public override async Task<IPayloadFeature> ReadAsync(XmlReader xmiReader, Uri currentLocation)
        {
            if (xmiReader == null)
            {
                throw new ArgumentNullException(nameof(xmiReader));
