...
namespace SysML2.NET.Serializer.Xmi.Readers
{
    ...
    /// </summary>
    public class DocumentationReader : XmiDataReader<IDocumentation>
    {
        /// <summary>
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="DocumentationReader" />
        /// </summary>
        /// <param name="cache">
        ...
        public DocumentationReader(IXmiDataCache cache, IXmiDataReaderFacade xmiDataReaderFacade, IExternalReferenceService externalReferenceService, ILoggerFactory loggerFactory, IXmiElementOriginMap elementOriginMap = null) : base(cache, xmiDataReaderFacade, externalReferenceService, loggerFactory, elementOriginMap)
        {
        ...
        /// <returns>The read <see cref="IDocumentation" /></returns>
        public override IDocumentation Read(XmlReader xmiReader, Uri currentLocation)
        {
            if (xmiReader == null)
            {
            ...
            IDocumentation poco = new SysML2.NET.Core.POCO.Root.Annotations.Documentation();
            ...
            if (xmiReader.MoveToContent() == XmlNodeType.Element)
            {
                ...
                if (!string.IsNullOrEmpty(xsiType) && xsiType != "sysml:Documentation")
                {
                    throw new InvalidOperationException($"The xsi:type {xsiType} is not supported by the DocumentationReader");
                }
                ...
                {
                    this.logger.LogCritical("Failed to add element type [{Poco}] with id [{Id}] as it was already in the Cache. The XMI document seems to have duplicate xmi:id values", "Documentation", poco.Id);
                }
                ...
                var localeXmlAttribute = xmiReader.GetAttribute("locale");

                if (!string.IsNullOrWhiteSpace(localeXmlAttribute))
                {
                    poco.Locale = localeXmlAttribute;
                }
                ...
                while (xmiReader.Read())
                {
                    if (xmiReader.NodeType == XmlNodeType.Element)
                    {
                        switch (xmiReader.LocalName)
                        {
                            ...
                            case "locale":
                                {
                                    var localeValue = xmiReader.ReadElementContentAsString();

                                    if (!string.IsNullOrWhiteSpace(localeValue))
                                    {
                                        poco.Locale = localeValue;
                                    }
                            ...
                            default:
                                this.logger.LogWarning("Unexpected element '{LocalName}' encountered while reading Documentation at line:position {LineNumber}:{LinePosition}", xmiReader.LocalName, xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                                xmiReader.Skip();
                                break;
                        }
                    }
        ...
        /// <summary>
        /// Reads asynchronously the <see cref="IDocumentation" /> object from its XML representation
        /// </summary>
        /// <param name="xmiReader">An instance of <see cref="XmlReader" /></param>
        /// <param name="currentLocation">The <see cref="Uri" /> that keep tracks of the current location</param>
        /// <returns>An awaitable <see cref="Task{TResult}"/> with the read <see cref="IDocumentation" /></returns>
        public override async Task<IDocumentation> ReadAsync(XmlReader xmiReader, Uri currentLocation)
        {
            if (xmiReader == null)
            {
                throw new ArgumentNullException(nameof(xmiReader));
                ...
                while (await xmiReader.ReadAsync())
                {
                    if (xmiReader.NodeType == XmlNodeType.Element)
                    {
                        switch (xmiReader.LocalName)
                        {
                            ...
                            case "locale":
                                {
                                    var localeValue = await xmiReader.ReadElementContentAsStringAsync();
