// -------------------------------------------------------------------------------------------------
// <copyright file="FlowDefinitionReader.cs" company="Starion Group S.A.">
//
...
namespace SysML2.NET.Serializer.Xmi.Readers
{
    ...
    public class FlowDefinitionReader : XmiDataReader<IFlowDefinition>
    {
        ...
        /// </summary>
        private readonly ILogger<FlowDefinitionReader> logger;

        /// <summary>
        /// Initializes a new instance of the <see cref="FlowDefinitionReader" />
        /// </summary>
        /// <param name="cache">
        ...
        /// <summary>
        /// Reads the <see cref="IFlowDefinition" /> object from its XML representation
        /// </summary>
        ...
        /// <returns>The read <see cref="IFlowDefinition" /></returns>
        public override IFlowDefinition Read(XmlReader xmiReader, Uri currentLocation)
        {
            if (xmiReader == null)
            {
            ...
            IFlowDefinition poco = new SysML2.NET.Core.POCO.Systems.Flows.FlowDefinition();

            if (xmiReader.MoveToContent() == XmlNodeType.Element)
            {
                this.logger.LogTrace("reading FlowDefinition at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                var xsiType = xmiReader.GetAttribute("xsi:type");

                if (!string.IsNullOrEmpty(xsiType) && xsiType != "sysml:FlowDefinition")
                {
                    throw new InvalidOperationException($"The xsi:type {xsiType} is not supported by the FlowDefinitionReader");
                }
        ...
        /// <summary>
        /// Reads asynchronously the <see cref="IFlowDefinition" /> object from its XML representation
        /// </summary>
        /// <param name="xmiReader">An instance of <see cref="XmlReader" /></param>
        /// <param name="currentLocation">The <see cref="Uri" /> that keep tracks of the current location</param>
        /// <returns>An awaitable <see cref="Task{TResult}"/> with the read <see cref="IFlowDefinition" /></returns>
        public override async Task<IFlowDefinition> ReadAsync(XmlReader xmiReader, Uri currentLocation)
        {
            if (xmiReader == null)
            {
                throw new ArgumentNullException(nameof(xmiReader));
