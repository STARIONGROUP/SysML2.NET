// -------------------------------------------------------------------------------------------------
// <copyright file="IncludeUseCaseUsageReader.cs" company="Starion Group S.A.">
//
...
namespace SysML2.NET.Serializer.Xmi.Readers
{
    ...
    /// <summary>
    /// The purpose of the <see cref="IncludeUseCaseUsageReader" /> is to read an instance of <see cref="IIncludeUseCaseUsage" />
    /// from the XMI document
    /// </summary>
    public class IncludeUseCaseUsageReader : XmiDataReader<IIncludeUseCaseUsage>
    {
        /// <summary>
        /// The instantiated logger from the injected <see cref="ILoggerFactory" /> 
        /// </summary>
        private readonly ILogger<IncludeUseCaseUsageReader> logger;

        /// <summary>
        /// Initializes a new instance of the <see cref="IncludeUseCaseUsageReader" />
        /// </summary>
        /// <param name="cache">
        ...
        public IncludeUseCaseUsageReader(IXmiDataCache cache, IXmiDataReaderFacade xmiDataReaderFacade, IExternalReferenceService externalReferenceService, ILoggerFactory loggerFactory, IXmiElementOriginMap elementOriginMap = null) : base(cache, xmiDataReaderFacade, externalReferenceService, loggerFactory, elementOriginMap)
        {
            this.logger = loggerFactory == null ? NullLogger<IncludeUseCaseUsageReader>.Instance : loggerFactory.CreateLogger<IncludeUseCaseUsageReader>();
        }

        /// <summary>
        /// Reads the <see cref="IIncludeUseCaseUsage" /> object from its XML representation
        /// </summary>
        ...
        /// <returns>The read <see cref="IIncludeUseCaseUsage" /></returns>
        public override IIncludeUseCaseUsage Read(XmlReader xmiReader, Uri currentLocation)
        {
            if (xmiReader == null)
            {
            ...
            IIncludeUseCaseUsage poco = new SysML2.NET.Core.POCO.Systems.UseCases.IncludeUseCaseUsage();

            if (xmiReader.MoveToContent() == XmlNodeType.Element)
            {
                this.logger.LogTrace("reading IncludeUseCaseUsage at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                var xsiType = xmiReader.GetAttribute("xsi:type");

                if (!string.IsNullOrEmpty(xsiType) && xsiType != "sysml:IncludeUseCaseUsage")
                {
                    throw new InvalidOperationException($"The xsi:type {xsiType} is not supported by the IncludeUseCaseUsageReader");
                }
                ...
                {
                    this.logger.LogCritical("Failed to add element type [{Poco}] with id [{Id}] as it was already in the Cache. The XMI document seems to have duplicate xmi:id values", "IncludeUseCaseUsage", poco.Id);
                }
                ...
                while (xmiReader.Read())
                {
                    if (xmiReader.NodeType == XmlNodeType.Element)
                    {
                        switch (xmiReader.LocalName)
                        {
                            ...
                            default:
                                this.logger.LogWarning("Unexpected element '{LocalName}' encountered while reading IncludeUseCaseUsage at line:position {LineNumber}:{LinePosition}", xmiReader.LocalName, xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                                xmiReader.Skip();
                                break;
                        }
                    }
        ...
        /// <summary>
        /// Reads asynchronously the <see cref="IIncludeUseCaseUsage" /> object from its XML representation
        /// </summary>
        /// <param name="xmiReader">An instance of <see cref="XmlReader" /></param>
        /// <param name="currentLocation">The <see cref="Uri" /> that keep tracks of the current location</param>
        /// <returns>An awaitable <see cref="Task{TResult}"/> with the read <see cref="IIncludeUseCaseUsage" /></returns>
        public override async Task<IIncludeUseCaseUsage> ReadAsync(XmlReader xmiReader, Uri currentLocation)
        {
            if (xmiReader == null)
            {
                throw new ArgumentNullException(nameof(xmiReader));
