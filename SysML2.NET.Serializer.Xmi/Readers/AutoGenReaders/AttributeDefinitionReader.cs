...
namespace SysML2.NET.Serializer.Xmi.Readers
{
    ...
    /// <summary>
    /// The purpose of the <see cref="AttributeDefinitionReader" /> is to read an instance of <see cref="IAttributeDefinition" />
    /// from the XMI document
    /// </summary>
    public class AttributeDefinitionReader : XmiDataReader<IAttributeDefinition>
    {
        /// <summary>
        /// The instantiated logger from the injected <see cref="ILoggerFactory" /> 
        /// </summary>
        private readonly ILogger<AttributeDefinitionReader> logger;

        /// <summary>
        /// Initializes a new instance of the <see cref="AttributeDefinitionReader" />
        /// </summary>
        /// <param name="cache">
        ...
        public AttributeDefinitionReader(IXmiDataCache cache, IXmiDataReaderFacade xmiDataReaderFacade, IExternalReferenceService externalReferenceService, ILoggerFactory loggerFactory, IXmiElementOriginMap elementOriginMap = null) : base(cache, xmiDataReaderFacade, externalReferenceService, loggerFactory, elementOriginMap)
        {
            this.logger = loggerFactory == null ? NullLogger<AttributeDefinitionReader>.Instance : loggerFactory.CreateLogger<AttributeDefinitionReader>();
        }

        /// <summary>
        /// Reads the <see cref="IAttributeDefinition" /> object from its XML representation
        /// </summary>
        ...
        /// <returns>The read <see cref="IAttributeDefinition" /></returns>
        public override IAttributeDefinition Read(XmlReader xmiReader, Uri currentLocation)
        {
            if (xmiReader == null)
            {
            ...
            IAttributeDefinition poco = new SysML2.NET.Core.POCO.Systems.Attributes.AttributeDefinition();
            ...
            if (xmiReader.MoveToContent() == XmlNodeType.Element)
            {
                ...
                if (!string.IsNullOrEmpty(xsiType) && xsiType != "sysml:AttributeDefinition")
                {
                    throw new InvalidOperationException($"The xsi:type {xsiType} is not supported by the AttributeDefinitionReader");
                }
                ...
                {
                    this.logger.LogCritical("Failed to add element type [{Poco}] with id [{Id}] as it was already in the Cache. The XMI document seems to have duplicate xmi:id values", "AttributeDefinition", poco.Id);
                }
                ...
                while (xmiReader.Read())
                {
                    if (xmiReader.NodeType == XmlNodeType.Element)
                    {
                        switch (xmiReader.LocalName)
                        {
                            ...
                            default:
                                this.logger.LogWarning("Unexpected element '{LocalName}' encountered while reading AttributeDefinition at line:position {LineNumber}:{LinePosition}", xmiReader.LocalName, xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                                xmiReader.Skip();
                                break;
                        }
                    }
        ...
        /// <summary>
        /// Reads asynchronously the <see cref="IAttributeDefinition" /> object from its XML representation
        /// </summary>
        /// <param name="xmiReader">An instance of <see cref="XmlReader" /></param>
        /// <param name="currentLocation">The <see cref="Uri" /> that keep tracks of the current location</param>
        /// <returns>An awaitable <see cref="Task{TResult}"/> with the read <see cref="IAttributeDefinition" /></returns>
        public override async Task<IAttributeDefinition> ReadAsync(XmlReader xmiReader, Uri currentLocation)
        {
            if (xmiReader == null)
            {
                throw new ArgumentNullException(nameof(xmiReader));
