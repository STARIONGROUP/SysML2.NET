// -------------------------------------------------------------------------------------------------
// <copyright file="OccurrenceDefinitionReader.cs" company="Starion Group S.A.">
//
...
namespace SysML2.NET.Serializer.Xmi.Readers
{
    ...
    /// <summary>
    /// The purpose of the <see cref="OccurrenceDefinitionReader" /> is to read an instance of <see cref="IOccurrenceDefinition" />
    /// from the XMI document
    /// </summary>
    public class OccurrenceDefinitionReader : XmiDataReader<IOccurrenceDefinition>
    {
        /// <summary>
        /// The instantiated logger from the injected <see cref="ILoggerFactory" /> 
        /// </summary>
        private readonly ILogger<OccurrenceDefinitionReader> logger;

        /// <summary>
        /// Initializes a new instance of the <see cref="OccurrenceDefinitionReader" />
        /// </summary>
        /// <param name="cache">
        ...
        public OccurrenceDefinitionReader(IXmiDataCache cache, IXmiDataReaderFacade xmiDataReaderFacade, IExternalReferenceService externalReferenceService, ILoggerFactory loggerFactory, IXmiElementOriginMap elementOriginMap = null) : base(cache, xmiDataReaderFacade, externalReferenceService, loggerFactory, elementOriginMap)
        {
            this.logger = loggerFactory == null ? NullLogger<OccurrenceDefinitionReader>.Instance : loggerFactory.CreateLogger<OccurrenceDefinitionReader>();
        }

        /// <summary>
        /// Reads the <see cref="IOccurrenceDefinition" /> object from its XML representation
        /// </summary>
        ...
        /// <returns>The read <see cref="IOccurrenceDefinition" /></returns>
        public override IOccurrenceDefinition Read(XmlReader xmiReader, Uri currentLocation)
        {
            if (xmiReader == null)
            {
            ...
            IOccurrenceDefinition poco = new SysML2.NET.Core.POCO.Systems.Occurrences.OccurrenceDefinition();
            ...
            if (xmiReader.MoveToContent() == XmlNodeType.Element)
            {
                ...
                {
                    this.logger.LogCritical("Failed to add element type [{Poco}] with id [{Id}] as it was already in the Cache. The XMI document seems to have duplicate xmi:id values", "OccurrenceDefinition", poco.Id);
                }
                ...
                while (xmiReader.Read())
                {
                    if (xmiReader.NodeType == XmlNodeType.Element)
                    {
                        switch (xmiReader.LocalName)
                        {
                            ...
                            default:
                                this.logger.LogWarning("Unexpected element '{LocalName}' encountered while reading OccurrenceDefinition at line:position {LineNumber}:{LinePosition}", xmiReader.LocalName, xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                                xmiReader.Skip();
                                break;
                        }
                    }
        ...
        /// <summary>
        /// Reads asynchronously the <see cref="IOccurrenceDefinition" /> object from its XML representation
        /// </summary>
        /// <param name="xmiReader">An instance of <see cref="XmlReader" /></param>
        /// <param name="currentLocation">The <see cref="Uri" /> that keep tracks of the current location</param>
        /// <returns>An awaitable <see cref="Task{TResult}"/> with the read <see cref="IOccurrenceDefinition" /></returns>
        public override async Task<IOccurrenceDefinition> ReadAsync(XmlReader xmiReader, Uri currentLocation)
        {
            if (xmiReader == null)
            {
                throw new ArgumentNullException(nameof(xmiReader));
