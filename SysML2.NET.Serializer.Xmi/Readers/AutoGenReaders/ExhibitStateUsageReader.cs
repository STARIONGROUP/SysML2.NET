// -------------------------------------------------------------------------------------------------
// <copyright file="ExhibitStateUsageReader.cs" company="Starion Group S.A.">
//
...
namespace SysML2.NET.Serializer.Xmi.Readers
{
    ...
    /// <summary>
    /// The purpose of the <see cref="ExhibitStateUsageReader" /> is to read an instance of <see cref="IExhibitStateUsage" />
    /// from the XMI document
    /// </summary>
    public class ExhibitStateUsageReader : XmiDataReader<IExhibitStateUsage>
    {
        /// <summary>
        /// The instantiated logger from the injected <see cref="ILoggerFactory" /> 
        /// </summary>
        private readonly ILogger<ExhibitStateUsageReader> logger;

        /// <summary>
        /// Initializes a new instance of the <see cref="ExhibitStateUsageReader" />
        /// </summary>
        /// <param name="cache">
        ...
        public ExhibitStateUsageReader(IXmiDataCache cache, IXmiDataReaderFacade xmiDataReaderFacade, IExternalReferenceService externalReferenceService, ILoggerFactory loggerFactory, IXmiElementOriginMap elementOriginMap = null) : base(cache, xmiDataReaderFacade, externalReferenceService, loggerFactory, elementOriginMap)
        {
            this.logger = loggerFactory == null ? NullLogger<ExhibitStateUsageReader>.Instance : loggerFactory.CreateLogger<ExhibitStateUsageReader>();
        }

        /// <summary>
        /// Reads the <see cref="IExhibitStateUsage" /> object from its XML representation
        /// </summary>
        ...
        /// <returns>The read <see cref="IExhibitStateUsage" /></returns>
        public override IExhibitStateUsage Read(XmlReader xmiReader, Uri currentLocation)
        {
            if (xmiReader == null)
            {
            ...
            IExhibitStateUsage poco = new SysML2.NET.Core.POCO.Systems.States.ExhibitStateUsage();

            if (xmiReader.MoveToContent() == XmlNodeType.Element)
            {
                this.logger.LogTrace("reading ExhibitStateUsage at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                var xsiType = xmiReader.GetAttribute("xsi:type");

                if (!string.IsNullOrEmpty(xsiType) && xsiType != "sysml:ExhibitStateUsage")
                {
                    throw new InvalidOperationException($"The xsi:type {xsiType} is not supported by the ExhibitStateUsageReader");
                }
                ...
                {
                    this.logger.LogCritical("Failed to add element type [{Poco}] with id [{Id}] as it was already in the Cache. The XMI document seems to have duplicate xmi:id values", "ExhibitStateUsage", poco.Id);
                }
                ...
                while (xmiReader.Read())
                {
                    if (xmiReader.NodeType == XmlNodeType.Element)
                    {
                        switch (xmiReader.LocalName)
                        {
                            ...
                            case "isParallel":
                                {
                                    ...
                                    {
                                        if (bool.TryParse(isParallelValue, out var isParallelValueAsBool))
                                        {
                                            poco.IsParallel = isParallelValueAsBool;
                                        }
                                        else
                                        {
                                            this.logger.LogWarning("Failed to parse bool value '{Value}' for property 'isParallel' on element {ElementId}", isParallelValue, poco.Id);
                                        }
                                    }
                            ...
                            default:
                                this.logger.LogWarning("Unexpected element '{LocalName}' encountered while reading ExhibitStateUsage at line:position {LineNumber}:{LinePosition}", xmiReader.LocalName, xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                                xmiReader.Skip();
                                break;
                        }
                    }
        ...
        /// <summary>
        /// Reads asynchronously the <see cref="IExhibitStateUsage" /> object from its XML representation
        /// </summary>
        /// <param name="xmiReader">An instance of <see cref="XmlReader" /></param>
        /// <param name="currentLocation">The <see cref="Uri" /> that keep tracks of the current location</param>
        /// <returns>An awaitable <see cref="Task{TResult}"/> with the read <see cref="IExhibitStateUsage" /></returns>
        public override async Task<IExhibitStateUsage> ReadAsync(XmlReader xmiReader, Uri currentLocation)
        {
            if (xmiReader == null)
            {
                throw new ArgumentNullException(nameof(xmiReader));
