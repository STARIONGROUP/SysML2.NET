// -------------------------------------------------------------------------------------------------
// <copyright file="NamespaceExposeReader.cs" company="Starion Group S.A.">
//
...
namespace SysML2.NET.Serializer.Xmi.Readers
{
    ...
    /// <summary>
    /// The purpose of the <see cref="NamespaceExposeReader" /> is to read an instance of <see cref="INamespaceExpose" />
    /// from the XMI document
    /// </summary>
    public class NamespaceExposeReader : XmiDataReader<INamespaceExpose>
    {
        /// <summary>
        /// The instantiated logger from the injected <see cref="ILoggerFactory" /> 
        /// </summary>
        private readonly ILogger<NamespaceExposeReader> logger;

        /// <summary>
        /// Initializes a new instance of the <see cref="NamespaceExposeReader" />
        /// </summary>
        /// <param name="cache">
        ...
        public NamespaceExposeReader(IXmiDataCache cache, IXmiDataReaderFacade xmiDataReaderFacade, IExternalReferenceService externalReferenceService, ILoggerFactory loggerFactory, IXmiElementOriginMap elementOriginMap = null) : base(cache, xmiDataReaderFacade, externalReferenceService, loggerFactory, elementOriginMap)
        {
            this.logger = loggerFactory == null ? NullLogger<NamespaceExposeReader>.Instance : loggerFactory.CreateLogger<NamespaceExposeReader>();
        }

        /// <summary>
        /// Reads the <see cref="INamespaceExpose" /> object from its XML representation
        /// </summary>
        ...
        /// <returns>The read <see cref="INamespaceExpose" /></returns>
        public override INamespaceExpose Read(XmlReader xmiReader, Uri currentLocation)
        {
            if (xmiReader == null)
            {
                ...
                while (xmiReader.Read())
                {
                    if (xmiReader.NodeType == XmlNodeType.Element)
                    {
                        switch (xmiReader.LocalName)
                        {
                            ...
                            default:
                                this.logger.LogWarning("Unexpected element '{LocalName}' encountered while reading NamespaceExpose at line:position {LineNumber}:{LinePosition}", xmiReader.LocalName, xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                                xmiReader.Skip();
                                break;
                        }
                    }
