// -------------------------------------------------------------------------------------------------
// <copyright file="TransitionFeatureMembershipReader.cs" company="Starion Group S.A.">
//
...
namespace SysML2.NET.Serializer.Xmi.Readers
{
    ...
    /// <summary>
    /// The purpose of the <see cref="TransitionFeatureMembershipReader" /> is to read an instance of <see cref="ITransitionFeatureMembership" />
    /// from the XMI document
    /// </summary>
    public class TransitionFeatureMembershipReader : XmiDataReader<ITransitionFeatureMembership>
    {
        /// <summary>
        /// The instantiated logger from the injected <see cref="ILoggerFactory" /> 
        /// </summary>
        private readonly ILogger<TransitionFeatureMembershipReader> logger;

        /// <summary>
        /// Initializes a new instance of the <see cref="TransitionFeatureMembershipReader" />
        /// </summary>
        /// <param name="cache">
        ...
        public TransitionFeatureMembershipReader(IXmiDataCache cache, IXmiDataReaderFacade xmiDataReaderFacade, IExternalReferenceService externalReferenceService, ILoggerFactory loggerFactory, IXmiElementOriginMap elementOriginMap = null) : base(cache, xmiDataReaderFacade, externalReferenceService, loggerFactory, elementOriginMap)
        {
            this.logger = loggerFactory == null ? NullLogger<TransitionFeatureMembershipReader>.Instance : loggerFactory.CreateLogger<TransitionFeatureMembershipReader>();
        }

        /// <summary>
        /// Reads the <see cref="ITransitionFeatureMembership" /> object from its XML representation
        /// </summary>
        ...
        /// <returns>The read <see cref="ITransitionFeatureMembership" /></returns>
        public override ITransitionFeatureMembership Read(XmlReader xmiReader, Uri currentLocation)
        {
            if (xmiReader == null)
            {
            ...
            ITransitionFeatureMembership poco = new SysML2.NET.Core.POCO.Systems.States.TransitionFeatureMembership();

            if (xmiReader.MoveToContent() == XmlNodeType.Element)
            {
                this.logger.LogTrace("reading TransitionFeatureMembership at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                var xsiType = xmiReader.GetAttribute("xsi:type");

                if (!string.IsNullOrEmpty(xsiType) && xsiType != "sysml:TransitionFeatureMembership")
                {
                    throw new InvalidOperationException($"The xsi:type {xsiType} is not supported by the TransitionFeatureMembershipReader");
                }
                ...
                {
                    this.logger.LogCritical("Failed to add element type [{Poco}] with id [{Id}] as it was already in the Cache. The XMI document seems to have duplicate xmi:id values", "TransitionFeatureMembership", poco.Id);
                }
                ...
                {
                    if (TransitionFeatureKindProvider.TryParse(kindXmlAttribute.AsSpan(), out var kindXmlAttributeAsEnum))
                    {
                ...
                while (xmiReader.Read())
                {
                    if (xmiReader.NodeType == XmlNodeType.Element)
                    {
                        switch (xmiReader.LocalName)
                        {
                            ...
                            case "kind":
                                {
                                    ...
                                    {
                                        if (TransitionFeatureKindProvider.TryParse(kindValue.AsSpan(), out var kindValueAsEnum))
                                        {
                            ...
                            default:
                                this.logger.LogWarning("Unexpected element '{LocalName}' encountered while reading TransitionFeatureMembership at line:position {LineNumber}:{LinePosition}", xmiReader.LocalName, xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                                xmiReader.Skip();
                                break;
                        }
                    }
        ...
        /// <summary>
        /// Reads asynchronously the <see cref="ITransitionFeatureMembership" /> object from its XML representation
        /// </summary>
        /// <param name="xmiReader">An instance of <see cref="XmlReader" /></param>
        /// <param name="currentLocation">The <see cref="Uri" /> that keep tracks of the current location</param>
        /// <returns>An awaitable <see cref="Task{TResult}"/> with the read <see cref="ITransitionFeatureMembership" /></returns>
        public override async Task<ITransitionFeatureMembership> ReadAsync(XmlReader xmiReader, Uri currentLocation)
        {
            if (xmiReader == null)
            {
                throw new ArgumentNullException(nameof(xmiReader));
