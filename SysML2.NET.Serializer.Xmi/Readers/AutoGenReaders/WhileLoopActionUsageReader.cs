// -------------------------------------------------------------------------------------------------
// <copyright file="WhileLoopActionUsageReader.cs" company="Starion Group S.A.">
//
...
namespace SysML2.NET.Serializer.Xmi.Readers
{
    ...
    /// <summary>
    /// The purpose of the <see cref="WhileLoopActionUsageReader" /> is to read an instance of <see cref="IWhileLoopActionUsage" />
    /// from the XMI document
    /// </summary>
    public class WhileLoopActionUsageReader : XmiDataReader<IWhileLoopActionUsage>
    {
        /// <summary>
        /// The instantiated logger from the injected <see cref="ILoggerFactory" /> 
        /// </summary>
        private readonly ILogger<WhileLoopActionUsageReader> logger;

        /// <summary>
        /// Initializes a new instance of the <see cref="WhileLoopActionUsageReader" />
        /// </summary>
        /// <param name="cache">
        ...
        public WhileLoopActionUsageReader(IXmiDataCache cache, IXmiDataReaderFacade xmiDataReaderFacade, IExternalReferenceService externalReferenceService, ILoggerFactory loggerFactory, IXmiElementOriginMap elementOriginMap = null) : base(cache, xmiDataReaderFacade, externalReferenceService, loggerFactory, elementOriginMap)
        {
            this.logger = loggerFactory == null ? NullLogger<WhileLoopActionUsageReader>.Instance : loggerFactory.CreateLogger<WhileLoopActionUsageReader>();
        }

        /// <summary>
        /// Reads the <see cref="IWhileLoopActionUsage" /> object from its XML representation
        /// </summary>
        ...
        /// <returns>The read <see cref="IWhileLoopActionUsage" /></returns>
        public override IWhileLoopActionUsage Read(XmlReader xmiReader, Uri currentLocation)
        {
            if (xmiReader == null)
            {
            ...
            IWhileLoopActionUsage poco = new SysML2.NET.Core.POCO.Systems.Actions.WhileLoopActionUsage();

            if (xmiReader.MoveToContent() == XmlNodeType.Element)
            {
                this.logger.LogTrace("reading WhileLoopActionUsage at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                var xsiType = xmiReader.GetAttribute("xsi:type");

                if (!string.IsNullOrEmpty(xsiType) && xsiType != "sysml:WhileLoopActionUsage")
                {
                    throw new InvalidOperationException($"The xsi:type {xsiType} is not supported by the WhileLoopActionUsageReader");
                }
                ...
                {
                    this.logger.LogCritical("Failed to add element type [{Poco}] with id [{Id}] as it was already in the Cache. The XMI document seems to have duplicate xmi:id values", "WhileLoopActionUsage", poco.Id);
                }
                ...
                while (xmiReader.Read())
                {
                    if (xmiReader.NodeType == XmlNodeType.Element)
                    {
                        switch (xmiReader.LocalName)
                        {
                            ...
                            default:
                                this.logger.LogWarning("Unexpected element '{LocalName}' encountered while reading WhileLoopActionUsage at line:position {LineNumber}:{LinePosition}", xmiReader.LocalName, xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                                xmiReader.Skip();
                                break;
                        }
                    }
        ...
        /// <summary>
        /// Reads asynchronously the <see cref="IWhileLoopActionUsage" /> object from its XML representation
        /// </summary>
        /// <param name="xmiReader">An instance of <see cref="XmlReader" /></param>
        /// <param name="currentLocation">The <see cref="Uri" /> that keep tracks of the current location</param>
        /// <returns>An awaitable <see cref="Task{TResult}"/> with the read <see cref="IWhileLoopActionUsage" /></returns>
        public override async Task<IWhileLoopActionUsage> ReadAsync(XmlReader xmiReader, Uri currentLocation)
        {
            if (xmiReader == null)
            {
                throw new ArgumentNullException(nameof(xmiReader));
