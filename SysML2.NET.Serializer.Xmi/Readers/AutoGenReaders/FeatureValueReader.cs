// -------------------------------------------------------------------------------------------------
// <copyright file="FeatureValueReader.cs" company="Starion Group S.A.">
//
...
namespace SysML2.NET.Serializer.Xmi.Readers
{
    ...
    /// <summary>
    /// The purpose of the <see cref="FeatureValueReader" /> is to read an instance of <see cref="IFeatureValue" />
    /// from the XMI document
    /// </summary>
    public class FeatureValueReader : XmiDataReader<IFeatureValue>
    {
        /// <summary>
        /// The instantiated logger from the injected <see cref="ILoggerFactory" /> 
        /// </summary>
        private readonly ILogger<FeatureValueReader> logger;

        /// <summary>
        /// Initializes a new instance of the <see cref="FeatureValueReader" />
        /// </summary>
        /// <param name="cache">
        ...
        public FeatureValueReader(IXmiDataCache cache, IXmiDataReaderFacade xmiDataReaderFacade, IExternalReferenceService externalReferenceService, ILoggerFactory loggerFactory, IXmiElementOriginMap elementOriginMap = null) : base(cache, xmiDataReaderFacade, externalReferenceService, loggerFactory, elementOriginMap)
        {
            this.logger = loggerFactory == null ? NullLogger<FeatureValueReader>.Instance : loggerFactory.CreateLogger<FeatureValueReader>();
        }

        /// <summary>
        /// Reads the <see cref="IFeatureValue" /> object from its XML representation
        /// </summary>
        ...
        /// <returns>The read <see cref="IFeatureValue" /></returns>
        public override IFeatureValue Read(XmlReader xmiReader, Uri currentLocation)
        {
            if (xmiReader == null)
            {
            ...
            IFeatureValue poco = new SysML2.NET.Core.POCO.Kernel.FeatureValues.FeatureValue();

            if (xmiReader.MoveToContent() == XmlNodeType.Element)
            {
                this.logger.LogTrace("reading FeatureValue at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                var xsiType = xmiReader.GetAttribute("xsi:type");

                if (!string.IsNullOrEmpty(xsiType) && xsiType != "sysml:FeatureValue")
                {
                    throw new InvalidOperationException($"The xsi:type {xsiType} is not supported by the FeatureValueReader");
                }
                ...
                {
                    this.logger.LogCritical("Failed to add element type [{Poco}] with id [{Id}] as it was already in the Cache. The XMI document seems to have duplicate xmi:id values", "FeatureValue", poco.Id);
                }
                ...
                var isDefaultXmlAttribute = xmiReader.GetAttribute("isDefault");

                if (!string.IsNullOrWhiteSpace(isDefaultXmlAttribute))
                {
                    if (bool.TryParse(isDefaultXmlAttribute, out var isDefaultXmlAttributeAsBool))
                    {
                        poco.IsDefault = isDefaultXmlAttributeAsBool;
                    }
                }
                ...
                var isInitialXmlAttribute = xmiReader.GetAttribute("isInitial");

                if (!string.IsNullOrWhiteSpace(isInitialXmlAttribute))
                {
                    if (bool.TryParse(isInitialXmlAttribute, out var isInitialXmlAttributeAsBool))
                    {
                        poco.IsInitial = isInitialXmlAttributeAsBool;
                    }
                }
                ...
                while (xmiReader.Read())
                {
                    if (xmiReader.NodeType == XmlNodeType.Element)
                    {
                        switch (xmiReader.LocalName)
                        {
                            ...
                            case "isDefault":
                                {
                                    var isDefaultValue = xmiReader.ReadElementContentAsString();

                                    if (!string.IsNullOrWhiteSpace(isDefaultValue))
                                    {
                                        if (bool.TryParse(isDefaultValue, out var isDefaultValueAsBool))
                                        {
                                            poco.IsDefault = isDefaultValueAsBool;
                                        }
                                        else
                                        {
                                            this.logger.LogWarning("Failed to parse bool value '{Value}' for property 'isDefault' on element {ElementId}", isDefaultValue, poco.Id);
                                        }
                                    }
                            ...
                            case "isInitial":
                                {
                                    var isInitialValue = xmiReader.ReadElementContentAsString();

                                    if (!string.IsNullOrWhiteSpace(isInitialValue))
                                    {
                                        if (bool.TryParse(isInitialValue, out var isInitialValueAsBool))
                                        {
                                            poco.IsInitial = isInitialValueAsBool;
                                        }
                                        else
                                        {
                                            this.logger.LogWarning("Failed to parse bool value '{Value}' for property 'isInitial' on element {ElementId}", isInitialValue, poco.Id);
                                        }
                                    }
                            ...
                            default:
                                this.logger.LogWarning("Unexpected element '{LocalName}' encountered while reading FeatureValue at line:position {LineNumber}:{LinePosition}", xmiReader.LocalName, xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                                xmiReader.Skip();
                                break;
                        }
                    }
        ...
        /// <summary>
        /// Reads asynchronously the <see cref="IFeatureValue" /> object from its XML representation
        /// </summary>
        /// <param name="xmiReader">An instance of <see cref="XmlReader" /></param>
        /// <param name="currentLocation">The <see cref="Uri" /> that keep tracks of the current location</param>
        /// <returns>An awaitable <see cref="Task{TResult}"/> with the read <see cref="IFeatureValue" /></returns>
        public override async Task<IFeatureValue> ReadAsync(XmlReader xmiReader, Uri currentLocation)
        {
            if (xmiReader == null)
            {
                throw new ArgumentNullException(nameof(xmiReader));
                ...
                while (await xmiReader.ReadAsync())
                {
                    if (xmiReader.NodeType == XmlNodeType.Element)
                    {
                        switch (xmiReader.LocalName)
                        {
                            ...
                            case "isDefault":
                                {
                                    var isDefaultValue = await xmiReader.ReadElementContentAsStringAsync();
                            ...
                            case "isInitial":
                                {
                                    var isInitialValue = await xmiReader.ReadElementContentAsStringAsync();
