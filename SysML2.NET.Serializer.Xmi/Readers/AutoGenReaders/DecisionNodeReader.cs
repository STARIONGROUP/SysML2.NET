...
namespace SysML2.NET.Serializer.Xmi.Readers
{
    ...
    /// </summary>
    public class DecisionNodeReader : XmiDataReader<IDecisionNode>
    {
        /// <summary>
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="DecisionNodeReader" />
        /// </summary>
        /// <param name="cache">
        ...
        public DecisionNodeReader(IXmiDataCache cache, IXmiDataReaderFacade xmiDataReaderFacade, IExternalReferenceService externalReferenceService, ILoggerFactory loggerFactory, IXmiElementOriginMap elementOriginMap = null) : base(cache, xmiDataReaderFacade, externalReferenceService, loggerFactory, elementOriginMap)
        {
        ...
        /// <returns>The read <see cref="IDecisionNode" /></returns>
        public override IDecisionNode Read(XmlReader xmiReader, Uri currentLocation)
        {
            if (xmiReader == null)
            {
            ...
            IDecisionNode poco = new SysML2.NET.Core.POCO.Systems.Actions.DecisionNode();
            ...
            if (xmiReader.MoveToContent() == XmlNodeType.Element)
            {
                ...
                if (!string.IsNullOrEmpty(xsiType) && xsiType != "sysml:DecisionNode")
                {
                    throw new InvalidOperationException($"The xsi:type {xsiType} is not supported by the DecisionNodeReader");
                }
                ...
                {
                    this.logger.LogCritical("Failed to add element type [{Poco}] with id [{Id}] as it was already in the Cache. The XMI document seems to have duplicate xmi:id values", "DecisionNode", poco.Id);
                }
                ...
                while (xmiReader.Read())
                {
                    if (xmiReader.NodeType == XmlNodeType.Element)
                    {
                        switch (xmiReader.LocalName)
                        {
                            ...
                            default:
                                this.logger.LogWarning("Unexpected element '{LocalName}' encountered while reading DecisionNode at line:position {LineNumber}:{LinePosition}", xmiReader.LocalName, xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                                xmiReader.Skip();
                                break;
                        }
                    }
        ...
        /// <summary>
        /// Reads asynchronously the <see cref="IDecisionNode" /> object from its XML representation
        /// </summary>
        /// <param name="xmiReader">An instance of <see cref="XmlReader" /></param>
        /// <param name="currentLocation">The <see cref="Uri" /> that keep tracks of the current location</param>
        /// <returns>An awaitable <see cref="Task{TResult}"/> with the read <see cref="IDecisionNode" /></returns>
        public override async Task<IDecisionNode> ReadAsync(XmlReader xmiReader, Uri currentLocation)
        {
            if (xmiReader == null)
            {
                throw new ArgumentNullException(nameof(xmiReader));
