// -------------------------------------------------------------------------------------------------
// <copyright file="SubjectMembershipReader.cs" company="Starion Group S.A.">
//
...
namespace SysML2.NET.Serializer.Xmi.Readers
{
    ...
    /// <summary>
    /// The purpose of the <see cref="SubjectMembershipReader" /> is to read an instance of <see cref="ISubjectMembership" />
    /// from the XMI document
    /// </summary>
    public class SubjectMembershipReader : XmiDataReader<ISubjectMembership>
    {
        /// <summary>
        /// The instantiated logger from the injected <see cref="ILoggerFactory" /> 
        /// </summary>
        private readonly ILogger<SubjectMembershipReader> logger;

        /// <summary>
        /// Initializes a new instance of the <see cref="SubjectMembershipReader" />
        /// </summary>
        /// <param name="cache">
        ...
        public SubjectMembershipReader(IXmiDataCache cache, IXmiDataReaderFacade xmiDataReaderFacade, IExternalReferenceService externalReferenceService, ILoggerFactory loggerFactory, IXmiElementOriginMap elementOriginMap = null) : base(cache, xmiDataReaderFacade, externalReferenceService, loggerFactory, elementOriginMap)
        {
            this.logger = loggerFactory == null ? NullLogger<SubjectMembershipReader>.Instance : loggerFactory.CreateLogger<SubjectMembershipReader>();
        }

        /// <summary>
        /// Reads the <see cref="ISubjectMembership" /> object from its XML representation
        /// </summary>
        ...
        /// <returns>The read <see cref="ISubjectMembership" /></returns>
        public override ISubjectMembership Read(XmlReader xmiReader, Uri currentLocation)
        {
            if (xmiReader == null)
            {
            ...
            ISubjectMembership poco = new SysML2.NET.Core.POCO.Systems.Requirements.SubjectMembership();

            if (xmiReader.MoveToContent() == XmlNodeType.Element)
            {
                this.logger.LogTrace("reading SubjectMembership at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                var xsiType = xmiReader.GetAttribute("xsi:type");

                if (!string.IsNullOrEmpty(xsiType) && xsiType != "sysml:SubjectMembership")
                {
                    throw new InvalidOperationException($"The xsi:type {xsiType} is not supported by the SubjectMembershipReader");
                }
                ...
                {
                    this.logger.LogCritical("Failed to add element type [{Poco}] with id [{Id}] as it was already in the Cache. The XMI document seems to have duplicate xmi:id values", "SubjectMembership", poco.Id);
                }
                ...
                while (xmiReader.Read())
                {
                    if (xmiReader.NodeType == XmlNodeType.Element)
                    {
                        switch (xmiReader.LocalName)
                        {
                            ...
                            default:
                                this.logger.LogWarning("Unexpected element '{LocalName}' encountered while reading SubjectMembership at line:position {LineNumber}:{LinePosition}", xmiReader.LocalName, xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                                xmiReader.Skip();
                                break;
                        }
                    }
        ...
        /// <summary>
        /// Reads asynchronously the <see cref="ISubjectMembership" /> object from its XML representation
        /// </summary>
        /// <param name="xmiReader">An instance of <see cref="XmlReader" /></param>
        /// <param name="currentLocation">The <see cref="Uri" /> that keep tracks of the current location</param>
        /// <returns>An awaitable <see cref="Task{TResult}"/> with the read <see cref="ISubjectMembership" /></returns>
        public override async Task<ISubjectMembership> ReadAsync(XmlReader xmiReader, Uri currentLocation)
        {
            if (xmiReader == null)
            {
                throw new ArgumentNullException(nameof(xmiReader));
