// -------------------------------------------------------------------------------------------------
// <copyright file="LiteralRationalReader.cs" company="Starion Group S.A.">
//
...
namespace SysML2.NET.Serializer.Xmi.Readers
{
    ...
    /// <summary>
    /// The purpose of the <see cref="LiteralRationalReader" /> is to read an instance of <see cref="ILiteralRational" />
    /// from the XMI document
    /// </summary>
    public class LiteralRationalReader : XmiDataReader<ILiteralRational>
    {
        /// <summary>
        /// The instantiated logger from the injected <see cref="ILoggerFactory" /> 
        /// </summary>
        private readonly ILogger<LiteralRationalReader> logger;

        /// <summary>
        /// Initializes a new instance of the <see cref="LiteralRationalReader" />
        /// </summary>
        /// <param name="cache">
        ...
        public LiteralRationalReader(IXmiDataCache cache, IXmiDataReaderFacade xmiDataReaderFacade, IExternalReferenceService externalReferenceService, ILoggerFactory loggerFactory, IXmiElementOriginMap elementOriginMap = null) : base(cache, xmiDataReaderFacade, externalReferenceService, loggerFactory, elementOriginMap)
        {
            this.logger = loggerFactory == null ? NullLogger<LiteralRationalReader>.Instance : loggerFactory.CreateLogger<LiteralRationalReader>();
        }

        /// <summary>
        /// Reads the <see cref="ILiteralRational" /> object from its XML representation
        /// </summary>
        ...
        /// <returns>The read <see cref="ILiteralRational" /></returns>
        public override ILiteralRational Read(XmlReader xmiReader, Uri currentLocation)
        {
            if (xmiReader == null)
            {
            ...
            ILiteralRational poco = new SysML2.NET.Core.POCO.Kernel.Expressions.LiteralRational();
            ...
            if (xmiReader.MoveToContent() == XmlNodeType.Element)
            {
                ...
                {
                    this.logger.LogCritical("Failed to add element type [{Poco}] with id [{Id}] as it was already in the Cache. The XMI document seems to have duplicate xmi:id values", "LiteralRational", poco.Id);
                }
                ...
                var valueXmlAttribute = xmiReader.GetAttribute("value");
                ...
                while (xmiReader.Read())
                {
                    if (xmiReader.NodeType == XmlNodeType.Element)
                    {
                        switch (xmiReader.LocalName)
                        {
                            ...
                            case "value":
                                {
                                    var valueValue = xmiReader.ReadElementContentAsString();
                                    ...
                                    if (!string.IsNullOrWhiteSpace(valueValue))
                                    {
                                        ...
                                        {
                                            this.logger.LogWarning("Failed to parse double value '{Value}' for property 'value' on element {ElementId}", valueValue, poco.Id);
                                        }
                                    }
                            ...
                            default:
                                this.logger.LogWarning("Unexpected element '{LocalName}' encountered while reading LiteralRational at line:position {LineNumber}:{LinePosition}", xmiReader.LocalName, xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                                xmiReader.Skip();
                                break;
                        }
                    }
        ...
        /// <summary>
        /// Reads asynchronously the <see cref="ILiteralRational" /> object from its XML representation
        /// </summary>
        /// <param name="xmiReader">An instance of <see cref="XmlReader" /></param>
        /// <param name="currentLocation">The <see cref="Uri" /> that keep tracks of the current location</param>
        /// <returns>An awaitable <see cref="Task{TResult}"/> with the read <see cref="ILiteralRational" /></returns>
        public override async Task<ILiteralRational> ReadAsync(XmlReader xmiReader, Uri currentLocation)
        {
            if (xmiReader == null)
            {
                throw new ArgumentNullException(nameof(xmiReader));
                ...
                while (await xmiReader.ReadAsync())
                {
                    if (xmiReader.NodeType == XmlNodeType.Element)
                    {
                        switch (xmiReader.LocalName)
                        {
                            ...
                            case "value":
                                {
                                    var valueValue = await xmiReader.ReadElementContentAsStringAsync();
