...
namespace SysML2.NET.Serializer.Xmi.Readers
{
    ...
    /// </summary>
    public class LiteralStringReader : XmiDataReader<ILiteralString>
    {
        /// <summary>
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="LiteralStringReader" />
        /// </summary>
        /// <param name="cache">
        ...
        public LiteralStringReader(IXmiDataCache cache, IXmiDataReaderFacade xmiDataReaderFacade, IExternalReferenceService externalReferenceService, ILoggerFactory loggerFactory, IXmiElementOriginMap elementOriginMap = null) : base(cache, xmiDataReaderFacade, externalReferenceService, loggerFactory, elementOriginMap)
        {
        ...
        /// <returns>The read <see cref="ILiteralString" /></returns>
        public override ILiteralString Read(XmlReader xmiReader, Uri currentLocation)
        {
            if (xmiReader == null)
            {
            ...
            ILiteralString poco = new SysML2.NET.Core.POCO.Kernel.Expressions.LiteralString();
            ...
            if (xmiReader.MoveToContent() == XmlNodeType.Element)
            {
                ...
                if (!string.IsNullOrEmpty(xsiType) && xsiType != "sysml:LiteralString")
                {
                    throw new InvalidOperationException($"The xsi:type {xsiType} is not supported by the LiteralStringReader");
                }
                ...
                {
                    this.logger.LogCritical("Failed to add element type [{Poco}] with id [{Id}] as it was already in the Cache. The XMI document seems to have duplicate xmi:id values", "LiteralString", poco.Id);
                }
                ...
                while (xmiReader.Read())
                {
                    if (xmiReader.NodeType == XmlNodeType.Element)
                    {
                        switch (xmiReader.LocalName)
                        {
                            ...
                            default:
                                this.logger.LogWarning("Unexpected element '{LocalName}' encountered while reading LiteralString at line:position {LineNumber}:{LinePosition}", xmiReader.LocalName, xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                                xmiReader.Skip();
                                break;
                        }
                    }
        ...
        /// <summary>
        /// Reads asynchronously the <see cref="ILiteralString" /> object from its XML representation
        /// </summary>
        /// <param name="xmiReader">An instance of <see cref="XmlReader" /></param>
        /// <param name="currentLocation">The <see cref="Uri" /> that keep tracks of the current location</param>
        /// <returns>An awaitable <see cref="Task{TResult}"/> with the read <see cref="ILiteralString" /></returns>
        public override async Task<ILiteralString> ReadAsync(XmlReader xmiReader, Uri currentLocation)
        {
            if (xmiReader == null)
            {
                throw new ArgumentNullException(nameof(xmiReader));
