// -------------------------------------------------------------------------------------------------
// <copyright file="VerificationCaseDefinitionReader.cs" company="Starion Group S.A.">
//
...
namespace SysML2.NET.Serializer.Xmi.Readers
{
    ...
    /// <summary>
    /// The purpose of the <see cref="VerificationCaseDefinitionReader" /> is to read an instance of <see cref="IVerificationCaseDefinition" />
    /// from the XMI document
    /// </summary>
    public class VerificationCaseDefinitionReader : XmiDataReader<IVerificationCaseDefinition>
    {
        /// <summary>
        /// The instantiated logger from the injected <see cref="ILoggerFactory" /> 
        /// </summary>
        private readonly ILogger<VerificationCaseDefinitionReader> logger;

        /// <summary>
        /// Initializes a new instance of the <see cref="VerificationCaseDefinitionReader" />
        /// </summary>
        /// <param name="cache">
        ...
        public VerificationCaseDefinitionReader(IXmiDataCache cache, IXmiDataReaderFacade xmiDataReaderFacade, IExternalReferenceService externalReferenceService, ILoggerFactory loggerFactory, IXmiElementOriginMap elementOriginMap = null) : base(cache, xmiDataReaderFacade, externalReferenceService, loggerFactory, elementOriginMap)
        {
            this.logger = loggerFactory == null ? NullLogger<VerificationCaseDefinitionReader>.Instance : loggerFactory.CreateLogger<VerificationCaseDefinitionReader>();
        }

        /// <summary>
        /// Reads the <see cref="IVerificationCaseDefinition" /> object from its XML representation
        /// </summary>
        ...
        /// <returns>The read <see cref="IVerificationCaseDefinition" /></returns>
        public override IVerificationCaseDefinition Read(XmlReader xmiReader, Uri currentLocation)
        {
            if (xmiReader == null)
            {
            ...
            IVerificationCaseDefinition poco = new SysML2.NET.Core.POCO.Systems.VerificationCases.VerificationCaseDefinition();

            if (xmiReader.MoveToContent() == XmlNodeType.Element)
            {
                this.logger.LogTrace("reading VerificationCaseDefinition at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                var xsiType = xmiReader.GetAttribute("xsi:type");

                if (!string.IsNullOrEmpty(xsiType) && xsiType != "sysml:VerificationCaseDefinition")
                {
                    throw new InvalidOperationException($"The xsi:type {xsiType} is not supported by the VerificationCaseDefinitionReader");
                }
                ...
                {
                    this.logger.LogCritical("Failed to add element type [{Poco}] with id [{Id}] as it was already in the Cache. The XMI document seems to have duplicate xmi:id values", "VerificationCaseDefinition", poco.Id);
                }
                ...
                while (xmiReader.Read())
                {
                    if (xmiReader.NodeType == XmlNodeType.Element)
                    {
                        switch (xmiReader.LocalName)
                        {
                            ...
                            default:
                                this.logger.LogWarning("Unexpected element '{LocalName}' encountered while reading VerificationCaseDefinition at line:position {LineNumber}:{LinePosition}", xmiReader.LocalName, xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                                xmiReader.Skip();
                                break;
                        }
                    }
        ...
        /// <summary>
        /// Reads asynchronously the <see cref="IVerificationCaseDefinition" /> object from its XML representation
        /// </summary>
        /// <param name="xmiReader">An instance of <see cref="XmlReader" /></param>
        /// <param name="currentLocation">The <see cref="Uri" /> that keep tracks of the current location</param>
        /// <returns>An awaitable <see cref="Task{TResult}"/> with the read <see cref="IVerificationCaseDefinition" /></returns>
        public override async Task<IVerificationCaseDefinition> ReadAsync(XmlReader xmiReader, Uri currentLocation)
        {
            if (xmiReader == null)
            {
                throw new ArgumentNullException(nameof(xmiReader));
