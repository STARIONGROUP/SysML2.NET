// -------------------------------------------------------------------------------------------------
// <copyright file="BindingConnectorAsUsageReader.cs" company="Starion Group S.A.">
//
...
namespace SysML2.NET.Serializer.Xmi.Readers
{
    ...
    /// <summary>
    /// The purpose of the <see cref="BindingConnectorAsUsageReader" /> is to read an instance of <see cref="IBindingConnectorAsUsage" />
    /// from the XMI document
    /// </summary>
    public class BindingConnectorAsUsageReader : XmiDataReader<IBindingConnectorAsUsage>
    {
        /// <summary>
        /// The instantiated logger from the injected <see cref="ILoggerFactory" /> 
        /// </summary>
        private readonly ILogger<BindingConnectorAsUsageReader> logger;

        /// <summary>
        /// Initializes a new instance of the <see cref="BindingConnectorAsUsageReader" />
        /// </summary>
        /// <param name="cache">
        ...
        public BindingConnectorAsUsageReader(IXmiDataCache cache, IXmiDataReaderFacade xmiDataReaderFacade, IExternalReferenceService externalReferenceService, ILoggerFactory loggerFactory, IXmiElementOriginMap elementOriginMap = null) : base(cache, xmiDataReaderFacade, externalReferenceService, loggerFactory, elementOriginMap)
        {
            this.logger = loggerFactory == null ? NullLogger<BindingConnectorAsUsageReader>.Instance : loggerFactory.CreateLogger<BindingConnectorAsUsageReader>();
        }

        /// <summary>
        /// Reads the <see cref="IBindingConnectorAsUsage" /> object from its XML representation
        /// </summary>
        ...
        /// <returns>The read <see cref="IBindingConnectorAsUsage" /></returns>
        public override IBindingConnectorAsUsage Read(XmlReader xmiReader, Uri currentLocation)
        {
            if (xmiReader == null)
            {
            ...
            IBindingConnectorAsUsage poco = new SysML2.NET.Core.POCO.Systems.Connections.BindingConnectorAsUsage();

            if (xmiReader.MoveToContent() == XmlNodeType.Element)
            {
                this.logger.LogTrace("reading BindingConnectorAsUsage at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                var xsiType = xmiReader.GetAttribute("xsi:type");

                if (!string.IsNullOrEmpty(xsiType) && xsiType != "sysml:BindingConnectorAsUsage")
                {
                    throw new InvalidOperationException($"The xsi:type {xsiType} is not supported by the BindingConnectorAsUsageReader");
                }
                ...
                {
                    this.logger.LogCritical("Failed to add element type [{Poco}] with id [{Id}] as it was already in the Cache. The XMI document seems to have duplicate xmi:id values", "BindingConnectorAsUsage", poco.Id);
                }
                ...
                while (xmiReader.Read())
                {
                    if (xmiReader.NodeType == XmlNodeType.Element)
                    {
                        switch (xmiReader.LocalName)
                        {
                            ...
                            default:
                                this.logger.LogWarning("Unexpected element '{LocalName}' encountered while reading BindingConnectorAsUsage at line:position {LineNumber}:{LinePosition}", xmiReader.LocalName, xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                                xmiReader.Skip();
                                break;
                        }
                    }
        ...
        /// <summary>
        /// Reads asynchronously the <see cref="IBindingConnectorAsUsage" /> object from its XML representation
        /// </summary>
        /// <param name="xmiReader">An instance of <see cref="XmlReader" /></param>
        /// <param name="currentLocation">The <see cref="Uri" /> that keep tracks of the current location</param>
        /// <returns>An awaitable <see cref="Task{TResult}"/> with the read <see cref="IBindingConnectorAsUsage" /></returns>
        public override async Task<IBindingConnectorAsUsage> ReadAsync(XmlReader xmiReader, Uri currentLocation)
        {
            if (xmiReader == null)
            {
                throw new ArgumentNullException(nameof(xmiReader));
