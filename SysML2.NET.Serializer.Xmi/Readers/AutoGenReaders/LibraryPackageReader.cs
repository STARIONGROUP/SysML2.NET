// -------------------------------------------------------------------------------------------------
// <copyright file="LibraryPackageReader.cs" company="Starion Group S.A.">
//
...
namespace SysML2.NET.Serializer.Xmi.Readers
{
    ...
    /// <summary>
    /// The purpose of the <see cref="LibraryPackageReader" /> is to read an instance of <see cref="ILibraryPackage" />
    /// from the XMI document
    /// </summary>
    public class LibraryPackageReader : XmiDataReader<ILibraryPackage>
    {
        /// <summary>
        /// The instantiated logger from the injected <see cref="ILoggerFactory" /> 
        /// </summary>
        private readonly ILogger<LibraryPackageReader> logger;

        /// <summary>
        /// Initializes a new instance of the <see cref="LibraryPackageReader" />
        /// </summary>
        /// <param name="cache">
        ...
        public LibraryPackageReader(IXmiDataCache cache, IXmiDataReaderFacade xmiDataReaderFacade, IExternalReferenceService externalReferenceService, ILoggerFactory loggerFactory, IXmiElementOriginMap elementOriginMap = null) : base(cache, xmiDataReaderFacade, externalReferenceService, loggerFactory, elementOriginMap)
        {
            this.logger = loggerFactory == null ? NullLogger<LibraryPackageReader>.Instance : loggerFactory.CreateLogger<LibraryPackageReader>();
        }

        /// <summary>
        /// Reads the <see cref="ILibraryPackage" /> object from its XML representation
        /// </summary>
        ...
        /// <returns>The read <see cref="ILibraryPackage" /></returns>
        public override ILibraryPackage Read(XmlReader xmiReader, Uri currentLocation)
        {
            if (xmiReader == null)
            {
            ...
            ILibraryPackage poco = new SysML2.NET.Core.POCO.Kernel.Packages.LibraryPackage();

            if (xmiReader.MoveToContent() == XmlNodeType.Element)
            {
                this.logger.LogTrace("reading LibraryPackage at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                var xsiType = xmiReader.GetAttribute("xsi:type");

                if (!string.IsNullOrEmpty(xsiType) && xsiType != "sysml:LibraryPackage")
                {
                    throw new InvalidOperationException($"The xsi:type {xsiType} is not supported by the LibraryPackageReader");
                }
                ...
                {
                    this.logger.LogCritical("Failed to add element type [{Poco}] with id [{Id}] as it was already in the Cache. The XMI document seems to have duplicate xmi:id values", "LibraryPackage", poco.Id);
                }
                ...
                var isStandardXmlAttribute = xmiReader.GetAttribute("isStandard");

                if (!string.IsNullOrWhiteSpace(isStandardXmlAttribute))
                {
                    if (bool.TryParse(isStandardXmlAttribute, out var isStandardXmlAttributeAsBool))
                    {
                        poco.IsStandard = isStandardXmlAttributeAsBool;
                    }
                }
                ...
                while (xmiReader.Read())
                {
                    if (xmiReader.NodeType == XmlNodeType.Element)
                    {
                        switch (xmiReader.LocalName)
                        {
                            ...
                            case "isStandard":
                                {
                                    var isStandardValue = xmiReader.ReadElementContentAsString();

                                    if (!string.IsNullOrWhiteSpace(isStandardValue))
                                    {
                                        if (bool.TryParse(isStandardValue, out var isStandardValueAsBool))
                                        {
                                            poco.IsStandard = isStandardValueAsBool;
                                        }
                                        else
                                        {
                                            this.logger.LogWarning("Failed to parse bool value '{Value}' for property 'isStandard' on element {ElementId}", isStandardValue, poco.Id);
                                        }
                                    }
                            ...
                            default:
                                this.logger.LogWarning("Unexpected element '{LocalName}' encountered while reading LibraryPackage at line:position {LineNumber}:{LinePosition}", xmiReader.LocalName, xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                                xmiReader.Skip();
                                break;
                        }
                    }
        ...
        /// <summary>
        /// Reads asynchronously the <see cref="ILibraryPackage" /> object from its XML representation
        /// </summary>
        /// <param name="xmiReader">An instance of <see cref="XmlReader" /></param>
        /// <param name="currentLocation">The <see cref="Uri" /> that keep tracks of the current location</param>
        /// <returns>An awaitable <see cref="Task{TResult}"/> with the read <see cref="ILibraryPackage" /></returns>
        public override async Task<ILibraryPackage> ReadAsync(XmlReader xmiReader, Uri currentLocation)
        {
            if (xmiReader == null)
            {
                throw new ArgumentNullException(nameof(xmiReader));
                ...
                while (await xmiReader.ReadAsync())
                {
                    if (xmiReader.NodeType == XmlNodeType.Element)
                    {
                        switch (xmiReader.LocalName)
                        {
                            ...
                            case "isStandard":
                                {
                                    var isStandardValue = await xmiReader.ReadElementContentAsStringAsync();
