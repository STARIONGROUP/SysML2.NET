// -------------------------------------------------------------------------------------------------
// <copyright file="OccurrenceUsageReader.cs" company="Starion Group S.A.">
//
...
namespace SysML2.NET.Serializer.Xmi.Readers
{
    ...
    /// <summary>
    /// The purpose of the <see cref="OccurrenceUsageReader" /> is to read an instance of <see cref="IOccurrenceUsage" />
    /// from the XMI document
    /// </summary>
    public class OccurrenceUsageReader : XmiDataReader<IOccurrenceUsage>
    {
        /// <summary>
        /// The instantiated logger from the injected <see cref="ILoggerFactory" /> 
        /// </summary>
        private readonly ILogger<OccurrenceUsageReader> logger;

        /// <summary>
        /// Initializes a new instance of the <see cref="OccurrenceUsageReader" />
        /// </summary>
        /// <param name="cache">
        ...
        public OccurrenceUsageReader(IXmiDataCache cache, IXmiDataReaderFacade xmiDataReaderFacade, IExternalReferenceService externalReferenceService, ILoggerFactory loggerFactory, IXmiElementOriginMap elementOriginMap = null) : base(cache, xmiDataReaderFacade, externalReferenceService, loggerFactory, elementOriginMap)
        {
            this.logger = loggerFactory == null ? NullLogger<OccurrenceUsageReader>.Instance : loggerFactory.CreateLogger<OccurrenceUsageReader>();
        }

        /// <summary>
        /// Reads the <see cref="IOccurrenceUsage" /> object from its XML representation
        /// </summary>
        ...
        /// <returns>The read <see cref="IOccurrenceUsage" /></returns>
        public override IOccurrenceUsage Read(XmlReader xmiReader, Uri currentLocation)
        {
            if (xmiReader == null)
            {
            ...
            IOccurrenceUsage poco = new SysML2.NET.Core.POCO.Systems.Occurrences.OccurrenceUsage();

            if (xmiReader.MoveToContent() == XmlNodeType.Element)
            {
                this.logger.LogTrace("reading OccurrenceUsage at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                var xsiType = xmiReader.GetAttribute("xsi:type");

                if (!string.IsNullOrEmpty(xsiType) && xsiType != "sysml:OccurrenceUsage")
                {
                    throw new InvalidOperationException($"The xsi:type {xsiType} is not supported by the OccurrenceUsageReader");
                }
                ...
                {
                    this.logger.LogCritical("Failed to add element type [{Poco}] with id [{Id}] as it was already in the Cache. The XMI document seems to have duplicate xmi:id values", "OccurrenceUsage", poco.Id);
                }
                ...
                while (xmiReader.Read())
                {
                    if (xmiReader.NodeType == XmlNodeType.Element)
                    {
                        switch (xmiReader.LocalName)
                        {
                            ...
                            default:
                                this.logger.LogWarning("Unexpected element '{LocalName}' encountered while reading OccurrenceUsage at line:position {LineNumber}:{LinePosition}", xmiReader.LocalName, xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                                xmiReader.Skip();
                                break;
                        }
                    }
        ...
        /// <summary>
        /// Reads asynchronously the <see cref="IOccurrenceUsage" /> object from its XML representation
        /// </summary>
        /// <param name="xmiReader">An instance of <see cref="XmlReader" /></param>
        /// <param name="currentLocation">The <see cref="Uri" /> that keep tracks of the current location</param>
        /// <returns>An awaitable <see cref="Task{TResult}"/> with the read <see cref="IOccurrenceUsage" /></returns>
        public override async Task<IOccurrenceUsage> ReadAsync(XmlReader xmiReader, Uri currentLocation)
        {
            if (xmiReader == null)
            {
                throw new ArgumentNullException(nameof(xmiReader));
