// -------------------------------------------------------------------------------------------------
// <copyright file="UseCaseDefinitionReader.cs" company="Starion Group S.A.">
//
...
namespace SysML2.NET.Serializer.Xmi.Readers
{
    ...
    /// <summary>
    /// The purpose of the <see cref="UseCaseDefinitionReader" /> is to read an instance of <see cref="IUseCaseDefinition" />
    /// from the XMI document
    /// </summary>
    public class UseCaseDefinitionReader : XmiDataReader<IUseCaseDefinition>
    {
        /// <summary>
        /// The instantiated logger from the injected <see cref="ILoggerFactory" /> 
        /// </summary>
        private readonly ILogger<UseCaseDefinitionReader> logger;

        /// <summary>
        /// Initializes a new instance of the <see cref="UseCaseDefinitionReader" />
        /// </summary>
        /// <param name="cache">
        ...
        public UseCaseDefinitionReader(IXmiDataCache cache, IXmiDataReaderFacade xmiDataReaderFacade, IExternalReferenceService externalReferenceService, ILoggerFactory loggerFactory, IXmiElementOriginMap elementOriginMap = null) : base(cache, xmiDataReaderFacade, externalReferenceService, loggerFactory, elementOriginMap)
        {
            this.logger = loggerFactory == null ? NullLogger<UseCaseDefinitionReader>.Instance : loggerFactory.CreateLogger<UseCaseDefinitionReader>();
        }

        /// <summary>
        /// Reads the <see cref="IUseCaseDefinition" /> object from its XML representation
        /// </summary>
        ...
        /// <returns>The read <see cref="IUseCaseDefinition" /></returns>
        public override IUseCaseDefinition Read(XmlReader xmiReader, Uri currentLocation)
        {
            if (xmiReader == null)
            {
            ...
            IUseCaseDefinition poco = new SysML2.NET.Core.POCO.Systems.UseCases.UseCaseDefinition();

            if (xmiReader.MoveToContent() == XmlNodeType.Element)
            {
                this.logger.LogTrace("reading UseCaseDefinition at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                var xsiType = xmiReader.GetAttribute("xsi:type");

                if (!string.IsNullOrEmpty(xsiType) && xsiType != "sysml:UseCaseDefinition")
                {
                    throw new InvalidOperationException($"The xsi:type {xsiType} is not supported by the UseCaseDefinitionReader");
                }
                ...
                {
                    this.logger.LogCritical("Failed to add element type [{Poco}] with id [{Id}] as it was already in the Cache. The XMI document seems to have duplicate xmi:id values", "UseCaseDefinition", poco.Id);
                }
                ...
                while (xmiReader.Read())
                {
                    if (xmiReader.NodeType == XmlNodeType.Element)
                    {
                        switch (xmiReader.LocalName)
                        {
                            ...
                            default:
                                this.logger.LogWarning("Unexpected element '{LocalName}' encountered while reading UseCaseDefinition at line:position {LineNumber}:{LinePosition}", xmiReader.LocalName, xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                                xmiReader.Skip();
                                break;
                        }
                    }
        ...
        /// <summary>
        /// Reads asynchronously the <see cref="IUseCaseDefinition" /> object from its XML representation
        /// </summary>
        /// <param name="xmiReader">An instance of <see cref="XmlReader" /></param>
        /// <param name="currentLocation">The <see cref="Uri" /> that keep tracks of the current location</param>
        /// <returns>An awaitable <see cref="Task{TResult}"/> with the read <see cref="IUseCaseDefinition" /></returns>
        public override async Task<IUseCaseDefinition> ReadAsync(XmlReader xmiReader, Uri currentLocation)
        {
            if (xmiReader == null)
            {
                throw new ArgumentNullException(nameof(xmiReader));
