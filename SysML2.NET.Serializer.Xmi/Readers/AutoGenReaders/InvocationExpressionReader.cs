// -------------------------------------------------------------------------------------------------
// <copyright file="InvocationExpressionReader.cs" company="Starion Group S.A.">
//
...
namespace SysML2.NET.Serializer.Xmi.Readers
{
    ...
    /// <summary>
    /// The purpose of the <see cref="InvocationExpressionReader" /> is to read an instance of <see cref="IInvocationExpression" />
    /// from the XMI document
    /// </summary>
    public class InvocationExpressionReader : XmiDataReader<IInvocationExpression>
    {
        /// <summary>
        /// The instantiated logger from the injected <see cref="ILoggerFactory" /> 
        /// </summary>
        private readonly ILogger<InvocationExpressionReader> logger;

        /// <summary>
        /// Initializes a new instance of the <see cref="InvocationExpressionReader" />
        /// </summary>
        /// <param name="cache">
        ...
        public InvocationExpressionReader(IXmiDataCache cache, IXmiDataReaderFacade xmiDataReaderFacade, IExternalReferenceService externalReferenceService, ILoggerFactory loggerFactory, IXmiElementOriginMap elementOriginMap = null) : base(cache, xmiDataReaderFacade, externalReferenceService, loggerFactory, elementOriginMap)
        {
            this.logger = loggerFactory == null ? NullLogger<InvocationExpressionReader>.Instance : loggerFactory.CreateLogger<InvocationExpressionReader>();
        }

        /// <summary>
        /// Reads the <see cref="IInvocationExpression" /> object from its XML representation
        /// </summary>
        ...
        /// <returns>The read <see cref="IInvocationExpression" /></returns>
        public override IInvocationExpression Read(XmlReader xmiReader, Uri currentLocation)
        {
            if (xmiReader == null)
            {
            ...
            IInvocationExpression poco = new SysML2.NET.Core.POCO.Kernel.Expressions.InvocationExpression();

            if (xmiReader.MoveToContent() == XmlNodeType.Element)
            {
                this.logger.LogTrace("reading InvocationExpression at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                var xsiType = xmiReader.GetAttribute("xsi:type");

                if (!string.IsNullOrEmpty(xsiType) && xsiType != "sysml:InvocationExpression")
                {
                    throw new InvalidOperationException($"The xsi:type {xsiType} is not supported by the InvocationExpressionReader");
                }
                ...
                {
                    this.logger.LogCritical("Failed to add element type [{Poco}] with id [{Id}] as it was already in the Cache. The XMI document seems to have duplicate xmi:id values", "InvocationExpression", poco.Id);
                }
                ...
                while (xmiReader.Read())
                {
                    if (xmiReader.NodeType == XmlNodeType.Element)
                    {
                        switch (xmiReader.LocalName)
                        {
                            ...
                            default:
                                this.logger.LogWarning("Unexpected element '{LocalName}' encountered while reading InvocationExpression at line:position {LineNumber}:{LinePosition}", xmiReader.LocalName, xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                                xmiReader.Skip();
                                break;
                        }
                    }
        ...
        /// <summary>
        /// Reads asynchronously the <see cref="IInvocationExpression" /> object from its XML representation
        /// </summary>
        /// <param name="xmiReader">An instance of <see cref="XmlReader" /></param>
        /// <param name="currentLocation">The <see cref="Uri" /> that keep tracks of the current location</param>
        /// <returns>An awaitable <see cref="Task{TResult}"/> with the read <see cref="IInvocationExpression" /></returns>
        public override async Task<IInvocationExpression> ReadAsync(XmlReader xmiReader, Uri currentLocation)
        {
            if (xmiReader == null)
            {
                throw new ArgumentNullException(nameof(xmiReader));
