// -------------------------------------------------------------------------------------------------
// <copyright file="InvariantReader.cs" company="Starion Group S.A.">
//
...
namespace SysML2.NET.Serializer.Xmi.Readers
{
    ...
    /// <summary>
    /// The purpose of the <see cref="InvariantReader" /> is to read an instance of <see cref="IInvariant" />
    /// from the XMI document
    /// </summary>
    public class InvariantReader : XmiDataReader<IInvariant>
    {
        /// <summary>
        /// The instantiated logger from the injected <see cref="ILoggerFactory" /> 
        /// </summary>
        private readonly ILogger<InvariantReader> logger;

        /// <summary>
        /// Initializes a new instance of the <see cref="InvariantReader" />
        /// </summary>
        /// <param name="cache">
        ...
        public InvariantReader(IXmiDataCache cache, IXmiDataReaderFacade xmiDataReaderFacade, IExternalReferenceService externalReferenceService, ILoggerFactory loggerFactory, IXmiElementOriginMap elementOriginMap = null) : base(cache, xmiDataReaderFacade, externalReferenceService, loggerFactory, elementOriginMap)
        {
            this.logger = loggerFactory == null ? NullLogger<InvariantReader>.Instance : loggerFactory.CreateLogger<InvariantReader>();
        }

        /// <summary>
        /// Reads the <see cref="IInvariant" /> object from its XML representation
        /// </summary>
        ...
        /// <returns>The read <see cref="IInvariant" /></returns>
        public override IInvariant Read(XmlReader xmiReader, Uri currentLocation)
        {
            if (xmiReader == null)
            {
            ...
            IInvariant poco = new SysML2.NET.Core.POCO.Kernel.Functions.Invariant();

            if (xmiReader.MoveToContent() == XmlNodeType.Element)
            {
                this.logger.LogTrace("reading Invariant at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                var xsiType = xmiReader.GetAttribute("xsi:type");

                if (!string.IsNullOrEmpty(xsiType) && xsiType != "sysml:Invariant")
                {
                    throw new InvalidOperationException($"The xsi:type {xsiType} is not supported by the InvariantReader");
                }
                ...
                {
                    this.logger.LogCritical("Failed to add element type [{Poco}] with id [{Id}] as it was already in the Cache. The XMI document seems to have duplicate xmi:id values", "Invariant", poco.Id);
                }
                ...
                while (xmiReader.Read())
                {
                    if (xmiReader.NodeType == XmlNodeType.Element)
                    {
                        switch (xmiReader.LocalName)
                        {
                            ...
                            case "isVariable":
                                {
                                    ...
                                    {
                                        if (bool.TryParse(isVariableValue, out var isVariableValueAsBool))
                                        {
                                            poco.IsVariable = isVariableValueAsBool;
                                        }
                                        else
                                        {
                                            this.logger.LogWarning("Failed to parse bool value '{Value}' for property 'isVariable' on element {ElementId}", isVariableValue, poco.Id);
                                        }
                                    }
                            ...
                            default:
                                this.logger.LogWarning("Unexpected element '{LocalName}' encountered while reading Invariant at line:position {LineNumber}:{LinePosition}", xmiReader.LocalName, xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                                xmiReader.Skip();
                                break;
                        }
                    }
        ...
        /// <summary>
        /// Reads asynchronously the <see cref="IInvariant" /> object from its XML representation
        /// </summary>
        /// <param name="xmiReader">An instance of <see cref="XmlReader" /></param>
        /// <param name="currentLocation">The <see cref="Uri" /> that keep tracks of the current location</param>
        /// <returns>An awaitable <see cref="Task{TResult}"/> with the read <see cref="IInvariant" /></returns>
        public override async Task<IInvariant> ReadAsync(XmlReader xmiReader, Uri currentLocation)
        {
            if (xmiReader == null)
            {
                throw new ArgumentNullException(nameof(xmiReader));
                ...
                while (await xmiReader.ReadAsync())
                {
                    if (xmiReader.NodeType == XmlNodeType.Element)
                    {
                        switch (xmiReader.LocalName)
                        {
                            ...
                            case "isVariable":
                                {
                                    var isVariableValue = await xmiReader.ReadElementContentAsStringAsync();
