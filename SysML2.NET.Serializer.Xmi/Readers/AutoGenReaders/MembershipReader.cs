// -------------------------------------------------------------------------------------------------
// <copyright file="MembershipReader.cs" company="Starion Group S.A.">
//
...
namespace SysML2.NET.Serializer.Xmi.Readers
{
    ...
    public class MembershipReader : XmiDataReader<IMembership>
    {
        ...
        /// </summary>
        private readonly ILogger<MembershipReader> logger;

        /// <summary>
        /// Initializes a new instance of the <see cref="MembershipReader" />
        /// </summary>
        /// <param name="cache">
        ...
        public MembershipReader(IXmiDataCache cache, IXmiDataReaderFacade xmiDataReaderFacade, IExternalReferenceService externalReferenceService, ILoggerFactory loggerFactory, IXmiElementOriginMap elementOriginMap = null) : base(cache, xmiDataReaderFacade, externalReferenceService, loggerFactory, elementOriginMap)
        {
            this.logger = loggerFactory == null ? NullLogger<MembershipReader>.Instance : loggerFactory.CreateLogger<MembershipReader>();
        }

        /// <summary>
        /// Reads the <see cref="IMembership" /> object from its XML representation
        /// </summary>
        ...
        /// <returns>The read <see cref="IMembership" /></returns>
        public override IMembership Read(XmlReader xmiReader, Uri currentLocation)
        {
            if (xmiReader == null)
            {
            ...
            IMembership poco = new SysML2.NET.Core.POCO.Root.Namespaces.Membership();

            if (xmiReader.MoveToContent() == XmlNodeType.Element)
            {
                this.logger.LogTrace("reading Membership at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                var xsiType = xmiReader.GetAttribute("xsi:type");

                if (!string.IsNullOrEmpty(xsiType) && xsiType != "sysml:Membership")
                {
                    throw new InvalidOperationException($"The xsi:type {xsiType} is not supported by the MembershipReader");
                }
                ...
                {
                    this.logger.LogCritical("Failed to add element type [{Poco}] with id [{Id}] as it was already in the Cache. The XMI document seems to have duplicate xmi:id values", "Membership", poco.Id);
                }
                ...
                var memberElementXmlAttribute = xmiReader.GetAttribute("memberElement");

                if (!string.IsNullOrWhiteSpace(memberElementXmlAttribute))
                {
                    if (Guid.TryParse(memberElementXmlAttribute, out var memberElementXmlAttributeReference))
                    {
                        this.Cache.AddSingleValueReferencePropertyIdentifier(poco.Id, "memberElement", memberElementXmlAttributeReference);
                    }
                    else
                    {
                        this.logger.LogWarning("Failed to parse GUID reference value '{Value}' for property 'memberElement' on element {ElementId}", memberElementXmlAttribute, poco.Id);
                    }
                }

                var memberNameXmlAttribute = xmiReader.GetAttribute("memberName");

                if (!string.IsNullOrWhiteSpace(memberNameXmlAttribute))
                {
                    poco.MemberName = memberNameXmlAttribute;
                }

                var memberShortNameXmlAttribute = xmiReader.GetAttribute("memberShortName");

                if (!string.IsNullOrWhiteSpace(memberShortNameXmlAttribute))
                {
                    poco.MemberShortName = memberShortNameXmlAttribute;
                }
                ...
                while (xmiReader.Read())
                {
                    if (xmiReader.NodeType == XmlNodeType.Element)
                    {
                        switch (xmiReader.LocalName)
                        {
                            ...
                            case "memberElement":
                                {
                                    ...
                                    if (!string.IsNullOrWhiteSpace(hrefAttribute))
                                    {
                                        ...
                                        if (Guid.TryParse(hrefSplit[1], out var memberElementId))
                                        {
                                            this.Cache.AddSingleValueReferencePropertyIdentifier(poco.Id, "memberElement", memberElementId);
                                        }
                                        else
                                        {
                                            this.logger.LogWarning("Failed to parse href GUID value '{HrefValue}' for property 'memberElement' on element {ElementId}", hrefSplit[1], poco.Id);
                                        }
                                    }
                                    else
                                    {
                                        var memberElementValue = (IElement)this.XmiDataReaderFacade.QueryXmiData(xmiReader, this.Cache, currentLocation, this.ExternalReferenceService, this.LoggerFactory, elementOriginMap: this.ElementOriginMap);

                                        poco.MemberElement = memberElementValue;
                                    }
                            ...
                            case "memberName":
                                {
                                    var memberNameValue = xmiReader.ReadElementContentAsString();

                                    if (!string.IsNullOrWhiteSpace(memberNameValue))
                                    {
                                        poco.MemberName = memberNameValue;
                                    }
                            ...
                            case "memberShortName":
                                {
                                    var memberShortNameValue = xmiReader.ReadElementContentAsString();

                                    if (!string.IsNullOrWhiteSpace(memberShortNameValue))
                                    {
                                        poco.MemberShortName = memberShortNameValue;
                                    }
                            ...
                            default:
                                this.logger.LogWarning("Unexpected element '{LocalName}' encountered while reading Membership at line:position {LineNumber}:{LinePosition}", xmiReader.LocalName, xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                                xmiReader.Skip();
                                break;
                        }
                    }
        ...
        /// <summary>
        /// Reads asynchronously the <see cref="IMembership" /> object from its XML representation
        /// </summary>
        /// <param name="xmiReader">An instance of <see cref="XmlReader" /></param>
        /// <param name="currentLocation">The <see cref="Uri" /> that keep tracks of the current location</param>
        /// <returns>An awaitable <see cref="Task{TResult}"/> with the read <see cref="IMembership" /></returns>
        public override async Task<IMembership> ReadAsync(XmlReader xmiReader, Uri currentLocation)
        {
            if (xmiReader == null)
            {
                throw new ArgumentNullException(nameof(xmiReader));
                ...
                while (await xmiReader.ReadAsync())
                {
                    if (xmiReader.NodeType == XmlNodeType.Element)
                    {
                        switch (xmiReader.LocalName)
                        {
                            ...
                            case "memberElement":
                                {
                                    ...
                                    {
                                        var memberElementValue = (IElement)await this.XmiDataReaderFacade.QueryXmiDataAsync(xmiReader, this.Cache, currentLocation, this.ExternalReferenceService, this.LoggerFactory, elementOriginMap: this.ElementOriginMap);
                            ...
                            case "memberName":
                                {
                                    var memberNameValue = await xmiReader.ReadElementContentAsStringAsync();
                            ...
                            case "memberShortName":
                                {
                                    var memberShortNameValue = await xmiReader.ReadElementContentAsStringAsync();
