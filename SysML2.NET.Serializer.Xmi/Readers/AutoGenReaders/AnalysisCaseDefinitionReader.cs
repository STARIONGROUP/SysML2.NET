// -------------------------------------------------------------------------------------------------
// <copyright file="AnalysisCaseDefinitionReader.cs" company="Starion Group S.A.">
//
...
namespace SysML2.NET.Serializer.Xmi.Readers
{
    ...
    /// <summary>
    /// The purpose of the <see cref="AnalysisCaseDefinitionReader" /> is to read an instance of <see cref="IAnalysisCaseDefinition" />
    /// from the XMI document
    /// </summary>
    public class AnalysisCaseDefinitionReader : XmiDataReader<IAnalysisCaseDefinition>
    {
        /// <summary>
        /// The instantiated logger from the injected <see cref="ILoggerFactory" /> 
        /// </summary>
        private readonly ILogger<AnalysisCaseDefinitionReader> logger;

        /// <summary>
        /// Initializes a new instance of the <see cref="AnalysisCaseDefinitionReader" />
        /// </summary>
        /// <param name="cache">
        ...
        public AnalysisCaseDefinitionReader(IXmiDataCache cache, IXmiDataReaderFacade xmiDataReaderFacade, IExternalReferenceService externalReferenceService, ILoggerFactory loggerFactory, IXmiElementOriginMap elementOriginMap = null) : base(cache, xmiDataReaderFacade, externalReferenceService, loggerFactory, elementOriginMap)
        {
            this.logger = loggerFactory == null ? NullLogger<AnalysisCaseDefinitionReader>.Instance : loggerFactory.CreateLogger<AnalysisCaseDefinitionReader>();
        }

        /// <summary>
        /// Reads the <see cref="IAnalysisCaseDefinition" /> object from its XML representation
        /// </summary>
        ...
        /// <returns>The read <see cref="IAnalysisCaseDefinition" /></returns>
        public override IAnalysisCaseDefinition Read(XmlReader xmiReader, Uri currentLocation)
        {
            if (xmiReader == null)
            {
            ...
            IAnalysisCaseDefinition poco = new SysML2.NET.Core.POCO.Systems.AnalysisCases.AnalysisCaseDefinition();

            if (xmiReader.MoveToContent() == XmlNodeType.Element)
            {
                this.logger.LogTrace("reading AnalysisCaseDefinition at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                var xsiType = xmiReader.GetAttribute("xsi:type");

                if (!string.IsNullOrEmpty(xsiType) && xsiType != "sysml:AnalysisCaseDefinition")
                {
                    throw new InvalidOperationException($"The xsi:type {xsiType} is not supported by the AnalysisCaseDefinitionReader");
                }
                ...
                {
                    this.logger.LogCritical("Failed to add element type [{Poco}] with id [{Id}] as it was already in the Cache. The XMI document seems to have duplicate xmi:id values", "AnalysisCaseDefinition", poco.Id);
                }
                ...
                while (xmiReader.Read())
                {
                    if (xmiReader.NodeType == XmlNodeType.Element)
                    {
                        switch (xmiReader.LocalName)
                        {
                            ...
                            default:
                                this.logger.LogWarning("Unexpected element '{LocalName}' encountered while reading AnalysisCaseDefinition at line:position {LineNumber}:{LinePosition}", xmiReader.LocalName, xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                                xmiReader.Skip();
                                break;
                        }
                    }
        ...
        /// <summary>
        /// Reads asynchronously the <see cref="IAnalysisCaseDefinition" /> object from its XML representation
        /// </summary>
        /// <param name="xmiReader">An instance of <see cref="XmlReader" /></param>
        /// <param name="currentLocation">The <see cref="Uri" /> that keep tracks of the current location</param>
        /// <returns>An awaitable <see cref="Task{TResult}"/> with the read <see cref="IAnalysisCaseDefinition" /></returns>
        public override async Task<IAnalysisCaseDefinition> ReadAsync(XmlReader xmiReader, Uri currentLocation)
        {
            if (xmiReader == null)
            {
                throw new ArgumentNullException(nameof(xmiReader));
