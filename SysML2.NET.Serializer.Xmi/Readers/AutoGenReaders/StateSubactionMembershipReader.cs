// -------------------------------------------------------------------------------------------------
// <copyright file="StateSubactionMembershipReader.cs" company="Starion Group S.A.">
//
...
namespace SysML2.NET.Serializer.Xmi.Readers
{
    ...
    /// <summary>
    /// The purpose of the <see cref="StateSubactionMembershipReader" /> is to read an instance of <see cref="IStateSubactionMembership" />
    /// from the XMI document
    /// </summary>
    public class StateSubactionMembershipReader : XmiDataReader<IStateSubactionMembership>
    {
        /// <summary>
        /// The instantiated logger from the injected <see cref="ILoggerFactory" /> 
        /// </summary>
        private readonly ILogger<StateSubactionMembershipReader> logger;

        /// <summary>
        /// Initializes a new instance of the <see cref="StateSubactionMembershipReader" />
        /// </summary>
        /// <param name="cache">
        ...
        public StateSubactionMembershipReader(IXmiDataCache cache, IXmiDataReaderFacade xmiDataReaderFacade, IExternalReferenceService externalReferenceService, ILoggerFactory loggerFactory, IXmiElementOriginMap elementOriginMap = null) : base(cache, xmiDataReaderFacade, externalReferenceService, loggerFactory, elementOriginMap)
        {
            this.logger = loggerFactory == null ? NullLogger<StateSubactionMembershipReader>.Instance : loggerFactory.CreateLogger<StateSubactionMembershipReader>();
        }

        /// <summary>
        /// Reads the <see cref="IStateSubactionMembership" /> object from its XML representation
        /// </summary>
        ...
        /// <returns>The read <see cref="IStateSubactionMembership" /></returns>
        public override IStateSubactionMembership Read(XmlReader xmiReader, Uri currentLocation)
        {
            if (xmiReader == null)
            {
            ...
            IStateSubactionMembership poco = new SysML2.NET.Core.POCO.Systems.States.StateSubactionMembership();

            if (xmiReader.MoveToContent() == XmlNodeType.Element)
            {
                this.logger.LogTrace("reading StateSubactionMembership at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                var xsiType = xmiReader.GetAttribute("xsi:type");

                if (!string.IsNullOrEmpty(xsiType) && xsiType != "sysml:StateSubactionMembership")
                {
                    throw new InvalidOperationException($"The xsi:type {xsiType} is not supported by the StateSubactionMembershipReader");
                }
                ...
                {
                    this.logger.LogCritical("Failed to add element type [{Poco}] with id [{Id}] as it was already in the Cache. The XMI document seems to have duplicate xmi:id values", "StateSubactionMembership", poco.Id);
                }
                ...
                {
                    if (StateSubactionKindProvider.TryParse(kindXmlAttribute.AsSpan(), out var kindXmlAttributeAsEnum))
                    {
                ...
                while (xmiReader.Read())
                {
                    if (xmiReader.NodeType == XmlNodeType.Element)
                    {
                        switch (xmiReader.LocalName)
                        {
                            ...
                            case "kind":
                                {
                                    ...
                                    {
                                        if (StateSubactionKindProvider.TryParse(kindValue.AsSpan(), out var kindValueAsEnum))
                                        {
                            ...
                            default:
                                this.logger.LogWarning("Unexpected element '{LocalName}' encountered while reading StateSubactionMembership at line:position {LineNumber}:{LinePosition}", xmiReader.LocalName, xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                                xmiReader.Skip();
                                break;
                        }
                    }
        ...
        /// <summary>
        /// Reads asynchronously the <see cref="IStateSubactionMembership" /> object from its XML representation
        /// </summary>
        /// <param name="xmiReader">An instance of <see cref="XmlReader" /></param>
        /// <param name="currentLocation">The <see cref="Uri" /> that keep tracks of the current location</param>
        /// <returns>An awaitable <see cref="Task{TResult}"/> with the read <see cref="IStateSubactionMembership" /></returns>
        public override async Task<IStateSubactionMembership> ReadAsync(XmlReader xmiReader, Uri currentLocation)
        {
            if (xmiReader == null)
            {
                throw new ArgumentNullException(nameof(xmiReader));
