// -------------------------------------------------------------------------------------------------
// <copyright file="AssertConstraintUsageReader.cs" company="Starion Group S.A.">
//
...
namespace SysML2.NET.Serializer.Xmi.Readers
{
    ...
    public class AssertConstraintUsageReader : XmiDataReader<IAssertConstraintUsage>
    {
        ...
        /// </summary>
        private readonly ILogger<AssertConstraintUsageReader> logger;

        /// <summary>
        /// Initializes a new instance of the <see cref="AssertConstraintUsageReader" />
        /// </summary>
        /// <param name="cache">
        ...
        public AssertConstraintUsageReader(IXmiDataCache cache, IXmiDataReaderFacade xmiDataReaderFacade, IExternalReferenceService externalReferenceService, ILoggerFactory loggerFactory, IXmiElementOriginMap elementOriginMap = null) : base(cache, xmiDataReaderFacade, externalReferenceService, loggerFactory, elementOriginMap)
        {
            this.logger = loggerFactory == null ? NullLogger<AssertConstraintUsageReader>.Instance : loggerFactory.CreateLogger<AssertConstraintUsageReader>();
        }

        /// <summary>
        /// Reads the <see cref="IAssertConstraintUsage" /> object from its XML representation
        /// </summary>
        ...
        /// <returns>The read <see cref="IAssertConstraintUsage" /></returns>
        public override IAssertConstraintUsage Read(XmlReader xmiReader, Uri currentLocation)
        {
            if (xmiReader == null)
            {
            ...
            IAssertConstraintUsage poco = new SysML2.NET.Core.POCO.Systems.Constraints.AssertConstraintUsage();

            if (xmiReader.MoveToContent() == XmlNodeType.Element)
            {
                this.logger.LogTrace("reading AssertConstraintUsage at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                var xsiType = xmiReader.GetAttribute("xsi:type");

                if (!string.IsNullOrEmpty(xsiType) && xsiType != "sysml:AssertConstraintUsage")
                {
                    throw new InvalidOperationException($"The xsi:type {xsiType} is not supported by the AssertConstraintUsageReader");
                }
                ...
                {
                    this.logger.LogCritical("Failed to add element type [{Poco}] with id [{Id}] as it was already in the Cache. The XMI document seems to have duplicate xmi:id values", "AssertConstraintUsage", poco.Id);
                }
                ...
                while (xmiReader.Read())
                {
                    if (xmiReader.NodeType == XmlNodeType.Element)
                    {
                        switch (xmiReader.LocalName)
                        {
                            ...
                            default:
                                this.logger.LogWarning("Unexpected element '{LocalName}' encountered while reading AssertConstraintUsage at line:position {LineNumber}:{LinePosition}", xmiReader.LocalName, xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                                xmiReader.Skip();
                                break;
                        }
                    }
        ...
        /// <summary>
        /// Reads asynchronously the <see cref="IAssertConstraintUsage" /> object from its XML representation
        /// </summary>
        /// <param name="xmiReader">An instance of <see cref="XmlReader" /></param>
        /// <param name="currentLocation">The <see cref="Uri" /> that keep tracks of the current location</param>
        /// <returns>An awaitable <see cref="Task{TResult}"/> with the read <see cref="IAssertConstraintUsage" /></returns>
        public override async Task<IAssertConstraintUsage> ReadAsync(XmlReader xmiReader, Uri currentLocation)
        {
            if (xmiReader == null)
            {
                throw new ArgumentNullException(nameof(xmiReader));
