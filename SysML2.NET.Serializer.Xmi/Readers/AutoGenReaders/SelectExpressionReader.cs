// -------------------------------------------------------------------------------------------------
// <copyright file="SelectExpressionReader.cs" company="Starion Group S.A.">
//
...
namespace SysML2.NET.Serializer.Xmi.Readers
{
    ...
    /// <summary>
    /// The purpose of the <see cref="SelectExpressionReader" /> is to read an instance of <see cref="ISelectExpression" />
    /// from the XMI document
    /// </summary>
    public class SelectExpressionReader : XmiDataReader<ISelectExpression>
    {
        /// <summary>
        /// The instantiated logger from the injected <see cref="ILoggerFactory" /> 
        /// </summary>
        private readonly ILogger<SelectExpressionReader> logger;

        /// <summary>
        /// Initializes a new instance of the <see cref="SelectExpressionReader" />
        /// </summary>
        /// <param name="cache">
        ...
        public SelectExpressionReader(IXmiDataCache cache, IXmiDataReaderFacade xmiDataReaderFacade, IExternalReferenceService externalReferenceService, ILoggerFactory loggerFactory, IXmiElementOriginMap elementOriginMap = null) : base(cache, xmiDataReaderFacade, externalReferenceService, loggerFactory, elementOriginMap)
        {
            this.logger = loggerFactory == null ? NullLogger<SelectExpressionReader>.Instance : loggerFactory.CreateLogger<SelectExpressionReader>();
        }

        /// <summary>
        /// Reads the <see cref="ISelectExpression" /> object from its XML representation
        /// </summary>
        ...
        /// <returns>The read <see cref="ISelectExpression" /></returns>
        public override ISelectExpression Read(XmlReader xmiReader, Uri currentLocation)
        {
            if (xmiReader == null)
            {
            ...
            ISelectExpression poco = new SysML2.NET.Core.POCO.Kernel.Expressions.SelectExpression();

            if (xmiReader.MoveToContent() == XmlNodeType.Element)
            {
                this.logger.LogTrace("reading SelectExpression at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                var xsiType = xmiReader.GetAttribute("xsi:type");

                if (!string.IsNullOrEmpty(xsiType) && xsiType != "sysml:SelectExpression")
                {
                    throw new InvalidOperationException($"The xsi:type {xsiType} is not supported by the SelectExpressionReader");
                }
                ...
                {
                    this.logger.LogCritical("Failed to add element type [{Poco}] with id [{Id}] as it was already in the Cache. The XMI document seems to have duplicate xmi:id values", "SelectExpression", poco.Id);
                }
                ...
                var operatorXmlAttribute = xmiReader.GetAttribute("operator");

                if (!string.IsNullOrWhiteSpace(operatorXmlAttribute))
                {
                    poco.Operator = operatorXmlAttribute;
                }
                ...
                while (xmiReader.Read())
                {
                    if (xmiReader.NodeType == XmlNodeType.Element)
                    {
                        switch (xmiReader.LocalName)
                        {
                            ...
                            case "operator":
                                {
                                    var operatorValue = xmiReader.ReadElementContentAsString();

                                    if (!string.IsNullOrWhiteSpace(operatorValue))
                                    {
                                        poco.Operator = operatorValue;
                                    }
                            ...
                            default:
                                this.logger.LogWarning("Unexpected element '{LocalName}' encountered while reading SelectExpression at line:position {LineNumber}:{LinePosition}", xmiReader.LocalName, xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                                xmiReader.Skip();
                                break;
                        }
                    }
        ...
        /// <summary>
        /// Reads asynchronously the <see cref="ISelectExpression" /> object from its XML representation
        /// </summary>
        /// <param name="xmiReader">An instance of <see cref="XmlReader" /></param>
        /// <param name="currentLocation">The <see cref="Uri" /> that keep tracks of the current location</param>
        /// <returns>An awaitable <see cref="Task{TResult}"/> with the read <see cref="ISelectExpression" /></returns>
        public override async Task<ISelectExpression> ReadAsync(XmlReader xmiReader, Uri currentLocation)
        {
            if (xmiReader == null)
            {
                throw new ArgumentNullException(nameof(xmiReader));
                ...
                while (await xmiReader.ReadAsync())
                {
                    if (xmiReader.NodeType == XmlNodeType.Element)
                    {
                        switch (xmiReader.LocalName)
                        {
                            ...
                            case "operator":
                                {
                                    var operatorValue = await xmiReader.ReadElementContentAsStringAsync();
