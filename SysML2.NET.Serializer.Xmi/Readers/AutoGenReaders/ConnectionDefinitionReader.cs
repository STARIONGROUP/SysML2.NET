// -------------------------------------------------------------------------------------------------
// <copyright file="ConnectionDefinitionReader.cs" company="Starion Group S.A.">
//
...
namespace SysML2.NET.Serializer.Xmi.Readers
{
    ...
    /// <summary>
    /// The purpose of the <see cref="ConnectionDefinitionReader" /> is to read an instance of <see cref="IConnectionDefinition" />
    /// from the XMI document
    /// </summary>
    public class ConnectionDefinitionReader : XmiDataReader<IConnectionDefinition>
    {
        /// <summary>
        /// The instantiated logger from the injected <see cref="ILoggerFactory" /> 
        /// </summary>
        private readonly ILogger<ConnectionDefinitionReader> logger;

        /// <summary>
        /// Initializes a new instance of the <see cref="ConnectionDefinitionReader" />
        /// </summary>
        /// <param name="cache">
        ...
        public ConnectionDefinitionReader(IXmiDataCache cache, IXmiDataReaderFacade xmiDataReaderFacade, IExternalReferenceService externalReferenceService, ILoggerFactory loggerFactory, IXmiElementOriginMap elementOriginMap = null) : base(cache, xmiDataReaderFacade, externalReferenceService, loggerFactory, elementOriginMap)
        {
            this.logger = loggerFactory == null ? NullLogger<ConnectionDefinitionReader>.Instance : loggerFactory.CreateLogger<ConnectionDefinitionReader>();
        }

        /// <summary>
        /// Reads the <see cref="IConnectionDefinition" /> object from its XML representation
        /// </summary>
        ...
        /// <returns>The read <see cref="IConnectionDefinition" /></returns>
        public override IConnectionDefinition Read(XmlReader xmiReader, Uri currentLocation)
        {
            if (xmiReader == null)
            {
            ...
            IConnectionDefinition poco = new SysML2.NET.Core.POCO.Systems.Connections.ConnectionDefinition();
            ...
            if (xmiReader.MoveToContent() == XmlNodeType.Element)
            {
                ...
                {
                    this.logger.LogCritical("Failed to add element type [{Poco}] with id [{Id}] as it was already in the Cache. The XMI document seems to have duplicate xmi:id values", "ConnectionDefinition", poco.Id);
                }
                ...
                while (xmiReader.Read())
                {
                    if (xmiReader.NodeType == XmlNodeType.Element)
                    {
                        switch (xmiReader.LocalName)
                        {
                            ...
                            default:
                                this.logger.LogWarning("Unexpected element '{LocalName}' encountered while reading ConnectionDefinition at line:position {LineNumber}:{LinePosition}", xmiReader.LocalName, xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                                xmiReader.Skip();
                                break;
                        }
                    }
        ...
        /// <summary>
        /// Reads asynchronously the <see cref="IConnectionDefinition" /> object from its XML representation
        /// </summary>
        /// <param name="xmiReader">An instance of <see cref="XmlReader" /></param>
        /// <param name="currentLocation">The <see cref="Uri" /> that keep tracks of the current location</param>
        /// <returns>An awaitable <see cref="Task{TResult}"/> with the read <see cref="IConnectionDefinition" /></returns>
        public override async Task<IConnectionDefinition> ReadAsync(XmlReader xmiReader, Uri currentLocation)
        {
            if (xmiReader == null)
            {
                throw new ArgumentNullException(nameof(xmiReader));
