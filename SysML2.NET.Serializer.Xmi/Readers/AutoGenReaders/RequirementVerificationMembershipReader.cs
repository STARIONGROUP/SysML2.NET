// -------------------------------------------------------------------------------------------------
// <copyright file="RequirementVerificationMembershipReader.cs" company="Starion Group S.A.">
//
...
namespace SysML2.NET.Serializer.Xmi.Readers
{
    ...
    /// <summary>
    /// The purpose of the <see cref="RequirementVerificationMembershipReader" /> is to read an instance of <see cref="IRequirementVerificationMembership" />
    /// from the XMI document
    /// </summary>
    public class RequirementVerificationMembershipReader : XmiDataReader<IRequirementVerificationMembership>
    {
        /// <summary>
        /// The instantiated logger from the injected <see cref="ILoggerFactory" /> 
        /// </summary>
        private readonly ILogger<RequirementVerificationMembershipReader> logger;

        /// <summary>
        /// Initializes a new instance of the <see cref="RequirementVerificationMembershipReader" />
        /// </summary>
        /// <param name="cache">
        ...
        public RequirementVerificationMembershipReader(IXmiDataCache cache, IXmiDataReaderFacade xmiDataReaderFacade, IExternalReferenceService externalReferenceService, ILoggerFactory loggerFactory, IXmiElementOriginMap elementOriginMap = null) : base(cache, xmiDataReaderFacade, externalReferenceService, loggerFactory, elementOriginMap)
        {
            this.logger = loggerFactory == null ? NullLogger<RequirementVerificationMembershipReader>.Instance : loggerFactory.CreateLogger<RequirementVerificationMembershipReader>();
        }

        /// <summary>
        /// Reads the <see cref="IRequirementVerificationMembership" /> object from its XML representation
        /// </summary>
        ...
        /// <returns>The read <see cref="IRequirementVerificationMembership" /></returns>
        public override IRequirementVerificationMembership Read(XmlReader xmiReader, Uri currentLocation)
        {
            if (xmiReader == null)
            {
            ...
            IRequirementVerificationMembership poco = new SysML2.NET.Core.POCO.Systems.VerificationCases.RequirementVerificationMembership();

            if (xmiReader.MoveToContent() == XmlNodeType.Element)
            {
                this.logger.LogTrace("reading RequirementVerificationMembership at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                var xsiType = xmiReader.GetAttribute("xsi:type");

                if (!string.IsNullOrEmpty(xsiType) && xsiType != "sysml:RequirementVerificationMembership")
                {
                    throw new InvalidOperationException($"The xsi:type {xsiType} is not supported by the RequirementVerificationMembershipReader");
                }
                ...
                {
                    this.logger.LogCritical("Failed to add element type [{Poco}] with id [{Id}] as it was already in the Cache. The XMI document seems to have duplicate xmi:id values", "RequirementVerificationMembership", poco.Id);
                }
                ...
                while (xmiReader.Read())
                {
                    if (xmiReader.NodeType == XmlNodeType.Element)
                    {
                        switch (xmiReader.LocalName)
                        {
                            ...
                            case "kind":
                                {
                                    ...
                                    {
                                        if (RequirementConstraintKindProvider.TryParse(kindValue.AsSpan(), out var kindValueAsEnum))
                                        {
                            ...
                            default:
                                this.logger.LogWarning("Unexpected element '{LocalName}' encountered while reading RequirementVerificationMembership at line:position {LineNumber}:{LinePosition}", xmiReader.LocalName, xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                                xmiReader.Skip();
                                break;
                        }
                    }
        ...
        /// <summary>
        /// Reads asynchronously the <see cref="IRequirementVerificationMembership" /> object from its XML representation
        /// </summary>
        /// <param name="xmiReader">An instance of <see cref="XmlReader" /></param>
        /// <param name="currentLocation">The <see cref="Uri" /> that keep tracks of the current location</param>
        /// <returns>An awaitable <see cref="Task{TResult}"/> with the read <see cref="IRequirementVerificationMembership" /></returns>
        public override async Task<IRequirementVerificationMembership> ReadAsync(XmlReader xmiReader, Uri currentLocation)
        {
            if (xmiReader == null)
            {
                throw new ArgumentNullException(nameof(xmiReader));
