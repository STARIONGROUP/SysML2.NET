// -------------------------------------------------------------------------------------------------
// <copyright file="EndFeatureMembershipReader.cs" company="Starion Group S.A.">
//
...
namespace SysML2.NET.Serializer.Xmi.Readers
{
    ...
    /// <summary>
    /// The purpose of the <see cref="EndFeatureMembershipReader" /> is to read an instance of <see cref="IEndFeatureMembership" />
    /// from the XMI document
    /// </summary>
    public class EndFeatureMembershipReader : XmiDataReader<IEndFeatureMembership>
    {
        /// <summary>
        /// The instantiated logger from the injected <see cref="ILoggerFactory" /> 
        /// </summary>
        private readonly ILogger<EndFeatureMembershipReader> logger;

        /// <summary>
        /// Initializes a new instance of the <see cref="EndFeatureMembershipReader" />
        /// </summary>
        /// <param name="cache">
        ...
        public EndFeatureMembershipReader(IXmiDataCache cache, IXmiDataReaderFacade xmiDataReaderFacade, IExternalReferenceService externalReferenceService, ILoggerFactory loggerFactory, IXmiElementOriginMap elementOriginMap = null) : base(cache, xmiDataReaderFacade, externalReferenceService, loggerFactory, elementOriginMap)
        {
            this.logger = loggerFactory == null ? NullLogger<EndFeatureMembershipReader>.Instance : loggerFactory.CreateLogger<EndFeatureMembershipReader>();
        }

        /// <summary>
        /// Reads the <see cref="IEndFeatureMembership" /> object from its XML representation
        /// </summary>
        ...
        /// <returns>The read <see cref="IEndFeatureMembership" /></returns>
        public override IEndFeatureMembership Read(XmlReader xmiReader, Uri currentLocation)
        {
            if (xmiReader == null)
            {
            ...
            IEndFeatureMembership poco = new SysML2.NET.Core.POCO.Core.Features.EndFeatureMembership();
            ...
            if (xmiReader.MoveToContent() == XmlNodeType.Element)
            {
                ...
                {
                    this.logger.LogCritical("Failed to add element type [{Poco}] with id [{Id}] as it was already in the Cache. The XMI document seems to have duplicate xmi:id values", "EndFeatureMembership", poco.Id);
                }
                ...
                while (xmiReader.Read())
                {
                    if (xmiReader.NodeType == XmlNodeType.Element)
                    {
                        switch (xmiReader.LocalName)
                        {
                            ...
                            default:
                                this.logger.LogWarning("Unexpected element '{LocalName}' encountered while reading EndFeatureMembership at line:position {LineNumber}:{LinePosition}", xmiReader.LocalName, xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                                xmiReader.Skip();
                                break;
                        }
                    }
        ...
        /// <summary>
        /// Reads asynchronously the <see cref="IEndFeatureMembership" /> object from its XML representation
        /// </summary>
        /// <param name="xmiReader">An instance of <see cref="XmlReader" /></param>
        /// <param name="currentLocation">The <see cref="Uri" /> that keep tracks of the current location</param>
        /// <returns>An awaitable <see cref="Task{TResult}"/> with the read <see cref="IEndFeatureMembership" /></returns>
        public override async Task<IEndFeatureMembership> ReadAsync(XmlReader xmiReader, Uri currentLocation)
        {
            if (xmiReader == null)
            {
                throw new ArgumentNullException(nameof(xmiReader));
