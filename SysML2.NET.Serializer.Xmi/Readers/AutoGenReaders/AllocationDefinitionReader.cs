// -------------------------------------------------------------------------------------------------
// <copyright file="AllocationDefinitionReader.cs" company="Starion Group S.A.">
//
...
namespace SysML2.NET.Serializer.Xmi.Readers
{
    ...
    public class AllocationDefinitionReader : XmiDataReader<IAllocationDefinition>
    {
        ...
        /// </summary>
        private readonly ILogger<AllocationDefinitionReader> logger;

        /// <summary>
        /// Initializes a new instance of the <see cref="AllocationDefinitionReader" />
        /// </summary>
        /// <param name="cache">
        ...
        public AllocationDefinitionReader(IXmiDataCache cache, IXmiDataReaderFacade xmiDataReaderFacade, IExternalReferenceService externalReferenceService, ILoggerFactory loggerFactory, IXmiElementOriginMap elementOriginMap = null) : base(cache, xmiDataReaderFacade, externalReferenceService, loggerFactory, elementOriginMap)
        {
            this.logger = loggerFactory == null ? NullLogger<AllocationDefinitionReader>.Instance : loggerFactory.CreateLogger<AllocationDefinitionReader>();
        }

        /// <summary>
        /// Reads the <see cref="IAllocationDefinition" /> object from its XML representation
        /// </summary>
        ...
        /// <returns>The read <see cref="IAllocationDefinition" /></returns>
        public override IAllocationDefinition Read(XmlReader xmiReader, Uri currentLocation)
        {
            if (xmiReader == null)
            {
            ...
            IAllocationDefinition poco = new SysML2.NET.Core.POCO.Systems.Allocations.AllocationDefinition();
            ...
            if (xmiReader.MoveToContent() == XmlNodeType.Element)
            {
                ...
                {
                    this.logger.LogCritical("Failed to add element type [{Poco}] with id [{Id}] as it was already in the Cache. The XMI document seems to have duplicate xmi:id values", "AllocationDefinition", poco.Id);
                }
                ...
                while (xmiReader.Read())
                {
                    if (xmiReader.NodeType == XmlNodeType.Element)
                    {
                        switch (xmiReader.LocalName)
                        {
                            ...
                            default:
                                this.logger.LogWarning("Unexpected element '{LocalName}' encountered while reading AllocationDefinition at line:position {LineNumber}:{LinePosition}", xmiReader.LocalName, xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                                xmiReader.Skip();
                                break;
                        }
                    }
        ...
        /// <summary>
        /// Reads asynchronously the <see cref="IAllocationDefinition" /> object from its XML representation
        /// </summary>
        /// <param name="xmiReader">An instance of <see cref="XmlReader" /></param>
        /// <param name="currentLocation">The <see cref="Uri" /> that keep tracks of the current location</param>
        /// <returns>An awaitable <see cref="Task{TResult}"/> with the read <see cref="IAllocationDefinition" /></returns>
        public override async Task<IAllocationDefinition> ReadAsync(XmlReader xmiReader, Uri currentLocation)
        {
            if (xmiReader == null)
            {
                throw new ArgumentNullException(nameof(xmiReader));
