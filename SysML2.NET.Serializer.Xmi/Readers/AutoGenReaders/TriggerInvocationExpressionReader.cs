// -------------------------------------------------------------------------------------------------
// <copyright file="TriggerInvocationExpressionReader.cs" company="Starion Group S.A.">
//
...
namespace SysML2.NET.Serializer.Xmi.Readers
{
    ...
    /// <summary>
    /// The purpose of the <see cref="TriggerInvocationExpressionReader" /> is to read an instance of <see cref="ITriggerInvocationExpression" />
    /// from the XMI document
    /// </summary>
    public class TriggerInvocationExpressionReader : XmiDataReader<ITriggerInvocationExpression>
    {
        /// <summary>
        /// The instantiated logger from the injected <see cref="ILoggerFactory" /> 
        /// </summary>
        private readonly ILogger<TriggerInvocationExpressionReader> logger;

        /// <summary>
        /// Initializes a new instance of the <see cref="TriggerInvocationExpressionReader" />
        /// </summary>
        /// <param name="cache">
        ...
        public TriggerInvocationExpressionReader(IXmiDataCache cache, IXmiDataReaderFacade xmiDataReaderFacade, IExternalReferenceService externalReferenceService, ILoggerFactory loggerFactory, IXmiElementOriginMap elementOriginMap = null) : base(cache, xmiDataReaderFacade, externalReferenceService, loggerFactory, elementOriginMap)
        {
            this.logger = loggerFactory == null ? NullLogger<TriggerInvocationExpressionReader>.Instance : loggerFactory.CreateLogger<TriggerInvocationExpressionReader>();
        }

        /// <summary>
        /// Reads the <see cref="ITriggerInvocationExpression" /> object from its XML representation
        /// </summary>
        ...
        /// <returns>The read <see cref="ITriggerInvocationExpression" /></returns>
        public override ITriggerInvocationExpression Read(XmlReader xmiReader, Uri currentLocation)
        {
            if (xmiReader == null)
            {
            ...
            ITriggerInvocationExpression poco = new SysML2.NET.Core.POCO.Systems.Actions.TriggerInvocationExpression();

            if (xmiReader.MoveToContent() == XmlNodeType.Element)
            {
                this.logger.LogTrace("reading TriggerInvocationExpression at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                var xsiType = xmiReader.GetAttribute("xsi:type");

                if (!string.IsNullOrEmpty(xsiType) && xsiType != "sysml:TriggerInvocationExpression")
                {
                    throw new InvalidOperationException($"The xsi:type {xsiType} is not supported by the TriggerInvocationExpressionReader");
                }
                ...
                {
                    this.logger.LogCritical("Failed to add element type [{Poco}] with id [{Id}] as it was already in the Cache. The XMI document seems to have duplicate xmi:id values", "TriggerInvocationExpression", poco.Id);
                }
                ...
                var kindXmlAttribute = xmiReader.GetAttribute("kind");

                if (!string.IsNullOrWhiteSpace(kindXmlAttribute))
                {
                    if (TriggerKindProvider.TryParse(kindXmlAttribute.AsSpan(), out var kindXmlAttributeAsEnum))
                    {
                ...
                while (xmiReader.Read())
                {
                    if (xmiReader.NodeType == XmlNodeType.Element)
                    {
                        switch (xmiReader.LocalName)
                        {
                            ...
                            case "kind":
                                {
                                    var kindValue = xmiReader.ReadElementContentAsString();

                                    if (!string.IsNullOrWhiteSpace(kindValue))
                                    {
                                        if (TriggerKindProvider.TryParse(kindValue.AsSpan(), out var kindValueAsEnum))
                                        {
                                            poco.Kind = kindValueAsEnum;
                                        }
                                        else
                                        {
                                            this.logger.LogWarning("Failed to parse enum value '{Value}' for property 'kind' on element {ElementId}", kindValue, poco.Id);
                                        }
                                    }
                            ...
                            default:
                                this.logger.LogWarning("Unexpected element '{LocalName}' encountered while reading TriggerInvocationExpression at line:position {LineNumber}:{LinePosition}", xmiReader.LocalName, xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                                xmiReader.Skip();
                                break;
                        }
                    }
        ...
        /// <summary>
        /// Reads asynchronously the <see cref="ITriggerInvocationExpression" /> object from its XML representation
        /// </summary>
        /// <param name="xmiReader">An instance of <see cref="XmlReader" /></param>
        /// <param name="currentLocation">The <see cref="Uri" /> that keep tracks of the current location</param>
        /// <returns>An awaitable <see cref="Task{TResult}"/> with the read <see cref="ITriggerInvocationExpression" /></returns>
        public override async Task<ITriggerInvocationExpression> ReadAsync(XmlReader xmiReader, Uri currentLocation)
        {
            if (xmiReader == null)
            {
                throw new ArgumentNullException(nameof(xmiReader));
                ...
                while (await xmiReader.ReadAsync())
                {
                    if (xmiReader.NodeType == XmlNodeType.Element)
                    {
                        switch (xmiReader.LocalName)
                        {
                            ...
                            case "kind":
                                {
                                    var kindValue = await xmiReader.ReadElementContentAsStringAsync();
