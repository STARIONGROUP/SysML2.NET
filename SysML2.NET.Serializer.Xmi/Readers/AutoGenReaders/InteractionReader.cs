...
namespace SysML2.NET.Serializer.Xmi.Readers
{
    ...
    /// </summary>
    public class InteractionReader : XmiDataReader<IInteraction>
    {
        /// <summary>
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="InteractionReader" />
        /// </summary>
        /// <param name="cache">
        ...
        public InteractionReader(IXmiDataCache cache, IXmiDataReaderFacade xmiDataReaderFacade, IExternalReferenceService externalReferenceService, ILoggerFactory loggerFactory, IXmiElementOriginMap elementOriginMap = null) : base(cache, xmiDataReaderFacade, externalReferenceService, loggerFactory, elementOriginMap)
        {
        ...
        /// <returns>The read <see cref="IInteraction" /></returns>
        public override IInteraction Read(XmlReader xmiReader, Uri currentLocation)
        {
            if (xmiReader == null)
            {
            ...
            IInteraction poco = new SysML2.NET.Core.POCO.Kernel.Interactions.Interaction();
            ...
            if (xmiReader.MoveToContent() == XmlNodeType.Element)
            {
                ...
                if (!string.IsNullOrEmpty(xsiType) && xsiType != "sysml:Interaction")
                {
                    throw new InvalidOperationException($"The xsi:type {xsiType} is not supported by the InteractionReader");
                }
                ...
                {
                    this.logger.LogCritical("Failed to add element type [{Poco}] with id [{Id}] as it was already in the Cache. The XMI document seems to have duplicate xmi:id values", "Interaction", poco.Id);
                }
                ...
                while (xmiReader.Read())
                {
                    if (xmiReader.NodeType == XmlNodeType.Element)
                    {
                        switch (xmiReader.LocalName)
                        {
                            ...
                            default:
                                this.logger.LogWarning("Unexpected element '{LocalName}' encountered while reading Interaction at line:position {LineNumber}:{LinePosition}", xmiReader.LocalName, xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                                xmiReader.Skip();
                                break;
                        }
                    }
        ...
        /// <summary>
        /// Reads asynchronously the <see cref="IInteraction" /> object from its XML representation
        /// </summary>
        /// <param name="xmiReader">An instance of <see cref="XmlReader" /></param>
        /// <param name="currentLocation">The <see cref="Uri" /> that keep tracks of the current location</param>
        /// <returns>An awaitable <see cref="Task{TResult}"/> with the read <see cref="IInteraction" /></returns>
        public override async Task<IInteraction> ReadAsync(XmlReader xmiReader, Uri currentLocation)
        {
            if (xmiReader == null)
            {
                throw new ArgumentNullException(nameof(xmiReader));
