...
namespace SysML2.NET.Serializer.Json.Core.DTO
{
    ...
    internal static class MultiplicityRangeDeSerializer
    {
        ...
        internal static IMultiplicityRange DeSerialize(JsonElement jsonElement, SerializationModeKind serializationModeKind, ILoggerFactory loggerFactory = null)
        {
            ...
            {
                foreach (var arrayItem in boundProperty.EnumerateArray())
                {
                    if (arrayItem.TryGetProperty("@id"u8, out var boundExternalIdProperty))
                    {
                        var propertyValue = boundExternalIdProperty.GetString();

                        if (propertyValue != null)
                        {
                            dtoInstance.bound.Add(Guid.Parse(propertyValue));
                        }
                    }
                }
            }
            ...
            {
                if (lowerBoundProperty.ValueKind == JsonValueKind.Null)
                {
                    dtoInstance.lowerBound = null;
                }
                else
                {
                    if (lowerBoundProperty.TryGetProperty("@id"u8, out var lowerBoundExternalIdProperty))
                    {
                        var propertyValue = lowerBoundExternalIdProperty.GetString();

                        if (propertyValue != null)
                        {
                            dtoInstance.lowerBound = Guid.Parse(propertyValue);
                        }
                    }
                }
            }
            ...
            {
                if (upperBoundProperty.ValueKind == JsonValueKind.Null)
                {
                ...
                {
                    if (upperBoundProperty.TryGetProperty("@id"u8, out var upperBoundExternalIdProperty))
                    {
                        var propertyValue = upperBoundExternalIdProperty.GetString();

                        if (propertyValue != null)
                        {
                            dtoInstance.upperBound = Guid.Parse(propertyValue);
                        }
                    }
                }
            }
