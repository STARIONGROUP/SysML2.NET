...
namespace SysML2.NET.Serializer.Json.Core.DTO
{
    ...
    {
        /// <summary>
        /// Deserializes an instance of <see cref="IFeatureTyping"/> from the provided <see cref="Utf8JsonReader"/>
        /// </summary>
        /// <param name="reader">
        /// The <see cref="Utf8JsonReader"/> positioned on the <see cref="JsonTokenType.StartObject"/> of the
        /// <see cref="IFeatureTyping"/> json object. On return the reader is positioned on the matching
        ...
        /// </remarks>
        internal static IFeatureTyping DeSerialize(ref Utf8JsonReader reader, SerializationModeKind serializationModeKind, bool deserializeDerivedProperties, ILoggerFactory loggerFactory = null)
        {
        ...
        /// <summary>
        /// Deserializes properties of a <see cref="SysML2.NET.Core.DTO.Core.Features.FeatureTyping" />
        /// from a <see cref="Utf8JsonReader" />, including derived properties
        /// </summary>
        /// <param name="dtoInstance">
        /// The <see cref="SysML2.NET.Core.DTO.Core.Features.FeatureTyping"/> instance holding deserialized values
        /// </param>
        /// <param name="reader">
        /// The <see cref="Utf8JsonReader"/> positioned on the <see cref="JsonTokenType.StartObject"/> of the
        /// <see cref="IFeatureTyping"/> json object
        /// </param>
        /// <param name="logger">
        /// The <see cref="ILogger"/> to produce logging statement
        /// </param>
        private static void DeserializeDtoIncludingDerivedProperties(SysML2.NET.Core.DTO.Core.Features.FeatureTyping dtoInstance, ref Utf8JsonReader reader, ILogger logger)
        {
            ...
            var typeSeen = false;
            var typedFeatureSeen = false;

            while (reader.Read() && reader.TokenType != JsonTokenType.EndObject)
            {
                if (reader.TokenType != JsonTokenType.PropertyName)
                {
                    throw new JsonException("Expected a property name in the FeatureTyping json object.");
                }
                ...
                if (reader.ValueTextEquals("type"u8))
                {
                    ...
                    else if (Utf8JsonReaderHelper.TryReadReferenceIdentifier(ref reader, out var typeValue))
                    {
                        dtoInstance.Type = typeValue;
                    }

                    continue;
                }

                if (reader.ValueTextEquals("typedFeature"u8))
                {
                    typedFeatureSeen = true;
                    reader.Read();
                    ...
                    else if (Utf8JsonReaderHelper.TryReadReferenceIdentifier(ref reader, out var typedFeatureValue))
                    {
                        dtoInstance.TypedFeature = typedFeatureValue;
                    }
            ...
            if (logger.IsEnabled(LogLevel.Debug))
            {
                ...
                {
                    logger.LogDebug("the documentation Json property was not found in the FeatureTyping: {Id}", dtoInstance.Id);
                }
                if (!elementIdSeen)
                {
                ...
                {
                    logger.LogDebug("the isLibraryElement Json property was not found in the FeatureTyping: {Id}", dtoInstance.Id);
                }
                if (!nameSeen)
                {
                    logger.LogDebug("the name Json property was not found in the FeatureTyping: {Id}", dtoInstance.Id);
                }
                if (!ownedAnnotationSeen)
                {
                    logger.LogDebug("the ownedAnnotation Json property was not found in the FeatureTyping: {Id}", dtoInstance.Id);
                }
                if (!ownedElementSeen)
                {
                    logger.LogDebug("the ownedElement Json property was not found in the FeatureTyping: {Id}", dtoInstance.Id);
                }
                ...
                if (!ownerSeen)
                {
                    logger.LogDebug("the owner Json property was not found in the FeatureTyping: {Id}", dtoInstance.Id);
                }
                if (!owningFeatureSeen)
                {
                    logger.LogDebug("the owningFeature Json property was not found in the FeatureTyping: {Id}", dtoInstance.Id);
                }
                if (!owningMembershipSeen)
                {
                    logger.LogDebug("the owningMembership Json property was not found in the FeatureTyping: {Id}", dtoInstance.Id);
                }
                if (!owningNamespaceSeen)
                {
                    logger.LogDebug("the owningNamespace Json property was not found in the FeatureTyping: {Id}", dtoInstance.Id);
                }
                ...
                {
                    logger.LogDebug("the qualifiedName Json property was not found in the FeatureTyping: {Id}", dtoInstance.Id);
                }
                if (!relatedElementSeen)
                {
                    logger.LogDebug("the relatedElement Json property was not found in the FeatureTyping: {Id}", dtoInstance.Id);
                }
                if (!shortNameSeen)
                {
                ...
                {
                    logger.LogDebug("the textualRepresentation Json property was not found in the FeatureTyping: {Id}", dtoInstance.Id);
                }
                if (!typeSeen)
                {
        ...
        /// The <see cref="ILogger"/> to produce logging statement
        /// </param>
        private static void DeserializeDtoExcludingDerivedProperties(SysML2.NET.Core.DTO.Core.Features.FeatureTyping dtoInstance, ref Utf8JsonReader reader, ILogger logger)
        {
            var aliasIdsSeen = false;
            var declaredNameSeen = false;
            var declaredShortNameSeen = false;
