// -------------------------------------------------------------------------------------------------
// <copyright file="TextualRepresentationReader.cs" company="Starion Group S.A.">
//
...
namespace SysML2.NET.Serializer.Xmi.Readers
{
    ...
    /// <summary>
    /// The purpose of the <see cref="TextualRepresentationReader" /> is to read an instance of <see cref="ITextualRepresentation" />
    /// from the XMI document
    /// </summary>
    public class TextualRepresentationReader : XmiDataReader<ITextualRepresentation>
    {
        /// <summary>
        /// The instantiated logger from the injected <see cref="ILoggerFactory" /> 
        /// </summary>
        private readonly ILogger<TextualRepresentationReader> logger;

        /// <summary>
        /// Initializes a new instance of the <see cref="TextualRepresentationReader" />
        /// </summary>
        /// <param name="cache">
        ...
        public TextualRepresentationReader(IXmiDataCache cache, IXmiDataReaderFacade xmiDataReaderFacade, IExternalReferenceService externalReferenceService, ILoggerFactory loggerFactory, IXmiElementOriginMap elementOriginMap = null) : base(cache, xmiDataReaderFacade, externalReferenceService, loggerFactory, elementOriginMap)
        {
            this.logger = loggerFactory == null ? NullLogger<TextualRepresentationReader>.Instance : loggerFactory.CreateLogger<TextualRepresentationReader>();
        }

        /// <summary>
        /// Reads the <see cref="ITextualRepresentation" /> object from its XML representation
        /// </summary>
        ...
        /// <returns>The read <see cref="ITextualRepresentation" /></returns>
        public override ITextualRepresentation Read(XmlReader xmiReader, Uri currentLocation)
        {
            if (xmiReader == null)
            {
            ...
            ITextualRepresentation poco = new SysML2.NET.Core.POCO.Root.Annotations.TextualRepresentation();

            if (xmiReader.MoveToContent() == XmlNodeType.Element)
            {
                this.logger.LogTrace("reading TextualRepresentation at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                var xsiType = xmiReader.GetAttribute("xsi:type");

                if (!string.IsNullOrEmpty(xsiType) && xsiType != "sysml:TextualRepresentation")
                {
                    throw new InvalidOperationException($"The xsi:type {xsiType} is not supported by the TextualRepresentationReader");
                }
                ...
                {
                    this.logger.LogCritical("Failed to add element type [{Poco}] with id [{Id}] as it was already in the Cache. The XMI document seems to have duplicate xmi:id values", "TextualRepresentation", poco.Id);
                }
                ...
                var bodyXmlAttribute = xmiReader.GetAttribute("body");

                if (!string.IsNullOrWhiteSpace(bodyXmlAttribute))
                {
                    poco.Body = bodyXmlAttribute;
                }
                ...
                var languageXmlAttribute = xmiReader.GetAttribute("language");

                if (!string.IsNullOrWhiteSpace(languageXmlAttribute))
                {
                    poco.Language = languageXmlAttribute;
                }
                ...
                while (xmiReader.Read())
                {
                    if (xmiReader.NodeType == XmlNodeType.Element)
                    {
                        switch (xmiReader.LocalName)
                        {
                            ...
                            case "body":
                                {
                                    var bodyValue = xmiReader.ReadElementContentAsString();
                            ...
                            case "language":
                                {
                                    var languageValue = xmiReader.ReadElementContentAsString();

                                    if (!string.IsNullOrWhiteSpace(languageValue))
                                    {
                                        poco.Language = languageValue;
                                    }
                            ...
                            default:
                                this.logger.LogWarning("Unexpected element '{LocalName}' encountered while reading TextualRepresentation at line:position {LineNumber}:{LinePosition}", xmiReader.LocalName, xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                                xmiReader.Skip();
                                break;
                        }
                    }
        ...
        /// <summary>
        /// Reads asynchronously the <see cref="ITextualRepresentation" /> object from its XML representation
        /// </summary>
        /// <param name="xmiReader">An instance of <see cref="XmlReader" /></param>
        /// <param name="currentLocation">The <see cref="Uri" /> that keep tracks of the current location</param>
        /// <returns>An awaitable <see cref="Task{TResult}"/> with the read <see cref="ITextualRepresentation" /></returns>
        public override async Task<ITextualRepresentation> ReadAsync(XmlReader xmiReader, Uri currentLocation)
        {
            if (xmiReader == null)
            {
                throw new ArgumentNullException(nameof(xmiReader));
                ...
                while (await xmiReader.ReadAsync())
                {
                    if (xmiReader.NodeType == XmlNodeType.Element)
                    {
                        switch (xmiReader.LocalName)
                        {
                            ...
                            case "body":
                                {
                                    var bodyValue = await xmiReader.ReadElementContentAsStringAsync();
                            ...
                            case "language":
                                {
                                    var languageValue = await xmiReader.ReadElementContentAsStringAsync();
