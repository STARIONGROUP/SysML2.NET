...
namespace SysML2.NET.Serializer.Xmi.Readers
{
    ...
    /// </summary>
    public class DependencyReader : XmiDataReader<IDependency>
    {
        /// <summary>
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="DependencyReader" />
        /// </summary>
        /// <param name="cache">
        ...
        /// <returns>The read <see cref="IDependency" /></returns>
        public override IDependency Read(XmlReader xmiReader, Uri currentLocation)
        {
            if (xmiReader == null)
            {
            ...
            IDependency poco = new SysML2.NET.Core.POCO.Root.Dependencies.Dependency();
            ...
            if (xmiReader.MoveToContent() == XmlNodeType.Element)
            {
                ...
                if (!string.IsNullOrEmpty(xsiType) && xsiType != "sysml:Dependency")
                {
                    throw new InvalidOperationException($"The xsi:type {xsiType} is not supported by the DependencyReader");
                }
                ...
                var clientXmlAttribute = xmiReader.GetAttribute("client");

                if (!string.IsNullOrWhiteSpace(clientXmlAttribute))
                {
                    var clientXmlAttributeReferences = new List<Guid>();

                    foreach (var clientXmlAttributeValue in clientXmlAttribute.Split(SplitMultiReference, StringSplitOptions.RemoveEmptyEntries))
                    {
                        if (Guid.TryParse(clientXmlAttributeValue, out var clientXmlAttributeReference))
                        {
                            clientXmlAttributeReferences.Add(clientXmlAttributeReference);
                        }
                        else
                        {
                            this.logger.LogWarning("Failed to parse GUID reference value '{Value}' for property 'client' on element {ElementId}", clientXmlAttributeValue, poco.Id);
                        }
                    }

                    if (clientXmlAttributeReferences.Count != 0)
                    {
                        this.Cache.AddMultipleValueReferencePropertyIdentifiers(poco.Id, "client", clientXmlAttributeReferences);
                    }
                }
                ...
                var supplierXmlAttribute = xmiReader.GetAttribute("supplier");

                if (!string.IsNullOrWhiteSpace(supplierXmlAttribute))
                {
                    var supplierXmlAttributeReferences = new List<Guid>();

                    foreach (var supplierXmlAttributeValue in supplierXmlAttribute.Split(SplitMultiReference, StringSplitOptions.RemoveEmptyEntries))
                    {
                        if (Guid.TryParse(supplierXmlAttributeValue, out var supplierXmlAttributeReference))
                        {
                            supplierXmlAttributeReferences.Add(supplierXmlAttributeReference);
                        }
                        else
                        {
                            this.logger.LogWarning("Failed to parse GUID reference value '{Value}' for property 'supplier' on element {ElementId}", supplierXmlAttributeValue, poco.Id);
                        }
                    }

                    if (supplierXmlAttributeReferences.Count != 0)
                    {
                        this.Cache.AddMultipleValueReferencePropertyIdentifiers(poco.Id, "supplier", supplierXmlAttributeReferences);
                    }
                }
                ...
                while (xmiReader.Read())
                {
                    if (xmiReader.NodeType == XmlNodeType.Element)
                    {
                        switch (xmiReader.LocalName)
                        {
                            ...
                            case "client":
                                {
                                    ...
                                    if (!string.IsNullOrWhiteSpace(hrefAttribute))
                                    {
                                        ...
                                        if (Guid.TryParse(hrefSplit[1], out var clientId))
                                        {
                                            this.Cache.AddMultipleValueReferencePropertyIdentifiers(poco.Id, "client", clientId);
                                        }
                                        else
                                        {
                                            this.logger.LogWarning("Failed to parse href GUID value '{HrefValue}' for property 'client' on element {ElementId}", hrefSplit[1], poco.Id);
                                        }
                                    }
                                    else
                                    {
                            ...
                            case "supplier":
                                {
                                    ...
                                    if (!string.IsNullOrWhiteSpace(hrefAttribute))
                                    {
                                        var hrefSplit = hrefAttribute.Split('#');
                                        this.ExternalReferenceService.AddExternalReferenceToProcess(currentLocation, hrefSplit[0]);
                                        if (Guid.TryParse(hrefSplit[1], out var supplierId))
                                        {
                                            this.Cache.AddMultipleValueReferencePropertyIdentifiers(poco.Id, "supplier", supplierId);
                                        }
                                        else
                                        {
                                            this.logger.LogWarning("Failed to parse href GUID value '{HrefValue}' for property 'supplier' on element {ElementId}", hrefSplit[1], poco.Id);
                                        }
                                    }
                                    else
                                    {
                            ...
                            default:
                                this.logger.LogWarning("Unexpected element '{LocalName}' encountered while reading Dependency at line:position {LineNumber}:{LinePosition}", xmiReader.LocalName, xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                                xmiReader.Skip();
                                break;
                        }
                    }
        ...
        /// <summary>
        /// Reads asynchronously the <see cref="IDependency" /> object from its XML representation
        /// </summary>
        /// <param name="xmiReader">An instance of <see cref="XmlReader" /></param>
        /// <param name="currentLocation">The <see cref="Uri" /> that keep tracks of the current location</param>
        /// <returns>An awaitable <see cref="Task{TResult}"/> with the read <see cref="IDependency" /></returns>
        public override async Task<IDependency> ReadAsync(XmlReader xmiReader, Uri currentLocation)
        {
            if (xmiReader == null)
            {
                throw new ArgumentNullException(nameof(xmiReader));
