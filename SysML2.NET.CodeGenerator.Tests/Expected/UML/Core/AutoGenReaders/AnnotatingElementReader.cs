...
namespace SysML2.NET.Serializer.Xmi.Readers
{
    ...
    /// <summary>
    /// The purpose of the <see cref="AnnotatingElementReader" /> is to read an instance of <see cref="IAnnotatingElement" />
    ...
    public class AnnotatingElementReader : XmiDataReader<IAnnotatingElement>
    {
        ...
        /// <summary>
        /// Reads asynchronously the <see cref="IAnnotatingElement" /> object from its XML representation
        /// </summary>
        /// <param name="xmiReader">An instance of <see cref="XmlReader" /></param>
        /// <param name="currentLocation">The <see cref="Uri" /> that keep tracks of the current location</param>
        /// <returns>An awaitable <see cref="Task{TResult}"/> with the read <see cref="IAnnotatingElement" /></returns>
        public override async Task<IAnnotatingElement> ReadAsync(XmlReader xmiReader, Uri currentLocation)
        {
            if (xmiReader == null)
            {
                throw new ArgumentNullException(nameof(xmiReader));
