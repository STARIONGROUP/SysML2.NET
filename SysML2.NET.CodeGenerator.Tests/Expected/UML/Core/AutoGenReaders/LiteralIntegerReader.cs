...
namespace SysML2.NET.Serializer.Xmi.Readers
{
    ...
    /// </summary>
    public class LiteralIntegerReader : XmiDataReader<ILiteralInteger>
    {
        /// <summary>
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="LiteralIntegerReader" />
        /// </summary>
        /// <param name="cache">
        ...
        public LiteralIntegerReader(IXmiDataCache cache, IXmiDataReaderFacade xmiDataReaderFacade, IExternalReferenceService externalReferenceService, ILoggerFactory loggerFactory, IXmiElementOriginMap elementOriginMap = null) : base(cache, xmiDataReaderFacade, externalReferenceService, loggerFactory, elementOriginMap)
        {
        ...
        /// <returns>The read <see cref="ILiteralInteger" /></returns>
        public override ILiteralInteger Read(XmlReader xmiReader, Uri currentLocation)
        {
            if (xmiReader == null)
            {
            ...
            ILiteralInteger poco = new SysML2.NET.Core.POCO.Kernel.Expressions.LiteralInteger();
            ...
            if (xmiReader.MoveToContent() == XmlNodeType.Element)
            {
                ...
                if (!string.IsNullOrEmpty(xsiType) && xsiType != "sysml:LiteralInteger")
                {
                    throw new InvalidOperationException($"The xsi:type {xsiType} is not supported by the LiteralIntegerReader");
                }
                ...
                {
                    this.logger.LogCritical("Failed to add element type [{Poco}] with id [{Id}] as it was already in the Cache. The XMI document seems to have duplicate xmi:id values", "LiteralInteger", poco.Id);
                }
                ...
                {
                    if (int.TryParse(valueXmlAttribute, out var valueXmlAttributeAsInt))
                    {
                        poco.Value = valueXmlAttributeAsInt;
                    }
                }
                ...
                while (xmiReader.Read())
                {
                    if (xmiReader.NodeType == XmlNodeType.Element)
                    {
                        switch (xmiReader.LocalName)
                        {
                            ...
                            case "value":
                                {
                                    ...
                                    {
                                        if (int.TryParse(valueValue, out var valueValueAsInt))
                                        {
                                            poco.Value = valueValueAsInt;
                                        }
                                        else
                                        {
                                            this.logger.LogWarning("Failed to parse int value '{Value}' for property 'value' on element {ElementId}", valueValue, poco.Id);
                                        }
                                    }
                            ...
                            default:
                                this.logger.LogWarning("Unexpected element '{LocalName}' encountered while reading LiteralInteger at line:position {LineNumber}:{LinePosition}", xmiReader.LocalName, xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                                xmiReader.Skip();
                                break;
                        }
                    }
        ...
        /// <summary>
        /// Reads asynchronously the <see cref="ILiteralInteger" /> object from its XML representation
        /// </summary>
        /// <param name="xmiReader">An instance of <see cref="XmlReader" /></param>
        /// <param name="currentLocation">The <see cref="Uri" /> that keep tracks of the current location</param>
        /// <returns>An awaitable <see cref="Task{TResult}"/> with the read <see cref="ILiteralInteger" /></returns>
        public override async Task<ILiteralInteger> ReadAsync(XmlReader xmiReader, Uri currentLocation)
        {
            if (xmiReader == null)
            {
                throw new ArgumentNullException(nameof(xmiReader));
