// -------------------------------------------------------------------------------------------------
// <copyright file="FeatureDictionaryWriter.cs" company="Starion Group S.A.">
//
...
namespace SysML2.NET.Serializer.Dictionary.Core.DTO
{
    ...
    /// <summary>
    /// The purpose of the <see cref="FeatureDictionaryWriter"/> is to write (convert) a <see cref="IFeature"/>
    /// to a <see cref="Dictionary{String, Object}"/>.
    /// </summary>
    public static class FeatureDictionaryWriter
    {
        /// <summary>
        /// Writes a <see cref="IFeature"/> to a <see cref="Dictionary{String, Object}"/> that contains a key-value-pair
        /// for each property. The type key is used to store type information (name of the Type).
        /// </summary>
        /// <param name="dataItem">
        /// The subject <see cref="IFeature"/> that is to be written to a <see cref="Dictionary{String, Object}"/>.
        /// </param>
        ...
        {
            var featureInstance = ThingNullAndTypeCheck(dataItem);

            switch (dictionaryKind)
            {
                case DictionaryKind.Complex:
                    return WriteComplex(featureInstance);
                case DictionaryKind.Simplified:
                    return WriteSimplified(featureInstance);
                default:
        ...
        /// the values are converted to an Array of String using JSON notation, i.e. [ value_1, ..., value_n ]
        /// </remarks>
        private static Dictionary<string, object> WriteSimplified(IFeature featureInstance)
        {
            var dictionary = new Dictionary<string, object>
            {
                { "@type", "Feature" },
                { "@id", featureInstance.Id.ToString() }
            };
        ...
        /// </remarks>
        private static Dictionary<string, object> WriteComplex(IFeature featureInstance)
        {
            var dictionary = new Dictionary<string, object>
            {
                { "@type", "Feature" },
                { "@id", featureInstance.Id }
            };
        ...
        /// Thrown when <paramref name="thing"/> is not of type <see cref="IFeature"/>
        /// </exception>
        private static IFeature ThingNullAndTypeCheck(IData dataItem)
        {
            if (dataItem == null)
            {
            ...
            if (!(dataItem is IFeature featureInstance))
            {
                throw new ArgumentException("The dataItem must be of Type IFeature", "dataItem");
            }
