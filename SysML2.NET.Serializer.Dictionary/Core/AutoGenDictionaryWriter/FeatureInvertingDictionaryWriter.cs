// -------------------------------------------------------------------------------------------------
// <copyright file="FeatureInvertingDictionaryWriter.cs" company="Starion Group S.A.">
//
...
namespace SysML2.NET.Serializer.Dictionary.Core.DTO
{
    ...
    /// <summary>
    /// The purpose of the <see cref="FeatureInvertingDictionaryWriter"/> is to write (convert) a <see cref="IFeatureInverting"/>
    /// to a <see cref="Dictionary{String, Object}"/>.
    /// </summary>
    public static class FeatureInvertingDictionaryWriter
    {
        /// <summary>
        /// Writes a <see cref="IFeatureInverting"/> to a <see cref="Dictionary{String, Object}"/> that contains a key-value-pair
        /// for each property. The type key is used to store type information (name of the Type).
        /// </summary>
        /// <param name="dataItem">
        /// The subject <see cref="IFeatureInverting"/> that is to be written to a <see cref="Dictionary{String, Object}"/>.
        /// </param>
        ...
        {
            var featureInvertingInstance = ThingNullAndTypeCheck(dataItem);

            switch (dictionaryKind)
            {
                case DictionaryKind.Complex:
                    return WriteComplex(featureInvertingInstance);
                case DictionaryKind.Simplified:
                    return WriteSimplified(featureInvertingInstance);
                default:
        ...
        /// for each property. The type key is used to store type information (name of the Type).
        /// </summary>
        /// <param name="featureInvertingInstance">
        /// The subject <see cref="IFeatureInverting"/> that is to be written to a <see cref="Dictionary{String, Object}"/>.
        /// </param>
        /// <returns>
        /// An instance of <see cref="Dictionary{String, Object}"/> that contains all the properties as key-value-pairs as well
        ...
        /// the values are converted to an Array of String using JSON notation, i.e. [ value_1, ..., value_n ]
        /// </remarks>
        private static Dictionary<string, object> WriteSimplified(IFeatureInverting featureInvertingInstance)
        {
            var dictionary = new Dictionary<string, object>
            {
                { "@type", "FeatureInverting" },
                { "@id", featureInvertingInstance.Id.ToString() }
            };
        ...
        /// </remarks>
        private static Dictionary<string, object> WriteComplex(IFeatureInverting featureInvertingInstance)
        {
            var dictionary = new Dictionary<string, object>
            {
                { "@type", "FeatureInverting" },
                { "@id", featureInvertingInstance.Id }
            };
        ...
        /// of type <see cref="IFeatureInverting"/>
        /// </summary>
        /// <param name="dataItem">
        ...
        /// Thrown when <paramref name="thing"/> is not of type <see cref="IFeatureInverting"/>
        /// </exception>
        private static IFeatureInverting ThingNullAndTypeCheck(IData dataItem)
        {
            if (dataItem == null)
            {
            ...
            if (!(dataItem is IFeatureInverting featureInvertingInstance))
            {
                throw new ArgumentException("The dataItem must be of Type IFeatureInverting", "dataItem");
            }
