// -------------------------------------------------------------------------------------------------
// <copyright file="LiteralInfinityDictionaryWriter.cs" company="Starion Group S.A.">
//
...
namespace SysML2.NET.Serializer.Dictionary.Core.DTO
{
    ...
    /// <summary>
    /// The purpose of the <see cref="LiteralInfinityDictionaryWriter"/> is to write (convert) a <see cref="ILiteralInfinity"/>
    /// to a <see cref="Dictionary{String, Object}"/>.
    /// </summary>
    public static class LiteralInfinityDictionaryWriter
    {
        /// <summary>
        /// Writes a <see cref="ILiteralInfinity"/> to a <see cref="Dictionary{String, Object}"/> that contains a key-value-pair
        /// for each property. The type key is used to store type information (name of the Type).
        /// </summary>
        /// <param name="dataItem">
        /// The subject <see cref="ILiteralInfinity"/> that is to be written to a <see cref="Dictionary{String, Object}"/>.
        /// </param>
        ...
        {
            var literalInfinityInstance = ThingNullAndTypeCheck(dataItem);

            switch (dictionaryKind)
            {
                case DictionaryKind.Complex:
                    return WriteComplex(literalInfinityInstance);
                case DictionaryKind.Simplified:
                    return WriteSimplified(literalInfinityInstance);
                default:
        ...
        /// for each property. The type key is used to store type information (name of the Type).
        /// </summary>
        /// <param name="literalInfinityInstance">
        /// The subject <see cref="ILiteralInfinity"/> that is to be written to a <see cref="Dictionary{String, Object}"/>.
        /// </param>
        /// <returns>
        /// An instance of <see cref="Dictionary{String, Object}"/> that contains all the properties as key-value-pairs as well
        ...
        /// the values are converted to an Array of String using JSON notation, i.e. [ value_1, ..., value_n ]
        /// </remarks>
        private static Dictionary<string, object> WriteSimplified(ILiteralInfinity literalInfinityInstance)
        {
            var dictionary = new Dictionary<string, object>
            {
                { "@type", "LiteralInfinity" },
                { "@id", literalInfinityInstance.Id.ToString() }
            };
        ...
        /// </remarks>
        private static Dictionary<string, object> WriteComplex(ILiteralInfinity literalInfinityInstance)
        {
            var dictionary = new Dictionary<string, object>
            {
                { "@type", "LiteralInfinity" },
                { "@id", literalInfinityInstance.Id }
            };
        ...
        /// of type <see cref="ILiteralInfinity"/>
        /// </summary>
        /// <param name="dataItem">
        ...
        /// Thrown when <paramref name="thing"/> is not of type <see cref="ILiteralInfinity"/>
        /// </exception>
        private static ILiteralInfinity ThingNullAndTypeCheck(IData dataItem)
        {
            if (dataItem == null)
            {
            ...
            if (!(dataItem is ILiteralInfinity literalInfinityInstance))
            {
                throw new ArgumentException("The dataItem must be of Type ILiteralInfinity", "dataItem");
            }
