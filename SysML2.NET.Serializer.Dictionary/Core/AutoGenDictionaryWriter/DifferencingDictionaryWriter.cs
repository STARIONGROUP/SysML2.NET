// -------------------------------------------------------------------------------------------------
// <copyright file="DifferencingDictionaryWriter.cs" company="Starion Group S.A.">
//
...
namespace SysML2.NET.Serializer.Dictionary.Core.DTO
{
    ...
    /// <summary>
    /// The purpose of the <see cref="DifferencingDictionaryWriter"/> is to write (convert) a <see cref="IDifferencing"/>
    /// to a <see cref="Dictionary{String, Object}"/>.
    /// </summary>
    public static class DifferencingDictionaryWriter
    {
        /// <summary>
        /// Writes a <see cref="IDifferencing"/> to a <see cref="Dictionary{String, Object}"/> that contains a key-value-pair
        /// for each property. The type key is used to store type information (name of the Type).
        /// </summary>
        /// <param name="dataItem">
        /// The subject <see cref="IDifferencing"/> that is to be written to a <see cref="Dictionary{String, Object}"/>.
        /// </param>
        ...
        {
            var differencingInstance = ThingNullAndTypeCheck(dataItem);

            switch (dictionaryKind)
            {
                case DictionaryKind.Complex:
                    return WriteComplex(differencingInstance);
                case DictionaryKind.Simplified:
                    return WriteSimplified(differencingInstance);
                default:
        ...
        /// for each property. The type key is used to store type information (name of the Type).
        /// </summary>
        /// <param name="differencingInstance">
        /// The subject <see cref="IDifferencing"/> that is to be written to a <see cref="Dictionary{String, Object}"/>.
        /// </param>
        /// <returns>
        /// An instance of <see cref="Dictionary{String, Object}"/> that contains all the properties as key-value-pairs as well
        ...
        /// the values are converted to an Array of String using JSON notation, i.e. [ value_1, ..., value_n ]
        /// </remarks>
        private static Dictionary<string, object> WriteSimplified(IDifferencing differencingInstance)
        {
            var dictionary = new Dictionary<string, object>
            {
                { "@type", "Differencing" },
                { "@id", differencingInstance.Id.ToString() }
            };
        ...
        /// </remarks>
        private static Dictionary<string, object> WriteComplex(IDifferencing differencingInstance)
        {
            var dictionary = new Dictionary<string, object>
            {
                { "@type", "Differencing" },
                { "@id", differencingInstance.Id }
            };
        ...
        /// of type <see cref="IDifferencing"/>
        /// </summary>
        /// <param name="dataItem">
        ...
        /// Thrown when <paramref name="thing"/> is not of type <see cref="IDifferencing"/>
        /// </exception>
        private static IDifferencing ThingNullAndTypeCheck(IData dataItem)
        {
            if (dataItem == null)
            {
            ...
            if (!(dataItem is IDifferencing differencingInstance))
            {
                throw new ArgumentException("The dataItem must be of Type IDifferencing", "dataItem");
            }
