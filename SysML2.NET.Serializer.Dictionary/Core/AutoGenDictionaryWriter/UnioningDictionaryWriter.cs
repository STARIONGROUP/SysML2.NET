// -------------------------------------------------------------------------------------------------
// <copyright file="UnioningDictionaryWriter.cs" company="Starion Group S.A.">
//
...
namespace SysML2.NET.Serializer.Dictionary.Core.DTO
{
    ...
    /// <summary>
    /// The purpose of the <see cref="UnioningDictionaryWriter"/> is to write (convert) a <see cref="IUnioning"/>
    /// to a <see cref="Dictionary{String, Object}"/>.
    /// </summary>
    public static class UnioningDictionaryWriter
    {
        /// <summary>
        /// Writes a <see cref="IUnioning"/> to a <see cref="Dictionary{String, Object}"/> that contains a key-value-pair
        /// for each property. The type key is used to store type information (name of the Type).
        /// </summary>
        /// <param name="dataItem">
        /// The subject <see cref="IUnioning"/> that is to be written to a <see cref="Dictionary{String, Object}"/>.
        /// </param>
        ...
        {
            var unioningInstance = ThingNullAndTypeCheck(dataItem);

            switch (dictionaryKind)
            {
                case DictionaryKind.Complex:
                    return WriteComplex(unioningInstance);
                case DictionaryKind.Simplified:
                    return WriteSimplified(unioningInstance);
                default:
        ...
        /// the values are converted to an Array of String using JSON notation, i.e. [ value_1, ..., value_n ]
        /// </remarks>
        private static Dictionary<string, object> WriteSimplified(IUnioning unioningInstance)
        {
            var dictionary = new Dictionary<string, object>
            {
                { "@type", "Unioning" },
                { "@id", unioningInstance.Id.ToString() }
            };
        ...
        /// </remarks>
        private static Dictionary<string, object> WriteComplex(IUnioning unioningInstance)
        {
            var dictionary = new Dictionary<string, object>
            {
                { "@type", "Unioning" },
                { "@id", unioningInstance.Id }
            };
        ...
        /// Thrown when <paramref name="thing"/> is not of type <see cref="IUnioning"/>
        /// </exception>
        private static IUnioning ThingNullAndTypeCheck(IData dataItem)
        {
            if (dataItem == null)
            {
            ...
            if (!(dataItem is IUnioning unioningInstance))
            {
                throw new ArgumentException("The dataItem must be of Type IUnioning", "dataItem");
            }
